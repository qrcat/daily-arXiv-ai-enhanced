...
<details>
  <summary>Details</summary>
Motivation: 探索ViTs在医学图像分析中的对抗性攻击脆弱性，尤其是对抗性水印的影响。

Method: 使用投影梯度下降（PGD）生成对抗性水印，测试ViTs和CNNs的脆弱性，并评估对抗训练的防御效果。

Result: ViTs对对抗性攻击非常脆弱（准确率降至27.6%），但对抗训练能显著提升防御性能（准确率恢复至90.0%）。
...
TL;DR: 提出了一种基于RGB智能手机摄像头的远程健身追踪系统，解决了现有方法在运动多样性和复杂性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有健身追踪模型要么运动种类有限，要么过于复杂难以部署，需要一种更私密、可扩展且成本效益高的解决方案。

Method: 开发了一个多任务运动分析模型，结合大规模健身数据集Olympia（涵盖1900多种运动），利用视觉语言模型进行运动检测和重复计数。

Result: 模型在Olympia数据集上运动检测准确率为76.5%，重复计数准确率为85.3%（误差±1）。

Conclusion: 通过单一视觉语言模型实现运动识别和计数，为AI健身追踪的普及迈出重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 传统SLAM算法在相机跟踪上表现优异，但生成的3D地图分辨率低且不完整；现有GS方法依赖场景优化，耗时长且泛化性不足。

Method: 使用RGB-D图像识别主干预测高斯参数，集成3D语义分割，并通过全局定位后仅优化1次高斯泼溅来修正定位漂移和浮动物。

Result: 在ScanNet基准测试中实现了最先进的语义SLAM性能，高斯数量比同类方法少一个数量级，并在NYUv2和TUM RGB-D数据集上展示了零样本泛化能力。

Conclusion: 该方法通过可泛化网络和高斯泼溅框架，高效构建高分辨率3D语义地图，解决了现有方法的局限性。
...
TL;DR: 论文研究了针对交通标志识别深度神经网络的错误最小化攻击，并提出基于数据增强的训练方法和检测模型以增强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 由于DNNs在交通标志识别中广泛使用且训练数据来源未知，确保模型安全免受攻击至关重要。

Method: 通过添加微小扰动进行攻击，提出基于非线性变换的数据增强训练方法，并设计检测模型识别中毒数据。

Result: 攻击使准确率从99.9%降至10.6%，但提出的方法恢复至96.05%，检测模型识别成功率超99%。

Conclusion: 需采用先进训练方法以抵御数据中毒攻击，提升交通标志识别系统的安全性。
...
TL;DR: 论文探讨了利用RGB图像和深度学习技术实现纺织品自动分类和污染物分割的可行性，为自动化纺织品回收提供了高效解决方案。


<details>
  <summary>Details</summary>
Motivation: 纺织品回收中材料成分识别和污染物检测的挑战促使研究低成本RGB图像的应用。

Method: 采用预训练架构（如EfficientNetB0）进行纺织品分类，结合Grounding DINO和SAM模型实现零样本分割。

Result: 分类准确率达81.25%，分割任务mIoU为0.90，表现优异。

Conclusion: RGB图像结合现代深度学习技术可有效支持自动化纺织品回收的关键预处理任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有监控系统因图像质量低和数据集偏见导致面部识别准确性下降，尤其在肤色变化和部分遮挡情况下表现不佳。

Method: 采用基于深度学习的面部属性操纵和重建技术（如自动编码器和GANs），生成多样化合成数据，并集成图像增强模块提升低分辨率或遮挡面部的清晰度。
...
TL;DR: 论文提出了一种自监督CNN（EV-LayerSegNet），用于事件相机的运动分割，通过分层场景动态表示学习光流和分割掩码，并利用去模糊质量作为自监督损失。


<details>
  <summary>Details</summary>
Motivation: 事件相机的高时间分辨率适合运动任务，但获取真实标签昂贵且困难，因此需要自监督方法。

Method: 通过分层动态表示分别学习仿射光流和分割掩码，利用去模糊质量作为自监督损失。

Result: 在模拟数据集上实现了71%的IoU和87%的检测率。
...
TL;DR: 提出了一种名为RARL的框架，通过强化学习提升医学视觉语言模型的推理能力，同时保持高效性和低资源适应性。
...
Result: RARL在医学图像分析和临床推理任务中显著优于监督微调，推理任务性能提升约7.78%，且计算资源需求更低。

Conclusion: 推理引导学习和推理提示能显著提升医学视觉语言模型的透明性、准确性和资源效率，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 零样本组合图像检索（CIR）的性能较低（20-25% Recall@10），需要改进以支持更精确的图像检索。

Method: 1. 微调BLIP-2，结合轻量级Q-Former融合视觉和文本特征。2. 尝试Retrieval-DPO方法，优化CLIP文本编码器，但效果不佳。

Result: 改进方法在FashionIQ基准测试中Recall@10提升至45.6%（衬衫）、40.1%（裙子）和50.4%（T恤），平均Recall@50达67.6%。Retrieval-DPO表现极差（0.02% Recall@10）。
...
TL;DR: PhysLab是一个针对教育场景的视频数据集，专注于学生进行复杂物理实验的场景，填补了现有数据集在标注粒度、领域覆盖和逻辑规则方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有数据集在标注粒度、领域覆盖和逻辑规则方面存在不足，限制了视觉解析在教育场景中的应用。

Method: 提出PhysLab数据集，包含620个长视频，涵盖四个代表性物理实验，提供多层次标注支持多种视觉任务。

Result: 建立了强基线并进行了广泛评估，突出了解析教育视频中的关键挑战。

Conclusion: PhysLab有望推动细粒度视觉解析和智能课堂系统的发展，促进计算机视觉与教育技术的结合。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度散焦（DFD）技术通常依赖多张不同光圈或对焦设置的图像来恢复深度信息，而单张散焦图像的DFD研究较少，因其问题约束不足。本文旨在解决这一问题。

Method: 通过利用局部散焦模糊与对比度变化的关系作为关键深度线索，结合暗通道先验，以端到端对抗训练的方式提升深度估计性能。

Result: 在真实数据上的实验表明，将暗通道先验引入单图像DFD中能获得有意义的深度估计结果。

Conclusion: 该方法验证了暗通道先验在单图像DFD中的有效性，为单张散焦图像的深度估计提供了新思路。
...
TL;DR: PGHM是一种基于3D高斯泼溅的通用高效框架，用于从单目视频快速重建高质量人体化身，解决了现有方法优化时间长和泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅方法在高质量渲染方面有优势，但仍面临单目输入下优化时间长和泛化能力不足的挑战。

Method: PGHM引入UV对齐的潜在身份图和分离的多头U-Net，通过条件解码器分解静态、姿态依赖和视角依赖的高斯属性。

Result: PGHM仅需约20分钟即可完成高质量化身重建，效率显著优于从头优化的方法。

Conclusion: PGHM展示了其在真实世界单目化身创建中的实用性和高效性。
...
TL;DR: Flood-DamageSense是一种专为洪水灾害建筑损坏评估设计的深度学习框架，通过融合多源数据提升分类性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有模型在洪水灾害后建筑损坏分类中表现不佳，主要因破坏特征不明显。

Method: 结合SAR/InSAR、高分辨率光学底图和长期洪水风险层，采用多模态Mamba架构和半孪生编码器进行多任务预测。

Result: 在Hurricane Harvey数据上，F1分数比现有方法提升19个百分点，尤其在轻微和中等损坏类别中表现突出。

Conclusion: Flood-DamageSense通过风险感知建模和SAR全天候能力，为灾后决策提供更快、更精细的损坏评估。
...
TL;DR: 本文提出了一种基于CNN-LSTM的可解释人工智能框架，用于高效分类胚胎图像，解决传统胚胎评分方法效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 不孕症对生活质量有显著影响，体外受精（IVF）是主要解决方案之一，但传统胚胎评分方法耗时且效率低。

Method: 采用CNN-LSTM融合架构，结合深度学习技术，实现高精度胚胎分类，并通过XAI保持模型可解释性。

Result: 模型在胚胎分类中表现出高准确性，同时具备可解释性。

Conclusion: 该研究为胚胎选择提供了一种高效且可解释的AI解决方案，有望提升IVF成功率。
...
Method: 系统回顾了深度学习方法，并引入真实退化数据集360Insta，进行全面评估。

Result: 新数据集填补了现有基准的空白，支持更鲁棒的超分辨率方法评估。

Conclusion: 总结了研究现状并展望未来方向，所有资源公开并定期更新。
...
Result: 实验表明，HMCF-ACMs和HDRF-ACMs在噪声抵抗和数值稳定性方面表现优越，能实现更精确的分割。

Conclusion: HMCF-ACMs和HDRF-ACMs通过任务自适应的初始配置，显著提升了图像分割的性能。
...
TL;DR: 研究探讨了野生动物（特别是非洲五大动物）的分布外检测问题，比较了参数化和非参数化方法，发现基于特征的方法在泛化能力上表现更优。


<details>
  <summary>Details</summary>
Motivation: 解决人类与野生动物冲突需要准确识别潜在威胁个体，但现有分类模型在未知类别上表现不佳，因此研究分布外检测方法。

Method: 选择了参数化的最近类均值（NCM）和非参数化的对比学习方法作为基线，并与其他常见分布外检测方法进行比较。

Result: 基于特征的方法（如NCM）在AUPR-IN、AUPR-OUT和AUTC指标上分别比最佳分布外方法提升了2%、4%和22%。

Conclusion: 特征方法在野生动物分布外检测中表现更优，为实际应用提供了更可靠的解决方案。
...
TL;DR: 论文提出了一种名为LPS的解码方法，用于减少多模态大语言模型中的幻觉现象，无需额外训练，且在噪声环境下表现优异。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视觉与语言融合方面取得了成功，但仍存在输出与图像内容不符的幻觉问题。

Method: 提出Local Perception Search (LPS)解码方法，利用局部视觉先验信息修正解码过程。

Result: 实验表明，LPS显著减少了幻觉现象，尤其在噪声环境下表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集中食谱目标、步骤说明和视觉内容之间缺乏细粒度对齐，限制了食品计算的应用。

Method: 提出RecipeGen数据集，并设计领域特定的评估指标来评估成分保真度和交互建模。

Result: RecipeGen覆盖了多样化的食材、烹饪过程、风格和菜肴类型，并提供了对代表性模型的基准测试结果。

Conclusion: RecipeGen为未来的食谱生成模型提供了基准和见解。
...
TL;DR: 本文提出了一种结合视觉预训练和深度几何线索的自我中心视频对象分割方法，性能优异。
...
TL;DR: 论文提出了一种新任务：从单视角SAR图像中恢复目标结构，通过两步算法框架实现3D语义结构表示。
...
Conclusion: 该方法为SAR图像的高级信息检索提供了新的语义理解途径。
...
<details>
  <summary>Details</summary>
Motivation: 生物识别认证系统易受欺骗攻击，现有研究多集中于特定模态的反欺骗技术，缺乏跨模态的统一高效解决方案。

Method: 提出LitMAS框架，采用模态对齐集中损失（Modality-Aligned Concentration Loss），增强类间分离性并保持跨模态一致性。

Result: LitMAS仅需6M参数，在七个数据集上的平均EER比现有最优方法低1.36%，高效且通用。

Conclusion: LitMAS在轻量化和跨模态通用性方面表现优异，适合边缘部署。
...
TL;DR: LoopDB是一个包含1000多张图像的闭环数据集，用于测试和训练闭环算法，支持SLAM和深度学习。


<details>
  <summary>Details</summary>
Motivation: 提供多样化的闭环数据集，用于评估和改进闭环算法的准确性，尤其是在SLAM和深度学习中的应用。

Method: 使用高分辨率相机采集图像，每场景包含五张连续图像，并提供旋转和平移的真实数据。

Result: 数据集公开可用，适用于算法基准测试和深度学习模型的训练与微调。
...
TL;DR: 该论文提出了一种基于神经控制微分方程和Savitzky-Golay路径的连续时间旋转物体动力学建模方法，解决了SO(3)外推中的噪声、稀疏观测和复杂动态问题。


<details>
  <summary>Details</summary>
Motivation: SO(3)外推在计算机视觉和机器人学中至关重要，但面临噪声观测、稀疏数据、复杂动态和长期预测需求等挑战。

Method: 使用神经控制微分方程（Neural Controlled Differential Equations）结合Savitzky-Golay路径，在SO(3)上建模旋转物体的连续时间动力学。

Result: 实验结果表明，该方法在真实数据上表现出优于现有方法的预测能力。

Conclusion: 该方法能够学习旋转轨迹的潜在动态系统，同时保持旋转的几何结构，为复杂动态下的长期预测提供了有效解决方案。
...
TL;DR: 提出了一种无需训练的扩散模型框架，用于在风格迁移中更好地保持身份信息，特别适用于小面部或远距离拍摄的图像。


<details>
  <summary>Details</summary>
Motivation: 现有风格迁移技术在保持身份信息方面表现不佳，尤其是在小面部或远距离图像中。

Method: 引入了"马赛克恢复内容图像"技术和无需训练的内容一致性损失，以增强身份保留和细节保护。

Result: 实验表明，该方法在保持高风格保真度和身份完整性方面显著优于基线模型。

Conclusion: 无需重新训练或微调，即可在小面部或远距离条件下实现高质量风格迁移和身份保留。
...
TL;DR: RDVP-MSD是一种无需训练的自适应框架，通过结合区域约束的双流视觉提示和多模态逐步分解链式思维，解决了伪装物体分割中的语义模糊和空间分离问题，实现了最先进的性能。
...
Result: 在多个COS基准测试中实现了最先进的性能，且推理速度更快。

Conclusion: RDVP-MSD无需训练即可显著提升分割准确性和效率，为伪装物体分割提供了有效解决方案。
...
TL;DR: Hi-LSplat提出了一种基于3D高斯溅射的分层语言模型，解决了视图不一致和开放词汇挑战，提升了3D场景的语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS模型依赖2D基础模型，导致视图不一致和开放词汇描述不一致，阻碍了分层语义理解。

Method: 通过构建3D分层语义树和引入实例级与部件级对比损失，实现视图一致的分层语义表示。

Result: 实验表明Hi-LSplat在3D开放词汇分割和定位中表现优异，尤其在分层语义数据集上。

Conclusion: Hi-LSplat有效解决了3D语义表示中的视图不一致问题，提升了复杂分层语义的捕捉能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究VP在鲁棒源模型下的表现，探索其是否能继承鲁棒性，以及是否存在鲁棒性与泛化能力的权衡问题。

Method: 提出了一种轻量级、即插即用的策略PBL，用于优化VP在鲁棒源模型下的表现。

Result: 实验表明，PBL能有效继承源模型的鲁棒性，并显著提升VP在下游任务中的泛化能力。

Conclusion: PBL是一种有效的策略，能够解决VP在鲁棒性和泛化能力之间的权衡问题，具有广泛适用性。
...
TL;DR: EndoARSS是一种基于DINOv2的多任务学习框架，用于内窥镜手术活动识别和语义分割，通过低秩适应和空间感知多尺度注意力提升性能，并在新数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 内窥镜手术场景复杂，传统深度学习模型在跨活动干扰下表现不佳，多任务学习可利用任务间关联提升性能。

Method: 基于DINOv2，结合低秩适应和任务高效共享低秩适配器，引入空间感知多尺度注意力增强特征表示。

Result: EndoARSS在多个基准测试中表现优异，显著提高了准确性和鲁棒性。
...
TL;DR: 该论文提出了一种基于视觉语言模型（VLM）的两阶段时间序列异常检测方法，显著提升了检测准确性和效率。
...
Method: 提出两阶段解决方案：ViT4TS（基于轻量级视觉编码器的候选异常定位）和VLM4TS（基于VLM的全局时间上下文整合与推理）。

Result: VLM4TS在未进行时间序列训练的情况下，F1-max分数比最佳基线提高24.6%，且效率提升36倍。

Conclusion: VLM4TS在时间序列异常检测中表现出色，优于现有方法，且高效实用。
...
TL;DR: 论文提出Multi-StyleGS方法，通过局部风格迁移和语义风格损失函数，实现3D高斯溅射场景的多风格适配，同时保持内存高效训练。


<details>
  <summary>Details</summary>
Motivation: 解决3D高斯溅射(GS)在适配多风格时面临的局部风格迁移和内存效率问题。

Method: 采用二分匹配机制自动匹配风格图像与渲染图像局部区域，提出语义风格损失函数和局部-全局特征匹配技术。

Result: 实验表明，该方法在风格化效果和编辑灵活性上优于现有方法，且内存效率更高。

Conclusion: Multi-StyleGS为3D场景风格化提供了高效且灵活的解决方案。
...
TL;DR: 论文研究了基于神经网络的惯性运动捕捉系统，比较了不同架构和方法在人体姿态估计中的表现，提出了一种高效的混合LSTM-Madgwick方法。


<details>
  <summary>Details</summary>
Motivation: 传统惯性运动捕捉系统依赖复杂生物力学模型和昂贵软件，研究旨在通过神经网络简化这一过程。

Method: 比较了不同神经网络架构和方法，测试了低成本和高端的MARG传感器，并进行了消融研究。

Result: 混合LSTM-Madgwick方法在高端传感器数据上表现最佳，误差为7.96。

Conclusion: 神经网络可用于人体姿态估计，性能接近现有最优融合滤波器。
...
TL;DR: 本文提出了一种基于位置预测的自监督学习方法（LOCA），用于多模态卫星图像的语义分割，显著优于现有的基于重建的自监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 卫星图像的语义分割对地球观测应用至关重要，但受限于标记训练数据的不足。现有方法（如MAE）侧重于重建而非定位，而定位是分割任务的基础。

Method: 通过扩展SatMAE的通道分组至多模态数据，并引入同组注意力掩码以促进预训练中的跨模态交互，同时采用相对补丁位置预测任务以增强空间推理能力。

Result: 在Sen1Floods11洪水映射数据集上，该方法显著优于基于重建的自监督学习方法。

Conclusion: 研究表明，针对多模态卫星图像调整的位置预测任务能学习到更适合语义分割的表示，优于基于重建的方法。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需预测其他代理的运动，现有编码器-解码器模型存在信息滞后问题，DONUT通过解码器专用设计提升预测一致性和实时性。

Method: 使用单自回归模型编码历史轨迹并预测未来轨迹，引入‘过预测’策略辅助预测更长时域轨迹。

Result: DONUT在Argoverse 2单代理运动预测基准上超越基线模型，达到新SOTA。

Conclusion: 解码器专用设计结合‘过预测’策略显著提升轨迹预测性能，为自动驾驶提供更优解决方案。
...
TL;DR: 论文提出了一种名为Vision-EKIPL的新型强化学习框架，通过引入外部辅助模型生成的高质量动作来优化策略模型，显著提升了多模态大语言模型的视觉推理能力和训练效率。
...
Result: 在Reason-RFT-CoT Benchmark上性能提升高达5%，优于现有最佳方法。
...
TL;DR: 本文提出了一种端到端的3D人脸识别方法，结合去噪与识别模块，显著提高了噪声点云下的识别精度。
...
Method: 设计了基于三正交平面的条件生成对抗网络（cGAN-TOP）去噪，并采用链接动态图卷积神经网络（LDGCNN）进行多尺度特征识别。
...
TL;DR: 提出了一种结合Vision Transformers和Vision Mamba的混合深度学习框架，用于通过眼动数据检测自闭症谱系障碍（ASD），显著提高了诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: ASD的准确诊断对早期干预至关重要，传统手工方法效果有限，需要更高效、可扩展的解决方案。
...
Conclusion: 该模型为资源有限或偏远地区的ASD筛查提供了高效、可解释的解决方案。
...
TL;DR: NSD-Imagery是一个新发布的基准数据集，用于评估从fMRI活动重建心理图像的能力，补充了现有的NSD数据集。研究发现，现有模型在心理图像重建上的表现与视觉重建表现脱钩，且简单架构模型泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 现有模型仅针对视觉图像重建进行评估，而心理图像重建在医学和脑机接口领域有重要应用需求。NSD-Imagery填补了这一空白。

Method: 通过NSD-Imagery数据集评估了多种NSD训练的视觉解码模型（如MindEye1、Brain Diffuser等）在心理图像重建上的表现，并分析了架构选择的影响。

Result: 简单线性解码架构和多模态特征解码模型在心理图像重建上表现更好，复杂架构容易过拟合视觉数据。心理图像重建性能与视觉重建性能脱钩。
...
Method: 基于流形假设和特征空间中的最近邻搜索，KNN-Defense通过恢复受扰动的输入而非重建表面几何或强制均匀点分布来防御攻击。

Result: 在ModelNet40数据集上，KNN-Defense显著提升了鲁棒性，特别是在点丢弃攻击下，对PointNet、PointNet++、DGCNN和PCT的准确率分别提升了20.1%、3.6%、3.44%和7.74%。
...
TL;DR: 论文提出了一种动态场景适应机制和关键帧管理机制，用于解决3D高斯泼溅技术在长期动态场景中的局限性，提升了新视角合成的性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅技术主要针对静态场景，对长期动态场景（如场景在视野外变化）的研究较少，需要一种机制来持续更新3D表示并保持几何和语义一致性。

Method: 引入动态场景适应机制，持续更新3D表示；提出关键帧管理机制，丢弃过时观测并保留有效信息。
...
TL;DR: 该论文提出了一种基于ImageBind的多模态嵌入深度学习模型，结合压力敏感垫（PSM）和双通道眼电图（EOG）信号，用于睡眠阶段分类，显著提高了分类准确性。


<details>
  <summary>Details</summary>
Motivation: 传统多导睡眠图（PSG）依赖脑电图（EEG），设备复杂且不适合家庭监测，因此需要开发更便捷的替代方法。

Method: 利用ImageBind模型整合PSM和双通道EOG数据，进行五阶段睡眠-觉醒分类。

Result: 该方法在85晚患者数据上表现优异，优于单通道EOG（DeepSleepNet）、纯PSM（ViViT）及其他多模态方法（MBT），且无需微调也能表现良好。

Conclusion: 预训练多模态嵌入模型可有效用于睡眠分期，接近依赖复杂EEG数据的系统，适合医疗应用。
...
TL;DR: 提出了一种基于事件的OCR方法，利用用户注视点减少带宽，并在低光和高动态场景中优于传统RGB相机。
...
Method: 利用用户注视点聚焦事件流，结合深度二值重建和多模态LLM进行OCR。

Result: 在低光环境下成功读取文本，带宽消耗比RGB相机低2400倍。

Conclusion: 事件相机结合注视点聚焦是一种高效的低带宽OCR解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探索Video LLMs在时间推理上的表现，以及图像和视频训练对模型能力的影响。

Method: 使用LongVU算法训练LLMs，并在TVBench基准上测试其时间推理能力，同时提出一种基于图像序列的微调方案。

Result: 图像训练的LLMs在时间推理任务上表现显著优于随机水平，且接近或优于视频训练的LLMs。
...
TL;DR: PHiM是一种新型状态空间模型，专为极坐标流式LiDAR设计，通过局部双向Mamba块和全局前向Mamba块实现高效目标检测，性能优于现有流式检测器。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要低延迟、高吞吐量的实时感知，传统LiDAR处理方法因全扫描和极坐标几何不匹配导致性能下降或复杂失真缓解。

Method: PHiM采用局部双向Mamba块进行扇区内空间编码，全局前向Mamba块进行扇区间时间建模，替代卷积和位置编码。
...
Method: 基于预训练视觉语言模型，采用分层时间步专家流架构和条件残差注意力机制。

Result: 在多模态理解任务中表现优异，图像生成质量与现有模型相当，推理速度快6倍。

Conclusion: LaTtE-Flow在性能和效率上取得了显著提升，适用于实时或资源受限场景。
...
TL;DR: 论文提出了一种针对光学字符识别任务优化的超分辨率网络，通过多任务学习框架和动态权重调整机制，结合高级视觉任务的辅助损失函数，提升了真实场景下的文本检测性能。
...
Method: 采用多任务学习框架，结合文本检测、识别、关键点定位和色调一致性等辅助损失函数，并使用动态权重调整机制平衡目标。基于SRResNet架构实现。

Result: 在模拟和真实文档数据集上的实验表明，该方法提高了文本检测的IoU指标，同时保持了图像保真度。

Conclusion: 多目标优化有助于缩小模拟训练与真实部署之间的差距，提升超分辨率模型在真实场景中的实用性。
...
TL;DR: AR-RAG是一种新的图像生成范式，通过自回归方式在生成过程中动态检索并整合最相关的视觉参考，解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成前进行静态检索，容易导致过复制和风格偏差等问题，AR-RAG旨在动态适应生成需求。

Method: 提出两种框架：DAiD（直接合并预测与检索的分布）和FAiD（通过多尺度卷积平滑检索特征并增强生成）。

Result: 在多个基准测试中（如Midjourney-30K、GenEval和DPG-Bench），AR-RAG显著优于现有图像生成模型。

Conclusion: AR-RAG通过动态检索和特征增强，显著提升了图像生成的性能与灵活性。
...
TL;DR: 本文提出了一种双视角中国手语数据集NationalCSL-DP，并设计了一个CNN-Transformer网络作为基线模型，通过融合策略显著提升了孤立手语识别的性能。
...
Method: 构建了覆盖中国手语词汇的双视角数据集NationalCSL-DP，并提出CNN-Transformer网络及融合策略进行预测。

Result: 实验证明数据集和基线模型有效，融合策略显著提升性能，但序列到序列模型难以学习双视角的互补特征。

Conclusion: 双视角数据集和融合策略为孤立手语识别提供了新思路，但模型仍需优化以更好地利用多视角信息。
...
<details>
  <summary>Details</summary>
Motivation: CLIP特征空间中存在显著的模态鸿沟，现有MLLM检索器依赖粗粒度对齐机制，限制了潜力。

Method: 提出MAPLE框架，结合MLLM的细粒度对齐先验，通过自动偏好数据构建和RPA损失进行强化学习。

Result: 实验表明，MAPLE在细粒度跨模态检索中表现显著提升。

Conclusion: MAPLE通过偏好引导对齐，有效处理语义细微差异，为跨模态学习提供新方向。
...
<details>
  <summary>Details</summary>
Motivation: 复杂纹理区域需要大量高斯点云来准确捕捉颜色变化，导致渲染速度低下。

Method: 通过修剪和优化网格处理平坦区域，同时保留高斯点云用于复杂几何，采用联合优化和透射感知监督。

Result: 实验表明，混合表示在保持渲染质量的同时，显著提升了FPS并减少了高斯点云数量。

Conclusion: 混合表示方法在室内场景中实现了高效的渲染速度和高质量的视觉效果。
...
TL;DR: 论文提出了一种名为COGO的策略，通过增强共享特征和抑制个体特征，显著提高了对抗样本在黑盒设置中的迁移性。
...
Method: COGO包括两部分：共性增强（CE）扰动中低频区域，个性抑制（IS）通过自适应阈值评估梯度相关性。

Result: 实验表明COGO显著提高了对抗攻击的迁移成功率，优于现有方法。

Conclusion: COGO通过优化共享特征和抑制个体特征，有效提升了对抗样本的迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 解决智能手机摄影中广角图像与长焦图像之间的域差距问题，并实现高效的高频细节转移。

Method: 通过学习两个压缩全局表示来弥合域差距，并设计多尺度匹配模块进行跨尺度特征匹配。此外，引入关键修剪以减少内存和推理时间。
...
TL;DR: 本文介绍了ICRA 2025 GOOSE 3D语义分割挑战赛获胜方案的技术细节，通过结合Point Prompt Tuning和Point Transformer v3，实现了对异构LiDAR数据的自适应处理，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人平台采集的异构3D点云语义分割问题，提升模型在复杂户外环境中的适应性。

Method: 采用Point Prompt Tuning与Point Transformer v3结合，通过平台特定条件和跨数据集类别对齐策略处理数据。

Result: 相比基线模型，mIoU提升高达22.59%，证明了方法的有效性。

Conclusion: 自适应点云理解方法在野外机器人应用中具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖于特定部署的调优或预定义的运动先验，而UNO旨在泛化适用于多种实际场景，如自动驾驶、无人机、移动机器人和手持设备。

Method: 采用专家混合策略进行局部状态估计，结合多个专用解码器处理不同类别的自运动模式，并使用可微分的Gumbel-Softmax模块构建帧间关联图、选择最优解码器并剔除错误估计。后端结合预训练的尺度无关深度先验和轻量级捆绑调整以确保几何一致性。

Result: 在KITTI、EuRoC-MAV和TUM-RGBD三个主要基准数据集上实现了最先进的性能。

Conclusion: UNO框架在多样化环境中表现出色，具有广泛的适用性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决表格结构识别中的挑战，特别是针对大型密集表格的准确性和效率问题。

Method: 采用Split-Merge框架，将行列分割视为序列标注任务，使用双Transformer编码器；合并过程通过网格分类任务和额外Transformer编码器实现。

Result: 在FinTabNet和PubTabNet上表现优异，减少了分辨率损失和计算复杂度，同时保持高精度和快速处理。
...
TL;DR: 论文提出了一种新任务AV-HaystacksQA，旨在解决多视频检索和时序定位问题，并提出了一个多智能体框架MAGNET和新的评估指标STEM与MTGS。
...
Method: 提出了AVHaystacks基准和MAGNET框架，用于多视频检索和时序定位任务。

Result: MAGNET在BLEU@4和GPT评估分数上分别提升了89%和65%。

Conclusion: AVHaystacks和MAGNET为多视频检索和时序定位提供了有效的解决方案和评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有检测方法多为黑箱，缺乏可解释性；MLLMs虽具备分析能力，但在视觉解释与人类推理对齐上存在不足。

Method: 构建标注数据集（含边界框和描述性标注），并通过多阶段优化策略微调MLLMs，平衡检测、定位和解释目标。

Result: 模型在AI生成图像检测和视觉缺陷定位上表现优异，显著超越基线方法。

Conclusion: 通过数据标注和多阶段优化，MLLMs能够实现高效且可解释的AI生成图像检测。
...
TL;DR: 论文提出了一种名为PRE-Net的两阶段模型，通过多模态知识扩展技术提升红外降水反演的准确性，解决了现有方法范围受限或精度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有红外降水反演算法精度低，而被动微波和雷达方法范围受限，因此需要一种能实现高精度全盘反演的红外方法。

Method: 采用两阶段流程：1) 扫描带蒸馏阶段，通过CoMWE技术将多模态知识迁移到红外模型；2) 全盘适应阶段，利用Self-MaskTune平衡多模态和红外知识。

Result: PRE-Net在PRE基准测试中表现优异，超越了PERSIANN-CCS、PDIR和IMERG等领先产品。

Conclusion: PRE-Net显著提升了红外降水反演的性能，为全盘高精度降水监测提供了有效解决方案。
...
TL;DR: LSSKD是一种自监督知识蒸馏框架，通过中间特征图的辅助分类器生成多样知识，无需依赖预训练教师网络，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预训练教师网络，而LSSKD旨在通过自监督知识蒸馏提升模型泛化能力，同时减少计算开销。

Method: 在中间特征图上添加辅助分类器，实现一对一知识迁移，无需教师网络。

Result: 在CIFAR-100和ImageNet上分别提升4.54%和0.32%，在少样本学习场景下也达到最优性能。

Conclusion: LSSKD高效轻量，适用于低计算设备，特别适合多模态感知和弱监督学习场景。
...
### [64] [Multi-Step Guided Diffusion for Image Restoration on Edge Devices: Toward Lightweight Perception in Embodied AI](https://arxiv.org/abs/2506.07286)
*Aditya Chakravarty*

Main category: cs.CV

TL;DR: 提出了一种多步优化策略，显著提升了扩散模型在图像恢复任务中的质量和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如MPGD）在每个去噪步骤中仅应用单次梯度更新，限制了恢复效果和泛化能力。

Method: 在每个去噪步骤中引入多步优化策略，增加梯度更新次数。

Result: 实验表明，该方法在超分辨率和去模糊任务中提升了LPIPS和PSNR指标，并在嵌入式设备上验证了其有效性。

Conclusion: MPGD作为一种轻量级模块，适用于实时视觉感知任务，如无人机和移动机器人。
...
### [65] [D2R: dual regularization loss with collaborative adversarial generation for model robustness](https://arxiv.org/abs/2506.07056)
...
Method: D2R Loss通过对抗分布和干净分布优化增强鲁棒性；CAG通过梯度协作生成对抗样本。

Result: 在多个基准数据集和目标模型上验证了方法的有效性，显著提升了模型鲁棒性。

Conclusion: D2R Loss与CAG结合能够显著提升模型的对抗鲁棒性。
...
### [66] [Hierarchical Scoring with 3D Gaussian Splatting for Instance Image-Goal Navigation](https://arxiv.org/abs/2506.07338)
*Yijie Deng,Shuaihang Yuan,Geeta Chandra Raju Bethala,Anthony Tzes,Yu-Shen Liu,Yi Fang*

Main category: cs.CV

TL;DR: 提出了一种基于分层评分范式的实例图像目标导航（IIN）框架，通过优化视点选择减少冗余，提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖随机采样视点或轨迹，导致冗余和计算开销大，缺乏优化的视点选择策略。
...
TL;DR: FLAIR-HUB是一个多传感器土地覆盖数据集，结合六种对齐模态数据，用于土地覆盖和作物分类研究。
...
Method: 利用多模态数据（如航空影像、Sentinel-1/2时间序列等）和深度学习模型（CNN、transformer）进行土地覆盖和作物分类。

Result: 最佳土地覆盖分类准确率为78.2%，mIoU为65.8%，多模态融合效果显著。

Conclusion: FLAIR-HUB为多模态数据融合和精细分类提供了重要支持，数据和代码已公开。
...
TL;DR: 提出了一种无监督伪装目标检测方法UCOD-DPL，通过动态伪标签学习和教师-学生框架，解决了现有方法中伪标签噪声和简单解码器性能不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有无监督伪装目标检测方法依赖固定策略生成伪标签并使用简单解码器，导致性能较低且易受噪声影响。
...
Method: SceneLCM将场景生成分解为四个流程：布局生成（LLM指导）、家具生成（CTS损失）、环境优化（多分辨率纹理场）和物理编辑（物理模拟）。
...
<details>
  <summary>Details</summary>
Motivation: 工业面板的文本检测任务复杂，现有方法多关注单一文本形状，缺乏对多尺度特征的全面探索。

Method: 开发了基于Transformer的高效混合器，学习多级特征间的依赖关系；设计了新的特征采样方法，明确编码文本的形状、位置和语义信息。

Result: 在自建的IPM数据集上验证了方法的优越性能，实际测试证明了其实用性。

Conclusion: EdgeSpotter在工业面板监控任务中表现出色，代码和演示已开源。
...
### [71] [R3D2: Realistic 3D Asset Insertion via Diffusion for Autonomous Driving Simulation](https://arxiv.org/abs/2506.07826)
...
TL;DR: R3D2是一种轻量级扩散模型，用于在自动驾驶验证中实现真实感3D资产插入，解决了传统神经重建方法的动态对象操作和可重用性问题。
...
Method: R3D2通过训练一个基于3D高斯散射生成的数据集的扩散模型，实现实时生成逼真的渲染效果（如阴影和一致光照）。

Result: R3D2显著提升了插入资产的真实感，支持文本到3D资产插入和跨场景对象转移，实现了自动驾驶验证的真正可扩展性。

Conclusion: R3D2为自动驾驶验证提供了一种可扩展且逼真的仿真解决方案，未来将公开数据集和代码以促进研究。
...
### [72] [Image segmentation and classification of E-waste for waste segregation](https://arxiv.org/abs/2506.07122)
*Prakriti Tripathi,Theertha Biju,Maniram Thota,Rakesh Lingam*

Main category: cs.CV

TL;DR: 论文提出了一种基于机器学习的电子废物分类方法，使用YOLOv11和Mask-RCNN模型，分别达到70和41 mAP，并计划与分拣机器人集成。
...
Conclusion: 模型将集成到分拣机器人中，实现电子废物的自动分类。
...
### [73] [LogoSP: Local-global Grouping of Superpoints for Unsupervised Semantic Segmentation of 3D Point Clouds](https://arxiv.org/abs/2506.07857)
*Zihui Zhang,Weisheng Dai,Hongtao Wen,Bo Yang*

Main category: cs.CV

TL;DR: LogoSP通过结合局部和全局点特征学习3D语义，利用频域中的全局模式生成高精度伪标签，显著优于现有无监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖局部特征，缺乏发现更丰富语义先验的能力，LogoSP旨在从局部和全局特征中学习3D语义。

Method: 通过频域中的全局模式对超点进行分组，生成语义伪标签，用于训练分割网络。

Result: 在两个室内和一个室外数据集上表现优异，显著超越现有无监督方法，达到最先进水平。

Conclusion: LogoSP证明了在无人工标签情况下，全局模式能有效表示有意义的3D语义。
...
### [74] [Hi-VAE: Efficient Video Autoencoding with Global and Detailed Motion](https://arxiv.org/abs/2506.07136)
*Huaize Liu,Wenzhang Sun,Qiyuan Zhang,Donglin Di,Biao Gong,Hao Li,Chen Wei,Changqing Zou*

Main category: cs.CV

TL;DR: Hi-VAE是一种高效视频自编码框架，通过分层编码视频动态的粗到细运动表示，显著减少冗余，实现高压缩比和高保真重建。


<details>
  <summary>Details</summary>
Motivation: 现有视频自编码方法未能高效建模动态时空冗余，导致压缩率低且训练成本高。

Method: Hi-VAE将视频动态分解为全局运动和细节运动两个潜在空间，使用自监督运动编码器压缩表示，并通过条件扩散解码器重建视频。

Result: 实验显示Hi-VAE压缩比达1428倍，远高于基线方法（如Cosmos-VAE的48倍），同时保持高质量重建。

Conclusion: Hi-VAE在高效压缩和重建质量上表现优异，为视频潜在表示和生成提供了新视角。
...
### [75] [FreeGave: 3D Physics Learning from Dynamic Videos by Gaussian Velocity](https://arxiv.org/abs/2506.07865)
*Jinxi Li,Ziyang Song,Siyuan Zhou,Bo Yang*

Main category: cs.CV

TL;DR: 论文提出FreeGave方法，无需物体先验即可学习复杂动态3D场景的物理特性，通过引入物理代码和发散自由模块，显著提升了未来帧预测和运动分割的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂物理运动或边界时表现不佳，且依赖物体先验（如掩码或类型）。本文旨在从多视角视频中建模3D场景的几何、外观和物理特性，无需先验知识。

Method: 提出FreeGave方法，引入物理代码和发散自由模块，估计每个高斯速度场，避免低效的PINN损失。

Result: 在三个公共数据集和新收集的真实数据集上验证了方法的优越性，尤其在未来帧外推和运动分割任务中表现突出。

Conclusion: 物理代码成功学习了无标签训练下的3D物理运动模式，证明了方法的有效性。
...
### [76] [Learning Compact Vision Tokens for Efficient Large Multimodal Models](https://arxiv.org/abs/2506.07138)
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型（LMMs）因计算成本高和视觉令牌序列处理复杂度高而面临挑战，需优化令牌序列以加速推理。

Method: 提出STF融合空间相邻令牌以减少序列长度，MBTF补充多粒度特征；结合两者平衡令牌减少与信息保留。

Result: 在8个视觉语言基准测试中，仅使用基线25%的视觉令牌即达到或超越基线性能。

Conclusion: STF和MBTF模块有效提升推理效率且不牺牲性能，代码和权重已开源。
...
### [77] [UA-Pose: Uncertainty-Aware 6D Object Pose Estimation and Online Object Completion with Partial References](https://arxiv.org/abs/2506.07996)
*Ming-Feng Li,Xin Yang,Fu-En Wang,Hritam Basak,Yuyin Sun,Shreekant Gayaka,Min Sun,Cheng-Hao Kuo*

Main category: cs.CV

TL;DR: UA-Pose提出了一种不确定性感知的6D物体姿态估计方法，针对部分参考信息（如不完整的3D模型或单张2D图像）进行优化，显著提升了姿态估计的准确性和物体完整性。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要完整的3D模型或大量参考图像，而部分参考信息（如不完整的3D模型或单张2D图像）下的6D姿态估计仍然具有挑战性。

Method: UA-Pose通过不确定性建模区分已见和未见区域，结合置信度评估和在线物体补全策略，优化姿态估计。支持两种输入：少量已知姿态的RGBD图像或单张2D图像。

Result: 在YCB-Video、YCBInEOAT和HO3D数据集上，UA-Pose显著优于现有方法，尤其在物体观测不完整时表现突出。

Conclusion: UA-Pose通过不确定性感知和在线补全策略，有效解决了部分参考信息下的6D姿态估计问题，具有实际应用潜力。
...
### [78] [GoTrack: Generic 6DoF Object Pose Refinement and Tracking](https://arxiv.org/abs/2506.07155)
...
Result: GoTrack与现有粗姿态估计方法结合，在标准6DoF物体姿态估计与跟踪基准上达到RGB-only的SOTA结果。
...
### [79] [Faster than Fast: Accelerating Oriented FAST Feature Detection on Low-end Embedded GPUs](https://arxiv.org/abs/2506.07164)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于ORB的SLAM系统在移动平台上实时性不足，主要因Oriented FAST计算耗时。

Method: 通过二进制编码策略快速确定候选点，以及可分离的Harris角点检测策略优化GPU指令。
...
### [80] [Frame Guidance: Training-Free Guidance for Frame-Level Control in Video Diffusion Models](https://arxiv.org/abs/2506.07177)
...
Conclusion: Frame Guidance为可控视频生成提供了一种无需训练的通用解决方案，兼容任何视频模型。
...
### [81] [Hierarchical Feature-level Reverse Propagation for Post-Training Neural Networks](https://arxiv.org/abs/2506.07188)
*Ni Ding,Lei He,Shengbo Eben Li,Keqiang Li*

Main category: cs.CV

TL;DR: 本文提出了一种分层解耦的后训练框架，用于提升端到端自动驾驶模型的透明度和训练灵活性。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶模型的黑盒特性导致其可解释性和安全性难以保障，需要一种透明且灵活的训练方法。

Method: 通过重构中间特征图并引入代理监督信号，实现特定组件的独立训练，避免了传统端到端反向传播的复杂性和耦合性。
...
### [82] [SAP-Bench: Benchmarking Multimodal Large Language Models in Surgical Action Planning](https://arxiv.org/abs/2506.07196)
*Mengya Xu,Zhongzhen Huang,Dillan Imans,Yiru Ye,Xiaofan Zhang,Qi Dou*

Main category: cs.CV

TL;DR: 论文提出SAP-Bench数据集和MLLM-SAP框架，用于评估多模态大语言模型在手术动作规划任务中的性能，发现现有模型在预测下一步动作方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试无法充分评估手术动作规划任务所需的精确分析能力，尤其是在生命关键领域中的可靠性和安全性。

Method: 引入SAP-Bench数据集，包含1226个临床验证的手术动作片段，并提出MLLM-SAP框架，利用MLLMs生成下一步动作建议。

Result: 评估了七种先进MLLMs，发现它们在预测下一步动作时表现不佳。

Conclusion: SAP-Bench为手术动作规划提供了高质量评估工具，揭示了当前模型的局限性。
...
### [83] [TV-LiVE: Training-Free, Text-Guided Video Editing via Layer Informed Vitality Exploitation](https://arxiv.org/abs/2506.07205)
*Min-Jung Kim,Dongjin Kim,Seokju Yun,Jaegul Choo*

Main category: cs.CV

TL;DR: TV-LiVE是一种无需训练、基于文本指导的视频编辑框架，通过利用关键层的活力信息实现对象添加和非刚性编辑。


<details>
  <summary>Details</summary>
Motivation: 当前视频编辑技术主要关注风格转换等简单任务，复杂任务如对象添加和非刚性编辑尚未充分探索。

Method: 识别视频生成模型中的关键层（与RoPE相关），选择性注入源模型的特征到目标模型，并通过显著层提取编辑区域。

Result: TV-LiVE在对象添加和非刚性编辑任务上优于现有方法。

Conclusion: TV-LiVE为复杂视频编辑任务提供了高效解决方案，展示了关键层活力的重要性。
...
### [84] [Backdoor Attack on Vision Language Models with Stealthy Semantic Manipulation](https://arxiv.org/abs/2506.07214)
*Zhiyuan Zhong,Zhen Sun,Yepang Liu,Xinlei He,Guanhong Tao*

Main category: cs.CV

TL;DR: 论文提出了一种新型的跨模态语义不匹配后门攻击方法BadSem，通过故意错配图像-文本对在训练中注入隐蔽后门，攻击成功率高且难以防御。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击主要依赖单模态触发器，未充分探索视觉语言模型（VLMs）的跨模态融合特性，因此研究跨模态语义不匹配作为隐式触发器的攻击方法。

Method: 提出BadSem攻击，通过构造SIMBad数据集（涉及颜色和物体属性）故意错配图像-文本对，在训练中注入后门。

Result: 在四种广泛使用的VLMs上，BadSem平均攻击成功率（ASR）超过98%，且能泛化到分布外数据集和跨模态传递。

Conclusion: 语义后门攻击难以通过现有防御策略（如系统提示和监督微调）缓解，凸显了VLMs中语义漏洞的紧迫安全问题。
...
### [85] [AugmentGest: Can Random Data Cropping Augmentation Boost Gesture Recognition Performance?](https://arxiv.org/abs/2506.07216)
...
Method: 整合多种数据增强技术（几何变换、随机裁剪、旋转、缩放、亮度对比度调整），生成每个样本的三个增强版本，四倍扩充数据集。

Result: 在多个基准数据集（DHG14/28、SHREC'17、JHMDB）和模型（e2eET、FPPR-PCD、DD-Net）上显著提升了性能，达到SOTA水平。

Conclusion: 该框架不仅提升了模型性能，还为手势识别和动作识别应用提供了可扩展的解决方案。
...
### [86] [Hallucination at a Glance: Controlled Visual Edits and Fine-Grained Multimodal Learning](https://arxiv.org/abs/2506.07227)
*Tianyi Bai,Yuxuan Fan,Jiantao Qiu,Fupeng Sun,Jiayi Song,Junlin Han,Zichen Liu,Conghui He,Wentao Zhang,Binhang Yuan*

Main category: cs.CV

TL;DR: 论文提出了一种针对多模态大语言模型（MLLMs）在细粒度视觉差异任务中表现不佳的解决方案，通过构建微编辑数据集（MED）和引入特征级一致性损失的监督微调框架，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: MLLMs在视觉语言任务中表现优异，但在细粒度视觉差异任务中容易产生幻觉或遗漏语义变化，主要原因是训练数据和学习目标的局限性。

Method: 提出了一种可控数据生成流程，构建了包含50K图像-文本对的微编辑数据集（MED），并设计了一个监督微调框架，结合特征级一致性损失。

Result: 在微编辑检测基准测试中，该方法显著提高了差异检测准确性并减少了幻觉现象，同时在标准视觉语言任务（如图像描述和视觉问答）中表现一致提升。

Conclusion: 研究表明，通过结合目标数据和对齐目标，可以有效增强MLLMs在细粒度视觉推理中的能力。
...
### [87] [Multi-Step Visual Reasoning with Visual Tokens Scaling and Verification](https://arxiv.org/abs/2506.07235)
*Tianyi Bai,Zengjie Hu,Fupeng Sun,Jiantao Qiu,Yizhen Jiang,Guangxin He,Bohan Zeng,Conghui He,Binhang Yuan,Wentao Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种动态推理框架，通过迭代和验证器引导的视觉标记缩放，提升多模态大语言模型的视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs采用静态推理范式，限制了其动态适应和迭代优化的能力，与人类感知的动态性形成鲜明对比。

Method: 提出基于马尔可夫决策过程的框架，结合提议视觉动作的推理器和通过多步DPO训练的验证器，并引入新数据集VTS。

Result: 方法在多个视觉推理基准上显著优于现有方法，提高了准确性和推理过程的可解释性。
...
### [88] [From Generation to Generalization: Emergent Few-Shot Learning in Video Diffusion Models](https://arxiv.org/abs/2506.07280)
*Pablo Acuaviva,Aram Davtyan,Mariam Hassan,Sebastian Stapf,Ahmad Rahimi,Alexandre Alahi,Paolo Favaro*

Main category: cs.CV

TL;DR: VDMs不仅是视频生成工具，还能通过少量样本微调适应新任务，表现出广泛泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索VDMs在视频生成之外的潜力，验证其是否具备结构化表示和视觉世界的隐式理解能力。
...
### [89] [FANVID: A Benchmark for Face and License Plate Recognition in Low-Resolution Videos](https://arxiv.org/abs/2506.07304)
*Kavitha Viswanathan,Vrinda Goel,Shlesh Gholap,Devayan Ghosh,Madhav Gupta,Dhruvi Ganatra,Sanket Potdar,Amit Sethi*

Main category: cs.CV

TL;DR: FANVID是一个新的视频基准数据集，包含低分辨率视频片段，用于提升人脸和车牌识别模型。它定义了人脸匹配和车牌识别两个任务，并提供了基线方法和评估指标。


<details>
  <summary>Details</summary>
Motivation: 现实监控中低分辨率视频难以识别个体信息，需要利用时间信息提升识别能力。

Method: 数据集包含1,463个低分辨率视频片段，提供人工标注的边界框和标签。任务包括人脸匹配和车牌识别，要求模型利用时间信息。

Result: 基线方法在人脸匹配和车牌识别任务中分别取得0.58和0.42的分数，显示任务的可行性和挑战性。
...
### [90] [AllTracker: Efficient Dense Point Tracking at High Resolution](https://arxiv.org/abs/2506.07310)
...
TL;DR: AllTracker是一个通过估计查询帧与视频中其他帧之间的流场来估计长距离点轨迹的模型，提供高分辨率和密集（全像素）的对应关系。


<details>
  <summary>Details</summary>
Motivation: 现有点跟踪方法无法提供高分辨率和密集对应关系，而现有光流方法仅能对应下一帧。AllTracker旨在填补这一空白。

Method: 结合光流和点跟踪技术，通过低分辨率网格上的迭代推断、2D卷积层和像素对齐注意力层实现时空信息传播。

Result: 模型参数高效（1600万参数），在高分辨率（768x1024像素）下实现最先进的点跟踪精度。

Conclusion: AllTracker的设计允许在更广泛的数据集上训练，且训练对性能至关重要。代码和模型权重已开源。
...
### [91] ["CASE: Contrastive Activation for Saliency Estimation](https://arxiv.org/abs/2506.07327)
*Dane Williamson,Yangfeng Ji,Matthew Dwyer*

Main category: cs.CV

TL;DR: 本文提出了一种诊断测试（class sensitivity）来评估显著性方法的可靠性，发现许多方法对类别标签不敏感，并提出了新的对比解释方法CASE，其表现优于现有方法。
...
Method: 提出诊断测试评估显著性方法的类别敏感性，并开发了对比解释方法CASE，通过实验验证其有效性。

Result: 实验表明许多显著性方法对类别标签不敏感，而CASE能生成更忠实且类别特定的解释。

Conclusion: CASE方法在类别敏感性和解释忠实性上优于现有方法，揭示了显著性方法的结构性缺陷。
...
### [92] [CBAM-STN-TPS-YOLO: Enhancing Agricultural Object Detection through Spatially Adaptive Attention Mechanisms](https://arxiv.org/abs/2506.07357)
*Satvik Praveen,Yoonsung Jung*

Main category: cs.CV

TL;DR: 论文提出了一种结合Thin-Plate Splines（TPS）和STNs的改进模型CBAM-STN-TPS-YOLO，用于提升农业场景中目标检测的精度，尤其在遮挡和非刚性变形情况下表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统目标检测模型（如YOLO）在农业场景中因遮挡、不规则结构和背景噪声导致精度下降，而现有的STNs通过仿射变换无法有效处理非刚性变形（如弯曲的叶子）。

Method: 模型整合了TPS以实现灵活的非刚性空间变换，并引入CBAM模块抑制背景噪声，突出关键特征。
...
### [93] [Multiple Object Stitching for Unsupervised Representation Learning](https://arxiv.org/abs/2506.07364)
*Chengchao Shen,Dawei Liu,Jianxin Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为MOS的方法，通过拼接单目标图像生成多目标图像，优化无监督表示学习，显著提升了多目标图像的表现。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法在单目标图像上表现优异，但在多目标图像上表现不佳，因此需要一种无需人工标注的方法来改进多目标图像的无监督表示。

Method: 通过拼接单目标图像生成多目标图像，利用预定义的对象对应关系，提供额外的监督信号。

Result: 在ImageNet、CIFAR和COCO数据集上，MOS方法在单目标和多目标图像上均取得了领先的无监督表示性能。

Conclusion: MOS方法通过简单有效的图像拼接策略，显著提升了多目标图像的无监督表示能力，适用于复杂下游任务。
...
### [94] [C3S3: Complementary Competition and Contrastive Selection for Semi-Supervised Medical Image Segmentation](https://arxiv.org/abs/2506.07368)
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中标注样本不足的问题，并改善现有方法在边界细节捕捉上的不足。
...
### [95] [Generative Models at the Frontier of Compression: A Survey on Generative Face Video Coding](https://arxiv.org/abs/2506.07369)
*Bolin Chen,Shanzhi Yin,Goluck Konuko,Giuseppe Valenzise,Zihan Zhang,Shiqi Wang,Yan Ye*

Main category: cs.CV

TL;DR: 本文综述了生成式人脸视频编码（GFVC）技术，分析了其在高保真低码率视频通信中的优势，并探讨了标准化和应用潜力。


<details>
  <summary>Details</summary>
Motivation: 随着深度生成模型的兴起，GFVC通过语义感知表示和逼真合成能力，显著提升了人脸视频压缩效率，超越了现有标准（如VVC）。本文旨在填补理论研究与工业标准化之间的关键空白。

Method: 通过综述现有GFVC方法、构建大规模主观评分数据库、总结标准化潜力，并开发低复杂度GFVC系统。

Result: GFVC在高保真低码率视频通信中表现优异，提出了适合GFVC的质量评估指标，并推动了标准化进程。

Conclusion: GFVC在工业应用中潜力巨大，但仍需解决当前挑战以抓住未来机遇。
...
### [96] [ARGUS: Hallucination and Omission Evaluation in Video-LLMs](https://arxiv.org/abs/2506.07371)
...
Method: 提出ARGUS基准，通过比较VideoLLM生成的字幕与人工标注的真实字幕，量化幻觉率（错误描述）和遗漏率（重要细节缺失）。
...
### [97] [DINO-CoDT: Multi-class Collaborative Detection and Tracking with Vision Foundation Models](https://arxiv.org/abs/2506.07375)
*Xunjie He,Christina Dao Wen Lee,Meiling Wang,Chengran Yuan,Zefan Huang,Yufeng Yue,Marcelo H. Ang Jr*

Main category: cs.CV

TL;DR: 提出了一种多类别协同检测与跟踪框架，通过全局空间注意力融合模块和视觉语义增强的REID模块，显著提升了检测与跟踪精度。


<details>
  <summary>Details</summary>
Motivation: 现有协同感知研究主要针对车辆类别，缺乏多类别解决方案，限制了实际应用。

Method: 设计了全局空间注意力融合模块（GSAF）增强多尺度特征学习，引入视觉语义REID模块减少ID切换错误，并开发速度自适应的轨迹管理模块（VATM）。

Result: 在V2X-Real和OPV2V数据集上显著优于现有方法。

Conclusion: 该框架为多类别道路用户提供了高效的协同检测与跟踪解决方案。
...
### [98] [Adapter Naturally Serves as Decoupler for Cross-Domain Few-Shot Semantic Segmentation](https://arxiv.org/abs/2506.07376)
*Jintao Tong,Ran Ma,Yixiong Zou,Guangyao Chen,Yuhua Li,Ruixuan Li*

Main category: cs.CV

TL;DR: 论文提出了一种跨域少样本分割方法（CD-FSS），通过适配器作为域信息解耦器，并设计了域特征导航器（DFN）和SAM-SVN方法，显著提升了性能。
...
Method: 提出域特征导航器（DFN）和SAM-SVN方法，利用模型固有结构解耦域信息。

Result: 在1-shot和5-shot场景下，分别超越现有最佳方法2.69%和4.68% MIoU。

Conclusion: DFN和SAM-SVN方法有效解决了域差距和少样本问题，提升了跨域少样本分割性能。
...
### [99] [MrM: Black-Box Membership Inference Attacks against Multimodal RAG Systems](https://arxiv.org/abs/2506.07399)
...
Method: 采用多目标数据扰动框架，结合反事实攻击和掩码选择策略，通过统计推理提取成员信息。

Result: 在多个视觉数据集和主流视觉语言模型上，MrM表现出色且对自适应防御鲁棒。

Conclusion: MrM填补了多模态RAG系统隐私攻击的空白，为未来防御研究提供了方向。
...
### [100] [Compressed Feature Quality Assessment: Dataset and Baselines](https://arxiv.org/abs/2506.07412)
*Changsheng Gao,Wei Zhou,Guosheng Lin,Weisi Lin*

Main category: cs.CV

TL;DR: 论文提出了压缩特征质量评估（CFQA）问题，并创建了首个包含原始和压缩特征的基准数据集，评估了三种常用指标的性能，发现需要更精细的指标来衡量语义失真。


<details>
  <summary>Details</summary>
Motivation: 大型模型在资源受限环境中的广泛应用需要高效传输中间特征表示，但特征压缩会引入难以量化的语义退化，因此需要研究如何评估压缩特征的语义保真度。

Method: 提出了CFQA研究问题，并构建了一个包含300个原始特征和12000个压缩特征的基准数据集，评估了MSE、余弦相似度和中心核对齐三种指标的性能。

Result: 结果表明数据集具有代表性，但现有指标无法完全捕捉压缩特征的语义失真，需要更精细的评估方法。

Conclusion: 论文为CFQA研究提供了基础资源，并呼吁开发更有效的语义失真评估指标。
...
### [101] [DPFormer: Dynamic Prompt Transformer for Continual Learning](https://arxiv.org/abs/2506.07414)
*Sheng-Kai Huang,Jiun-Feng Chang,Chun-Rong Huang*

Main category: cs.CV

TL;DR: 提出动态提示变换器（DPFormer）解决持续学习中的稳定性-可塑性困境和任务间混淆问题，通过提示方案记忆旧知识并学习新知识，性能优于现有方法。
...
Method: 采用动态提示变换器（DPFormer）和提示方案，结合统一分类模块（二元交叉熵损失、知识蒸馏损失和辅助损失）进行端到端训练。

Result: 在CIFAR-100、ImageNet100和ImageNet1K数据集上表现优于现有方法。

Conclusion: DPFormer有效解决了持续学习中的关键问题，性能显著提升。
...
### [102] [FAMSeg: Fetal Femur and Cranial Ultrasound Segmentation Using Feature-Aware Attention and Mamba Enhancement](https://arxiv.org/abs/2506.07431)
...
Method: 设计了纵向和横向独立视角扫描卷积块、特征感知模块，结合Mamba优化的残差结构，抑制噪声干扰并增强局部多维扫描。

Result: FAMSeg网络在实验中实现了最快的损失减少和最佳分割性能。

Conclusion: 该模型有效解决了超声图像分割中的噪声和相似性问题，提升了分割精度。
...
### [103] [Prompt to Protection: A Comparative Study of Multimodal LLMs in Construction Hazard Recognition](https://arxiv.org/abs/2506.07436)
...
Method: 对五种LLMs（Claude-3 Opus、GPT-4.5、GPT-4o、GPT-o3和Gemini 2.0 Pro）进行对比评估，采用零样本、少样本和链式思维三种提示策略。

Result: CoT提示策略在所有模型中表现最佳，GPT-4.5和GPT-o3在多数情况下优于其他模型。
...
### [104] [PhysiInter: Integrating Physical Mapping for High-Fidelity Human Interaction Generation](https://arxiv.org/abs/2506.07456)
*Wei Yao,Yunlian Sun,Chang Liu,Hongwen Zhang,Jinhui Tang*

Main category: cs.CV

TL;DR: 提出了一种结合物理映射的方法，用于生成多人交互动作，提升动作的物理真实性和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有动作捕捉技术和生成模型常忽略物理约束，导致动作不真实，尤其在多人交互中问题更突出。

Method: 通过物理仿真环境中的动作模仿，将目标动作映射到物理有效空间，并结合MC和MI损失函数优化模型。

Result: 实验显示，该方法在动作质量上显著提升，物理保真度提高了3%-89%。

Conclusion: 物理映射方法有效解决了动作生成中的物理约束问题，提升了多人交互动作的真实性。
...
### [105] [GLOS: Sign Language Generation with Temporally Aligned Gloss-Level Conditioning](https://arxiv.org/abs/2506.07460)
*Taeryung Lee,Hyeongjin Nam,Gyeongsik Moon,Kyoung Mu Lee*

Main category: cs.CV

TL;DR: 论文提出了一种基于时间对齐的词汇级条件（GLOS）的手语生成框架，解决了现有方法在词汇顺序和语义准确性上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有手语生成方法因使用句子级条件而无法捕捉手语的时间结构和词汇级语义，导致词汇顺序混乱和动作模糊。

Method: 采用词汇级条件（gloss-level conditions）和时间对齐条件模块（TAC），以细粒度控制手语动作并保持词汇顺序。

Result: 在CSL-Daily和Phoenix-2014T数据集上，该方法在词汇顺序和语义准确性上优于现有方法。

Conclusion: GLOS框架通过时间对齐的词汇级条件显著提升了手语生成的质量。
...
### [106] [DeepVideo-R1: Video Reinforcement Fine-Tuning via Difficulty-aware Regressive GRPO](https://arxiv.org/abs/2506.07464)
*Jinyoung Park,Jeehye Na,Jinyoung Kim,Hyunwoo J. Kim*

Main category: cs.CV

TL;DR: 本文探讨了GRPO在视频大语言模型中的应用问题，提出了Reg-GRPO和难度感知数据增强策略，显著提升了视频推理性能。


<details>
  <summary>Details</summary>
Motivation: GRPO在视频大语言模型中的应用研究较少，且存在依赖安全措施和优势消失问题，需改进。

Method: 提出Reg-GRPO将GRPO目标重构为回归任务，直接预测优势值；设计难度感知数据增强策略动态扩充训练样本。
...
### [107] [Ambiguity-Restrained Text-Video Representation Learning for Partially Relevant Video Retrieval](https://arxiv.org/abs/2506.07471)
*CH Cho,WJ Moon,W Jun,MS Jung,JP Heo*

Main category: cs.CV

TL;DR: 论文提出了一种名为ARL的框架，用于处理部分相关视频检索中的文本-视频模糊性问题，通过多正对比学习和双重三元组损失优化语义关系。


<details>
  <summary>Details</summary>
Motivation: 现有的PRVR训练假设文本查询与视频是一对一关系，忽略了文本与视频内容之间的模糊性。

Method: 提出ARL框架，通过不确定性和相似性检测模糊对，采用多正对比学习和双重三元组损失进行分层学习，并引入跨模型模糊检测以减少误差传播。

Result: ARL框架在PRVR任务中表现出色。

Conclusion: ARL有效解决了文本-视频模糊性问题，提升了PRVR的性能。
...
### [108] [CoCoA-Mix: Confusion-and-Confidence-Aware Mixture Model for Context Optimization](https://arxiv.org/abs/2506.07484)
*Dasol Hong,Wooju Lee,Hyun Myung*

Main category: cs.CV

TL;DR: 论文提出了一种混淆感知损失（CoA-loss）和置信感知权重（CoA-weights）的方法，通过优化决策边界和混合模型，提升了视觉语言模型在任务特定适应中的专业化和泛化能力。
...
Method: 提出CoA-loss优化决策边界，并通过混合模型（CoCoA-Mix）结合CoA-weights提升泛化能力。

Result: 实验表明CoCoA-Mix在专业化和泛化方面优于现有方法。

Conclusion: CoCoA-Mix通过混淆感知和置信感知机制，有效解决了冻结编码器带来的问题。
...
### [109] [Drive Any Mesh: 4D Latent Diffusion for Mesh Deformation from Video](https://arxiv.org/abs/2506.07489)
*Yahao Shi,Yang Liu,Yanmin Wu,Xing Liu,Chen Zhao,Jie Luo,Bin Zhou*

Main category: cs.CV

TL;DR: DriveAnyMesh是一种基于单目视频驱动的网格动画方法，解决了现有4D生成技术在渲染引擎兼容性和效率上的问题。


<details>
  <summary>Details</summary>
Motivation: 当前4D生成技术存在渲染效率低、手动工作量大且缺乏跨类别泛化能力的问题，需要一种更高效且兼容现代渲染引擎的方法。

Method: 提出了一种4D扩散模型，通过去噪潜在集序列并解码为网格动画，利用基于变压器的变分自编码器捕获3D形状和运动信息。

Result: 实验表明，DriveAnyMesh能快速生成高质量复杂运动动画，且兼容现代渲染引擎。
...
### [110] [SpatialLM: Training Large Language Models for Structured Indoor Modeling](https://arxiv.org/abs/2506.07491)
*Yongsen Mao,Junhao Zhong,Chuan Fang,Jia Zheng,Rui Tang,Hao Zhu,Ping Tan,Zihan Zhou*

Main category: cs.CV

TL;DR: SpatialLM是一种大型语言模型，用于处理3D点云数据并生成结构化3D场景理解输出，如墙壁、门窗等建筑元素。它基于开源LLM微调，在公开基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 增强现代LLM的空间理解能力，以应用于增强现实、机器人等领域。

Method: 采用标准多模态LLM架构，从开源LLM直接微调，并使用大规模合成数据集训练。

Result: 在布局估计任务中达到最先进性能，在3D物体检测中表现竞争性。

Conclusion: SpatialLM展示了提升LLM空间理解能力的可行路径，适用于多种应用场景。
...
### [111] [Genesis: Multimodal Driving Scene Generation with Spatio-Temporal and Cross-Modal Consistency](https://arxiv.org/abs/2506.07497)
...
### [112] [MoQAE: Mixed-Precision Quantization for Long-Context LLM Inference via Mixture of Quantization-Aware Experts](https://arxiv.org/abs/2506.07533)
*Wei Tao,Haocheng Lu,Xiaoyang Qu,Bin Zhang,Kai Lu,Jiguang Wan,Jianzong Wang*

Main category: cs.CV

TL;DR: MoQAE是一种新型混合精度量化方法，通过量化感知专家混合（MoE）优化大语言模型的长上下文推理内存消耗。


<details>
  <summary>Details</summary>
Motivation: 解决现有量化方法无法同时兼顾效果和效率的问题。

Method: 将不同量化位宽配置视为专家，采用MoE选择最优配置；分块输入路由器；设计轻量级路由器微调；引入路由冻结和共享机制。

Result: 在多个基准数据集上表现优于现有KV缓存量化方法。

Conclusion: MoQAE在效率和效果上均优于现有方法。
...
### [113] [Domain Randomization for Object Detection in Manufacturing Applications using Synthetic Data: A Comprehensive Study](https://arxiv.org/abs/2506.07539)
*Xiaomeng Zhu,Jacob Henningsson,Duruo Li,Pär Mårtensson,Lars Hanson,Mårten Björkman,Atsuto Maki*

Main category: cs.CV

TL;DR: 本文提出了一种综合数据生成流程，用于制造业目标检测的合成数据生成，并引入SIP15-OD数据集。实验表明，该方法在合成数据训练下取得了高性能。


<details>
  <summary>Details</summary>
Motivation: 解决制造业目标检测中合成数据生成的关键问题，探索仿真到现实的可行性。

Method: 提出数据生成流程，涵盖物体特性、背景、光照等因素，并引入SIP15-OD数据集。

Result: 在Yolov8模型上，合成数据训练的mAP@50得分高达96.4%（机器人数据集）和94.1%-99.5%（SIP15-OD）。

Conclusion: 提出的域随机化方法有效，能接近真实数据分布。
...
### [114] [APTOS-2024 challenge report: Generation of synthetic 3D OCT images from fundus photographs](https://arxiv.org/abs/2506.07542)
...
TL;DR: APTOS-2024挑战赛探索了从2D眼底图像生成3D OCT图像的可行性，提出了新的评估方法和解决方案，以提升眼科医疗的可及性。


<details>
  <summary>Details</summary>
Motivation: 解决OCT设备成本高、操作复杂的问题，利用2D眼底图像的便捷性，通过生成式AI实现3D OCT图像的合成。

Method: 挑战赛框架包括基准数据集、两种保真度评估指标（图像级和视频级），并分析了42个初步提交和9个决赛方案。

Result: 342支团队参与，领先方法结合了数据预处理、预训练、视觉基础模型和架构改进。

Conclusion: APTOS-2024首次验证了从眼底图像生成3D OCT的可行性，为资源匮乏地区提供了潜在解决方案。
...
### [115] [Synthesize Privacy-Preserving High-Resolution Images via Private Textual Intermediaries](https://arxiv.org/abs/2506.07555)
*Haoxiang Wang,Zinan Lin,Da Yu,Huishuai Zhang*

Main category: cs.CV

TL;DR: SPTI是一种通过文本中介生成高分辨率差分隐私（DP）图像的新方法，无需训练模型，仅使用现成模型进行推理，显著提高了合成图像的质量。
...
Conclusion: SPTI提供了一种资源高效且兼容专有模型的框架，可生成高质量DP合成图像，扩展了对私有视觉数据的访问。
...
### [116] [Cross-channel Perception Learning for H&E-to-IHC Virtual Staining](https://arxiv.org/abs/2506.07559)
*Hao Yang,JianYu Wu,Run Fang,Xuelian Zhao,Yuan Ji,Zhiyu Chen,Guibin He,Junceng Guo,Yang Liu,Xinhua Zeng*

Main category: cs.CV

TL;DR: 提出了一种新的跨通道感知学习（CCPL）策略，用于解决H&E到IHC染色中忽略的核与膜跨通道关联问题，通过双通道特征提取和特征蒸馏损失提升虚拟染色质量。


<details>
  <summary>Details</summary>
Motivation: 现有H&E到IHC研究常忽视核与膜的跨通道关联，限制了虚拟染色技术的分析和诊断潜力。

Method: CCPL通过分解HER2染色为核与膜通道，利用Gigapath的Tile Encoder提取双通道特征，计算跨通道关联和特征蒸馏损失，并通过光学密度图分析确保染色一致性。

Result: 实验表明，CCPL在PSNR、SSIM等指标和病理学家评估中表现优异，能高质量生成虚拟染色图像并支持自动化诊断。

Conclusion: CCPL有效解决了跨通道关联问题，提升了虚拟染色质量，为多媒体医疗数据的病理诊断提供了可靠支持。
...
### [117] [OpenDance: Multimodal Controllable 3D Dance Generation Using Large-scale Internet Data](https://arxiv.org/abs/2506.07565)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究因缺乏细粒度多模态数据和灵活的多条件生成能力，限制了舞蹈生成的多样性和可控性。
...
### [118] [Towards the Influence of Text Quantity on Writer Retrieval](https://arxiv.org/abs/2506.07566)
*Marco Peer,Robert Sablatnig,Florian Kleber*

Main category: cs.CV

TL;DR: 本文研究了基于笔迹相似性的作者检索任务，探讨了文本量对检索性能的影响，发现即使仅使用一行文本，性能仍能保持较高水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注页面级检索，而本文旨在分析不同文本量（行级和词级）对作者检索性能的影响。

Method: 评估了三种先进的作者检索系统（包括手工特征和深度学习方法），并在CVL和IAM数据集上测试了不同文本量的性能。

Result: 实验表明，仅用一行文本时性能下降20-30%，但使用至少四行文本时仍能达到全页性能的90%以上。深度学习方法在低文本场景下表现优于手工特征。

Conclusion: 文本依赖的检索在低文本场景下仍能保持较强性能，深度学习方法（如NetVLAD）在低文本场景中优于传统方法。
...
### [119] [LLM-driven Indoor Scene Layout Generation via Scaled Human-aligned Data Synthesis and Multi-Stage Preference Optimization](https://arxiv.org/abs/2506.07570)
*Yixuan Yang,Zhen Luo,Tongsheng Ding,Junru Lu,Mingqi Gao,Jinyu Yang,Victor Sanchez,Feng Zheng*

Main category: cs.CV

TL;DR: 论文提出3D-SynthPlace数据集和OptiScene模型，优化基于LLM的室内布局生成，解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有室内布局生成方法存在空间不一致、计算成本高或泛化能力不足的问题，需要更高效且通用的解决方案。

Method: 通过'GPT synthesize, Human inspect'流程创建3D-SynthPlace数据集，并基于此训练两阶段LLM模型OptiScene（SFT和DPO）。

Result: OptiScene在生成质量和成功率上优于传统方法，并在交互任务中表现优异。

Conclusion: 3D-SynthPlace和OptiScene为室内布局生成提供了高效且通用的新方法。
...
### [120] [Learning Speaker-Invariant Visual Features for Lipreading](https://arxiv.org/abs/2506.07572)
...
Method: SIFLip采用隐式解耦和显式解耦模块，利用文本嵌入和说话者识别任务分离说话者特征。

Result: 实验表明SIFLip在多个公开数据集上显著提升泛化性能，优于现有方法。

Conclusion: SIFLip通过解耦说话者特征有效提升了唇读模型的泛化能力。
...
### [121] [Uncertainty-o: One Model-agnostic Framework for Unveiling Uncertainty in Large Multimodal Models](https://arxiv.org/abs/2506.07575)
*Ruiyang Zhang,Hu Zhang,Hao Fei,Zhedong Zheng*

Main category: cs.CV

TL;DR: 论文提出Uncertainty-o框架，用于评估和量化多模态模型（LMMs）的不确定性，解决统一评估、提示和量化三大问题。


<details>
  <summary>Details</summary>
Motivation: 多模态模型（LMMs）虽被认为比纯语言模型（LLMs）更鲁棒，但其不确定性评估仍存在挑战，包括统一评估方法、提示方式和量化手段。

Method: 提出Uncertainty-o框架，通过模型无关的方法揭示LMMs的不确定性，探索多模态提示扰动，并推导多模态语义不确定性的量化公式。

Result: 在18个基准测试和10种LMMs上的实验表明，Uncertainty-o能有效估计LMMs的不确定性，提升下游任务（如幻觉检测和缓解）的性能。

Conclusion: Uncertainty-o为LMMs的不确定性评估提供了统一且有效的解决方案，对下游任务有显著改进。
...
### [122] [Super Encoding Network: Recursive Association of Multi-Modal Encoders for Video Understanding](https://arxiv.org/abs/2506.07576)
*Boyu Chen,Siran Chen,Kunchang Li,Qinglin Xu,Yu Qiao,Yali Wang*

Main category: cs.CV

TL;DR: 提出了一种统一的超级编码网络（SEN），通过递归关联多模态编码器，增强视频理解任务的表现。
...
Method: 将预训练编码器视为“超级神经元”，设计递归关联（RA）块，递归融合多模态信息，实现知识整合、分发和提示。

Result: SEN显著提升了四种代表性视频任务（跟踪、识别、聊天和编辑）的表现，例如像素级跟踪的Jaccard指数提高2.7%，文本对齐提升6.4%。

Conclusion: SEN通过递归多模态交互，有效提升了视频理解任务的性能，为世界建模提供了新思路。
...
### [123] [Explore the vulnerability of black-box models via diffusion models](https://arxiv.org/abs/2506.07590)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型的高保真图像生成能力可能被恶意利用，导致安全与隐私风险，如版权侵犯、敏感信息泄露等。

Method: 利用扩散模型API生成合成图像，训练替代模型，以最小查询量对黑盒模型进行攻击。

Result: 在七个基准测试中，方法平均提升27.37%，仅用0.01倍查询预算，对抗攻击成功率达98.68%。

Conclusion: 扩散模型的API可能成为安全威胁，需采取措施防范此类攻击。
...
### [124] [SceneRAG: Scene-level Retrieval-Augmented Generation for Video Understanding](https://arxiv.org/abs/2506.07600)
*Nianbo Zeng,Haowen Hou,Fei Richard Yu,Si Shi,Ying Tiffany He*

Main category: cs.CV

TL;DR: SceneRAG是一个基于大语言模型的框架，通过处理视频的ASR转录和时间元数据，将视频分割为叙事一致的场景，并结合视觉和文本模态信息构建知识图谱，显著提升长视频理解任务的表现。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索增强生成（RAG）的视频理解方法通常将视频分割为固定长度的片段，破坏了上下文连续性且难以捕捉真实场景边界。受人类自然组织连续经验为连贯场景的能力启发，研究提出了SceneRAG。

Method: SceneRAG利用大语言模型处理ASR转录和时间元数据，将视频分割为叙事一致的场景，并通过轻量级启发式和迭代校正优化边界。每个场景中，框架融合视觉和文本信息提取实体关系，动态构建知识图谱，支持多跳检索和生成。

Result: 在包含134小时多样化内容的LongerVideos基准测试中，SceneRAG显著优于现有基线，生成任务的胜率高达72.5%。

Conclusion: SceneRAG通过场景分割和多模态信息融合，有效解决了长视频理解中的上下文连续性和场景边界问题，为复杂视频内容的理解提供了新思路。
...
### [125] [SurgBench: A Unified Large-Scale Benchmark for Surgical Video Analysis](https://arxiv.org/abs/2506.07603)
...
Result: 现有视频基础模型在多样化任务中泛化能力不足，而基于SurgBench-P的预训练显著提升性能并增强跨领域泛化能力。

Conclusion: SurgBench为手术视频分析提供了全面支持，显著提升了模型的泛化能力和性能。
...
### [126] [DragNeXt: Rethinking Drag-Based Image Editing](https://arxiv.org/abs/2506.07611)
*Yuan Zhou,Junbao Zhou,Qingshan Xu,Kesen Zhao,Yuxuan Wang,Hao Fei,Richang Hong,Hanwang Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种新的拖拽式图像编辑方法DragNeXt，通过明确指定拖拽区域和类型解决模糊性问题，并简化了编辑流程。


<details>
  <summary>Details</summary>
Motivation: 现有拖拽式图像编辑方法存在模糊性和复杂性问题，无法高质量完成编辑任务。

Method: 将拖拽式图像编辑重新定义为变形、旋转和平移操作，提出Latent Region Optimization（LRO）框架，并通过Progressive Backward Self-Intervention（PBSI）实现。

Result: 在NextBench上的实验表明，DragNeXt显著优于现有方法。

Conclusion: DragNeXt通过简化流程和利用区域结构信息，有效提升了拖拽式图像编辑的质量和效率。
...
### [127] [Scaling Human Activity Recognition: A Comparative Evaluation of Synthetic Data Generation and Augmentation Techniques](https://arxiv.org/abs/2506.07612)
*Zikang Leng,Archith Iyer,Thomas Plötz*

Main category: cs.CV

TL;DR: 本文比较了两种虚拟IMU数据生成方法与传统数据增强技术的效果，发现虚拟IMU数据在有限数据条件下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决HAR中标记数据稀缺问题，探索视频和语言两种跨模态生成虚拟IMU数据的有效性。

Method: 构建大规模虚拟IMU数据集，对比视频、语言生成方法及传统数据增强技术，并在三个基准数据集上评估。

Result: 虚拟IMU数据显著优于单独使用真实或增强数据，尤其在数据有限时表现更优。
...
### [128] [Event-Priori-Based Vision-Language Model for Efficient Visual Understanding](https://arxiv.org/abs/2506.07627)
*Haotong Qin,Cheng Hu,Michele Magno*

Main category: cs.CV

TL;DR: EP-VLM利用动态事件视觉的运动先验，通过事件数据引导RGB视觉输入的稀疏化，显著提升了视觉语言模型的效率，同时保持高精度。
...
Result: 在Qwen2-VL-2B基准上，EP-VLM节省50%计算量，同时保持98%的原始精度。

Conclusion: 事件视觉先验可显著提升VLM效率，为边缘设备的高效视觉理解提供新思路。
...
### [129] [HuSc3D: Human Sculpture dataset for 3D object reconstruction](https://arxiv.org/abs/2506.07628)
*Weronika Smolak-Dyżewska,Dawid Malarz,Grzegorz Wilczyński,Rafał Tobiasz,Joanna Waczyńska,Piotr Borycki,Przemysław Spurek*

Main category: cs.CV

TL;DR: HuSc3D数据集填补了现有3D重建基准的不足，专注于真实场景的复杂性，如动态背景和色彩差异。


<details>
  <summary>Details</summary>
Motivation: 现有数据集过于理想化，无法反映真实场景的挑战，如动态背景和色彩差异。

Method: 提出HuSc3D数据集，包含六个白色雕塑场景，具有复杂穿孔和色彩单一性，图像数量差异显著。

Result: 评估显示HuSc3D能有效区分模型性能，揭示其对几何细节、色彩模糊和数据量的敏感性。

Conclusion: HuSc3D为3D重建提供了更真实的基准，突显现有方法的局限性。
...
### [130] [HieraEdgeNet: A Multi-Scale Edge-Enhanced Framework for Automated Pollen Recognition](https://arxiv.org/abs/2506.07637)
*Yuchong Long,Wen Sun,Ningxiao Sun,Wenxiao Wang,Chao Li,Shan Yin*

Main category: cs.CV

TL;DR: HieraEdgeNet是一种多尺度边缘增强框架，显著提升了显微镜下花粉识别的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统花粉识别方法效率低且主观性强，现有深度学习模型在显微镜目标（如花粉）的定位精度上表现不佳。

Method: 提出HieraEdgeNet框架，包含三个模块：HEM（多尺度边缘特征提取）、SEF（边缘与语义信息融合）和CSPOKM（细节特征优化）。

Result: 在120类花粉数据集上，mAP@.5达到0.9501，优于YOLOv12n和RT-DETR等基准模型。

Conclusion: HieraEdgeNet通过系统整合边缘信息，为高精度、高效率的显微镜目标检测提供了强大解决方案。
...
### [131] [Synthetic Visual Genome](https://arxiv.org/abs/2506.07643)
...
TL;DR: 论文提出了ROBIN模型，通过密集标注的关系训练，能够生成高质量的场景图，并在关系理解任务中超越更大规模的模型。
...
Method: 使用合成数据集SVG训练ROBIN模型，并通过SG-EDIT框架利用GPT-4o优化场景图。

Result: ROBIN-3B模型在关系理解任务中表现优异，甚至超越更大规模的模型，并在指代表达理解任务中达到88.9分。

Conclusion: 训练于精炼的场景图数据对提升多样化视觉推理任务性能至关重要。
...
### [132] [FMaMIL: Frequency-Driven Mamba Multi-Instance Learning for Weakly Supervised Lesion Segmentation in Medical Images](https://arxiv.org/abs/2506.07652)
*Hangbei Cheng,Xiaorong Dong,Xueyu Liu,Jianan Zhang,Xuetao Ma,Mingqiang Wei,Liansheng Wang,Junxin Chen,Yongfei Wu*

Main category: cs.CV

TL;DR: FMaMIL是一种基于图像级标签的弱监督病变分割框架，通过两阶段方法（Mamba编码器和频域编码模块）实现高效分割，无需像素级标注。
...
Method: 两阶段框架：1）使用Mamba编码器和频域编码模块生成CAM；2）通过软标签监督和自校正机制优化伪标签。
...
### [133] [ProSplat: Improved Feed-Forward 3D Gaussian Splatting for Wide-Baseline Sparse Views](https://arxiv.org/abs/2506.07670)
...
<details>
  <summary>Details</summary>
Motivation: 解决3D高斯泼溅（3DGS）在宽基线场景下因纹理细节不足和几何不一致导致的性能下降问题。

Method: 第一阶段生成3D高斯基元，第二阶段通过改进模型（结合MORI和DWEA）增强渲染视图。MORI补充缺失纹理，DWEA确保几何一致性。

Result: 在RealEstate10K和DL3DV-10K数据集上，PSNR平均提升1 dB。

Conclusion: ProSplat在宽基线条件下显著优于现有方法，实现了高保真渲染。
...
### [134] [OpenSplat3D: Open-Vocabulary 3D Instance Segmentation using Gaussian Splatting](https://arxiv.org/abs/2506.07697)
*Jens Piekenbrinck,Christian Schmidt,Alexander Hermans,Narunas Vaskevicius,Timm Linder,Bastian Leibe*

Main category: cs.CV

TL;DR: OpenSplat3D扩展了3D高斯点云（3DGS）的能力，实现了无需手动标注的开放词汇3D实例分割。


<details>
  <summary>Details</summary>
Motivation: 3DGS在神经场景重建中表现出色，但缺乏语义理解能力。本文旨在扩展其功能，实现开放词汇的3D实例分割。

Method: 结合特征点云技术和Segment Anything Model的实例掩码，通过对比损失和语言嵌入实现文本驱动的实例分割。

Result: 在LERF-mask、LERF-OVS和ScanNet++验证集上展示了高效的分割能力。
...
### [135] [NOVA3D: Normal Aligned Video Diffusion Model for Single Image to 3D Generation](https://arxiv.org/abs/2506.07698)
*Yuxiao Yang,Peihao Li,Yuhong Zhang,Junzhe Lu,Xianglong He,Minghan Qin,Weitao Wang,Haoqian Wang*

Main category: cs.CV

TL;DR: NOVA3D是一种创新的单图像到3D生成框架，通过利用预训练视频扩散模型的强3D先验和几何信息，解决了多视图一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D生成方法因缺乏足够的3D先验导致多视图一致性不足，NOVA3D旨在解决这一问题。

Method: NOVA3D利用预训练视频扩散模型的3D先验，结合几何信息进行多视图视频微调，并提出Geometry-Temporal Alignment (GTA)注意力机制和去冲突几何融合算法。

Result: 实验表明NOVA3D在多视图一致性和纹理保真度上优于现有基线方法。
...
### [136] [Adaptive Blind Super-Resolution Network for Spatial-Specific and Spatial-Agnostic Degradations](https://arxiv.org/abs/2506.07705)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了不同退化类型的多样性，采用统一网络处理多种退化，效果不佳。

Method: 引入动态滤波网络，包含全局和局部分支，分别处理空间无关和空间相关的退化类型。

Result: 在合成和真实图像数据集上，该方法优于现有盲超分辨率算法。

Conclusion: 动态滤波网络能有效处理两类退化问题，提升图像重建质量。
...
### [137] [Consistent Video Editing as Flow-Driven Image-to-Video Generation](https://arxiv.org/abs/2506.07713)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理复杂运动模式（如非刚性物体运动），视频编辑任务中运动传递和形状变形的一致性挑战亟待解决。

Method: FlowV2V将任务分解为第一帧编辑和条件I2V生成，利用伪光流序列对齐变形形状，确保编辑一致性。

Result: 在DAVIS-EDIT数据集上，FlowV2V在DOVER和warping error指标上分别提升13.67%和50.66%，表现优于现有方法。

Conclusion: FlowV2V通过光流建模和任务分解，显著提升了视频编辑的复杂运动处理能力和时间一致性。
...
### [138] [ReverB-SNN: Reversing Bit of the Weight and Activation for Spiking Neural Networks](https://arxiv.org/abs/2506.07720)
*Yufei Guo,Yuhan Zhang,Zhou Jie,Xiaode Liu,Xin Tong,Yuanpei Chen,Weihang Peng,Zhe Ma*

Main category: cs.CV

TL;DR: 提出了一种名为ReverB-SNN的方法，通过反转权重和激活的比特位，结合实值激活和二进制权重，提升SNN的信息容量和准确性。
...
### [139] [ETA: Efficiency through Thinking Ahead, A Dual Approach to Self-Driving with Large Models](https://arxiv.org/abs/2506.07725)
*Shadi Hamdan,Chonghao Sima,Zetong Yang,Hongyang Li,Fatma Güney*

Main category: cs.CV

TL;DR: 论文提出ETA系统，通过异步计算和批量推理解决大模型在自动驾驶中的实时性问题，提升性能8%且保持50ms的推理速度。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统中大模型推理速度慢的问题，同时保留其高性能优势。

Method: 采用异步系统ETA，通过特征传播、小模型实时提取和动作掩码机制整合双特征。

Result: 在Bench2Drive CARLA Leaderboard-v2上，ETA将驾驶分数提升至69.53，性能提升8%，推理速度保持在50ms。

Conclusion: ETA系统成功平衡了大模型的高性能和小模型的实时性，为自动驾驶系统提供了高效解决方案。
...
### [140] [SpikeSMOKE: Spiking Neural Networks for Monocular 3D Object Detection with Cross-Scale Gated Coding](https://arxiv.org/abs/2506.07737)
*Xuemei Chen,Huamin Wang,Hangchi Shen,Shukai Duan,Shiping Wen,Tingwen Huang*

Main category: cs.CV

TL;DR: 论文提出SpikeSMOKE架构，结合SNNs的低功耗特性，用于单目3D目标检测，并提出跨尺度门控编码机制（CSGC）增强特征表示，同时设计轻量级残差块以减少计算量。


<details>
  <summary>Details</summary>
Motivation: 3D目标检测在自动驾驶等领域的广泛应用导致能耗增加，而SNNs的低功耗特性为此提供了新解决方案。

Method: 应用SNNs于单目3D目标检测，提出SpikeSMOKE架构和CSGC机制，结合跨尺度融合和门控过滤增强特征表示，并设计轻量级残差块。

Result: 在KITTI数据集上，SpikeSMOKE+CSGC在Easy、Moderate、Hard类别上分别提升2.82、3.2、3.17 AP|R11，能耗降低72.2%，性能仅下降4%。SpikeSMOKE-L参数和计算量分别减少3倍和10倍。

Conclusion: SpikeSMOKE架构成功实现低功耗3D目标检测，CSGC机制和轻量设计显著提升性能并降低能耗。
...
### [141] [AssetDropper: Asset Extraction via Diffusion Models with Reward-Driven Optimization](https://arxiv.org/abs/2506.07738)
...
Method: 提出AssetDropper框架，通过预训练奖励模型实现闭环反馈，提取并优化资产。

Result: 实验表明，AssetDropper在资产提取任务中达到最先进水平。

Conclusion: AssetDropper为设计师提供了高效的资产提取工具，并推动了相关下游任务的研究。
...
### [142] [ArchiLense: A Framework for Quantitative Analysis of Architectural Styles Based on Vision Large Language Models](https://arxiv.org/abs/2506.07739)
*Jing Zhong,Jun Yin,Peilin Li,Pengyu Zeng,Miao Zhang,Shuai Lu,Ran Luo*

Main category: cs.CV

TL;DR: 该研究提出了一种基于视觉语言模型的框架ArchiLense，用于自动识别和分类建筑风格，解决了传统方法的主观性和区域偏见问题。
...
Method: 构建ArchDiffBench数据集（1,765张高质量建筑图像），并开发ArchiLense框架，结合计算机视觉和深度学习技术。

Result: ArchiLense在风格识别上表现优异，与专家标注一致性达92.4%，分类准确率为84.5%。

Conclusion: 该方法提供了更客观、准确的建筑文化比较视角，超越了传统分析的主观性。
...
### [143] [Flow-Anything: Learning Real-World Optical Flow Estimation from Large-Scale Single-view Images](https://arxiv.org/abs/2506.07740)
*Yingping Liang,Ying Fu,Yutao Hu,Wenqi Shao,Jiaming Liu,Debing Zhang*

Main category: cs.CV

TL;DR: Flow-Anything是一个大规模数据生成框架，通过单视图图像生成真实世界的光流训练数据，解决了合成数据集的领域差距问题。


<details>
  <summary>Details</summary>
Motivation: 解决光流估计中因合成数据集训练导致的真实世界应用性能限制，提升数据扩展的效益。

Method: 1. 使用单目深度估计网络将单视图图像转换为3D表示；2. 开发对象无关的体积渲染模块和深度感知修复模块，生成动态对象的光流数据。

Result: 生成的FA-Flow数据集在真实世界图像上表现优异，超越了现有无监督和监督方法。

Conclusion: Flow-Anything为光流估计提供了真实世界数据生成的新方法，并可作为基础模型提升下游视频任务性能。
...
### [144] [Difference Inversion: Interpolate and Isolate the Difference with Token Consistency for Image Analogy Generation](https://arxiv.org/abs/2506.07750)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如视觉上下文学习或视觉指令）通常局限于特定模型（如InstructPix2Pix或Inpainting模型），可能导致偏见或编辑能力不足。

Method: 提出Difference Inversion方法，包括Delta Interpolation提取差异、Token Consistency Loss和Zero Initialization of Token Embeddings确保训练准确性。

Result: 实验表明，Difference Inversion在定量和定性上均优于现有基线，能以模型无关的方式生成更可行的B'。

Conclusion: Difference Inversion是一种高效且通用的方法，能够在不依赖特定模型的情况下生成高质量的图像B'。
...
### [145] [Trend-Aware Fashion Recommendation with Visual Segmentation and Semantic Similarity](https://arxiv.org/abs/2506.07773)
*Mohamed Djilani,Nassim Ali Ousalah,Nidhal Eddine Chenni*

Main category: cs.CV

TL;DR: 提出了一种结合视觉、语义和用户行为的时尚推荐系统，通过深度学习模型和合成用户行为提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 解决时尚推荐中视觉、语义和用户行为的多维度融合问题，平衡个性化与流行趋势。

Method: 使用语义分割提取服装区域特征，结合预训练CNN模型，生成合成用户行为数据，通过加权评分函数推荐。

Result: 在DeepFashion数据集上，ResNet-50表现最佳，类别相似度达64.95%，流行度误差最低。

Conclusion: 该方法为个性化时尚推荐提供了可扩展框架，有效结合视觉与流行度信息。
...
### [146] [Language-Vision Planner and Executor for Text-to-Visual Reasoning](https://arxiv.org/abs/2506.07778)
*Yichang Xu,Gaowen Liu,Ramana Rao Kompella,Sihao Hu,Tiansheng Huang,Fatih Ilhan,Selim Furkan Tekin,Zachary Yahn,Ling Liu*

Main category: cs.CV

TL;DR: VLAgent是一个多模态视觉-文本推理系统，通过生成分步推理计划并实时执行，显著提升了视觉语言模型的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在泛化性能上表现不足，VLAgent通过结合规划脚本与执行验证，解决了这一问题。

Method: VLAgent通过上下文学习微调LLM生成分步计划，设计语法-语义解析器修正逻辑错误，并采用集成方法优化执行模块。

Result: 在四个视觉推理基准测试中，VLAgent表现优于现有视觉语言模型和基于LLM的视觉组合方法。

Conclusion: VLAgent通过其独特的优化模块（如SS-Parser和Plan Repairer），在多模态推理任务中实现了显著性能提升。
...
### [147] [Design and Evaluation of Deep Learning-Based Dual-Spectrum Image Fusion Methods](https://arxiv.org/abs/2506.07779)
*Beining Xu,Junxian Li*

Main category: cs.CV

TL;DR: 该论文构建了一个高质量的双光谱数据集，并提出了一种全面的评估框架，以提升可见光和红外图像融合的效果及其在下游任务中的性能。
...
Conclusion: 论文的贡献包括高质量数据集、任务感知的评估框架以及对现有融合方法的全面分析，为未来研究提供了方向。
...
### [148] [Re-ranking Reasoning Context with Tree Search Makes Large Vision-Language Models Stronger](https://arxiv.org/abs/2506.07785)
*Qi Yang,Chenghao Zhang,Lubin Fan,Kun Ding,Jieping Ye,Shiming Xiang*

Main category: cs.CV

TL;DR: 提出了一种名为RCTS的多模态RAG框架，通过构建推理上下文丰富的知识库和树搜索重排序方法，提升大型视觉语言模型（LVLMs）在视觉问答（VQA）任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法在知识推理示例稀缺和检索知识响应不稳定方面存在挑战，需要改进。

Method: 提出RCTS框架，包括自洽评估机制丰富知识库推理模式，以及蒙特卡洛树搜索与启发式奖励（MCTS-HR）重排序方法。

Result: 在多个VQA数据集上实现最先进性能，显著优于上下文学习（ICL）和普通RAG方法。

Conclusion: RCTS框架通过高质量推理上下文和重排序方法，有效提升了LVLMs的响应一致性和性能。
...
### [149] [Image Reconstruction as a Tool for Feature Analysis](https://arxiv.org/abs/2506.07803)
*Eduard Allakhverdov,Dmitrii Tarasov,Elizaveta Goncharova,Andrey Kuznetsov*

Main category: cs.CV

TL;DR: 论文提出了一种通过图像重建解释视觉特征的新方法，比较了SigLIP和SigLIP2模型，发现基于图像任务预训练的编码器保留更多图像信息，并揭示了特征空间的操作对图像重建的影响。
...
Method: 通过图像重建比较不同视觉编码器（如SigLIP和SigLIP2），并分析特征空间的操作对重建图像的影响。

Result: 基于图像任务预训练的编码器保留更多信息；特征空间的旋转操作影响颜色编码。
...
### [150] [Incorporating Uncertainty-Guided and Top-k Codebook Matching for Real-World Blind Image Super-Resolution](https://arxiv.org/abs/2506.07809)
...
### [151] [Looking Beyond Visible Cues: Implicit Video Question Answering via Dual-Clue Reasoning](https://arxiv.org/abs/2506.07811)
*Tieyuan Chen,Huabin Liu,Yi Wang,Chaofan Gan,Mingxi Lyu,Gui Zou,Weiyao Lin*

Main category: cs.CV

TL;DR: 论文提出了一种新任务I-VQA（隐式视频问答），专注于在显式视觉证据不可用的情况下回答问题，并提出了IRM（隐式推理模型）框架，通过双流建模和视觉增强模块显著提升了性能。
...
Method: 提出IRM框架，包含动作-意图模块（AIM）和视觉增强模块（VEM），分别用于生成线索候选和增强视觉表示。

Result: IRM在I-VQA任务中表现优异，分别超越GPT-4o、OpenAI-o3和VideoChat2 0.76%、1.37%和4.87%。

Conclusion: IRM在隐式视觉问答任务中表现出色，并在广告理解和交通预测等类似任务中达到SOTA水平。
...
### [152] [Self-Cascaded Diffusion Models for Arbitrary-Scale Image Super-Resolution](https://arxiv.org/abs/2506.07813)
*Junseo Bang,Joonhee Lee,Kyeonghyun Lee,Haechang Lee,Dong Un Kang,Se Young Chun*

Main category: cs.CV

TL;DR: CasArbi是一种新型的自级联扩散框架，用于任意尺度图像超分辨率，通过逐步增强分辨率解决传统方法的局限性。
...
Method: CasArbi采用自级联扩散框架，将大尺度分解为小尺度序列，逐步提升分辨率，并结合坐标引导的残差扩散模型。

Result: CasArbi在多个任意尺度超分辨率基准测试中，在感知和失真性能指标上均优于现有方法。

Conclusion: CasArbi通过渐进式上采样和扩散模型的结合，为任意尺度超分辨率提供了高效且灵活的解决方案。
...
### [153] [M2Restore: Mixture-of-Experts-based Mamba-CNN Fusion Framework for All-in-One Image Restoration](https://arxiv.org/abs/2506.07814)
*Yongzhen Wang,Yongjun Li,Zhuoran Zheng,Xiao-Ping Zhang,Mingqiang Wei*

Main category: cs.CV

TL;DR: M2Restore提出了一种基于Mixture-of-Experts的Mamba-CNN融合框架，用于高效、鲁棒的全能图像恢复，解决了现有方法在动态变化退化场景中泛化能力不足和局部细节与全局依赖平衡不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 自然图像常受复合退化（如雨、雪、雾）影响，现有图像恢复方法在泛化能力和局部-全局平衡方面表现不足。
...
### [154] [Diffusion models under low-noise regime](https://arxiv.org/abs/2506.07841)
*Elizabeth Pavlova,Xue-Xin Wei*

Main category: cs.CV

TL;DR: 扩散模型在低噪声条件下的行为研究，揭示了训练数据规模、几何结构及目标函数对去噪轨迹的影响。


<details>
  <summary>Details</summary>
Motivation: 探究扩散模型在小噪声条件下的行为，以填补其在低噪声扩散动力学中的研究空白，提升模型鲁棒性和可解释性。

Method: 使用CelebA子集和解析高斯混合基准，系统研究低噪声扩散动力学下的模型行为。

Result: 发现模型在数据流形附近的行为会因训练数据不同而分化，即使高噪声输出收敛。

Conclusion: 研究为理解生成模型在实际小扰动场景中的可靠性提供了新视角。
...
### [155] [F2Net: A Frequency-Fused Network for Ultra-High Resolution Remote Sensing Segmentation](https://arxiv.org/abs/2506.07847)
*Hengzhi Chen,Liqian Feng,Wenhua Wu,Xiaogang Zhu,Shawn Leo,Kun Hu*

Main category: cs.CV

TL;DR: F2Net是一个频率感知框架，通过分解超高分辨率遥感图像为高频和低频分量进行专门处理，解决了传统方法在细节丢失和全局上下文碎片化之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率遥感图像的语义分割在环境监测和城市规划中至关重要，但传统方法存在计算和优化挑战。

Method: F2Net将图像分解为高频和低频分量，分别处理细节和全局上下文，并通过混合频率融合模块和两种新颖的损失函数实现语义一致性和训练稳定性。

Result: 在DeepGlobe和Inria Aerial基准测试中，F2Net分别达到80.22和83.39的mIoU，表现最优。

Conclusion: F2Net通过频率分解和融合，实现了超高分辨率遥感图像语义分割的高效和准确。
...
### [156] [PolyVivid: Vivid Multi-Subject Video Generation with Cross-Modal Interaction and Enhancement](https://arxiv.org/abs/2506.07848)
...
Conclusion: PolyVivid在多主体视频定制中实现了灵活且身份一致的生成，显著优于现有基线。
...
### [157] [SAM2Auto: Auto Annotation Using FLASH](https://arxiv.org/abs/2506.07850)
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型（VLMs）因标注数据稀缺而发展滞后的问题。

Method: 结合SMART-OD（自动掩码生成与开放世界目标检测）和FLASH（实时视频实例分割），通过统计方法减少检测错误并保持对象跟踪一致性。

Result: 实验表明，SAM2Auto的标注精度与人工标注相当，同时大幅降低时间和人力成本。
...
### [158] [Egocentric Event-Based Vision for Ping Pong Ball Trajectory Prediction](https://arxiv.org/abs/2506.07860)
*Ivan Alberico,Marco Cannici,Giovanni Cioffi,Davide Scaramuzza*

Main category: cs.CV

TL;DR: 提出了一种基于事件相机的实时乒乓球轨迹预测系统，利用高时间分辨率提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统相机在高速度下存在延迟和运动模糊问题，事件相机能提供更高时间分辨率，提升轨迹预测的准确性和实时性。

Method: 结合事件相机和Meta Project Aria眼镜的传感器数据，利用注视点视觉技术处理关键区域，降低计算延迟。

Result: 系统在最坏情况下总延迟为4.5毫秒，比传统30 FPS系统快14.7倍，轨迹预测性能显著提升。

Conclusion: 首次实现基于事件相机的乒乓球轨迹预测，展示了高时间分辨率在实时预测中的优势。
...
### [159] [VIVAT: Virtuous Improving VAE Training through Artifact Mitigation](https://arxiv.org/abs/2506.07863)
...
Method: 提出VIVAT方法，分析五种伪影的根源，并通过调整损失权重、改进填充策略和引入空间条件归一化进行优化。

Result: 在多个基准测试中，VIVAT在图像重建（PSNR和SSIM）和文本到图像生成（CLIP分数）方面取得最优结果。

Conclusion: VIVAT在保持KL-VAE框架简单性的同时，解决了其实际挑战，为优化VAE训练提供了实用方案。
...
TL;DR: 提出了一种基于时空状态空间模型（STSSM）的高效事件相机光流估计算法，性能优越且计算复杂度低。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型（如CNN、RNN、ViT）在光流估计中性能优越但计算效率不足，而异步事件方法（如SNN、GNN）效率高但时空信息捕捉不足。

Method: 引入STSSM模块和新网络架构，利用状态空间模型高效捕捉事件数据的时空相关性。

Result: 在DSEC基准测试中，模型推理速度快4.5倍，计算量比TMA低8倍，比EV-FlowNet低2倍，性能竞争力强。

Conclusion: STSSM模块为事件相机光流估计提供了高效且高性能的解决方案。
...
TL;DR: CrosswalkNet是一种高效的深度学习框架，用于从高分辨率航拍图像中检测行人横道，采用定向边界框（OBB）提升检测精度，并在多地区数据集上表现出色。
...
Method: 提出CrosswalkNet框架，结合OBB、注意力机制和优化技术，利用23,000多个标注样本训练。

Result: 在多个地区数据集上实现高精度（96.5%）和高召回率（93.3%），无需迁移学习。

Conclusion: CrosswalkNet为决策者和规划者提供了提升行人安全和城市流动性的有效工具。
...
TL;DR: 论文提出了一种名为EgoM2P的框架，用于解决第一人称视角多模态数据处理的挑战，通过高效的时序标记器和掩码建模实现多任务学习。


<details>
  <summary>Details</summary>
Motivation: 第一人称视角的多模态数据（如RGB视频、深度、相机位姿和视线）在增强现实、机器人和人机交互中有广泛应用，但数据异构性和缺失模态的伪标签生成困难限制了现有方法的扩展。

Method: 引入高效的时序标记器，提出EgoM2P框架，通过掩码建模学习多模态时序标记，支持多任务学习（如视线预测、相机跟踪和深度估计）。

Result: EgoM2P在多任务中表现优于或匹配专用模型，且速度更快。

Conclusion: EgoM2P是一个高效、通用的第一人称视角4D理解模型，将开源以推动相关研究。
...
TL;DR: 论文提出了一种针对视频扩散模型的去学习技术，仅需5对多模态提示对即可生成“拒绝向量”，用于中和模型中的有害概念。
...
Method: 通过多模态提示对生成拒绝向量，并采用低秩分解方法优化嵌入的协方差差异，以隔离目标概念。

Result: 方法成功中和多种有害内容（如裸露、暴力、版权等），同时保持生成视频的视觉质量。

Conclusion: 该方法无需重新训练或原始数据，直接将拒绝方向嵌入模型权重，提高了对抗性绕过尝试的鲁棒性。
...
TL;DR: 论文提出了一种新的多模态推理方法，通过生成上下文感知的QA对和开源数据集WeThink，结合混合奖励机制的强化学习，显著提升了多模态大语言模型的性能。
...
Method: 1. 开发可扩展的多模态QA合成管道；2. 开源WeThink数据集；3. 探索混合奖励机制的强化学习。

Result: 在14个多模态基准测试中表现优异，数据多样性持续提升模型性能。

Conclusion: 提出的方法有效提升了多模态推理能力，自动化数据管道进一步增强了模型的泛化能力。
...
### [165] [A Comparative Study of U-Net Architectures for Change Detection in Satellite Images](https://arxiv.org/abs/2506.07925)
*Yaxita Amin,Naimisha S Trivedi,Rashmi Bhattad*

Main category: cs.CV

TL;DR: 本文通过分析34篇论文，对18种U-Net变体在遥感变化检测中的应用进行了全面比较和评估，填补了该领域的空白。
...
### [166] [Mimicking or Reasoning: Rethinking Multi-Modal In-Context Learning in Vision-Language Models](https://arxiv.org/abs/2506.07936)
*Chengyue Huang,Yuchen Zhu,Sichen Zhu,Jingyun Xiao,Moises Andrade,Shivang Chopra,Zsolt Kira*

Main category: cs.CV

TL;DR: 研究发现，视觉语言模型（VLMs）在多模态上下文学习（MM-ICL）中依赖浅层启发式方法而非真实任务理解，性能在分布偏移时下降。提出的新方法（MM-ICL with Reasoning）未显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 验证VLMs是否真正具备多模态上下文学习能力，而非依赖浅层策略。

Method: 提出MM-ICL with Reasoning，为每个示例生成答案和推理依据，并在不同数据集和模型上进行实验。

Result: VLMs在分布偏移时性能下降，且对演示信息利用有限，新方法未显著提升效果。
...
### [167] [Decoupling the Image Perception and Multimodal Reasoning for Reasoning Segmentation with Digital Twin Representations](https://arxiv.org/abs/2506.07943)
*Yizhen Li,Dell Zhang,Xuelong Li,Yiqing Shen*

Main category: cs.CV

TL;DR: DTwinSeger是一种新型的多模态视觉-文本任务方法，通过数字孪生（DT）表示解耦感知与推理，利用LLM在DT表示上进行显式推理，实现了在多个基准测试中的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在图像标记化过程中破坏了对象的连续空间关系，限制了多模态推理能力。

Method: DTwinSeger将任务分为两阶段：首先生成保留空间关系的DT表示，然后利用LLM在DT表示上进行推理。

Result: 在两个图像RS基准和三个图像参考分割基准上实现了最优性能。

Conclusion: DT表示是视觉与文本之间的有效桥梁，仅用LLM即可完成复杂多模态推理任务。
...
### [168] [Creating a Historical Migration Dataset from Finnish Church Records, 1800-1920](https://arxiv.org/abs/2506.07960)
*Ari Vesalainen,Jenna Kanerva,Aida Nitsch,Kiia Korsu,Ilari Larkiola,Laura Ruotsalainen,Filip Ginter*

Main category: cs.CV

TL;DR: 本文通过深度学习技术自动化处理芬兰1800-1920年间的手写教会迁移记录，构建了一个包含600多万条目的结构化数据集，用于研究历史人口迁移模式。


<details>
  <summary>Details</summary>
Motivation: 利用教会记录研究芬兰历史人口迁移模式，填补数字化历史数据的空白。

Method: 采用深度学习流程（包括布局分析、表格检测、单元格分类和手写识别）自动化提取数据。

Result: 成功构建了一个包含600多万条目的结构化数据集，支持研究内部迁移、城市化、家庭迁移及疾病传播。

Conclusion: 该研究展示了如何将大量手写档案转化为结构化数据，为历史和人口研究提供支持。
...
### [169] [SlideCoder: Layout-aware RAG-enhanced Hierarchical Slide Generation from Design](https://arxiv.org/abs/2506.07964)
...
TL;DR: Slide2Code提出了一种基于参考图像的幻灯片生成方法SlideCoder，通过布局感知和检索增强技术，显著提升了生成效果。


<details>
  <summary>Details</summary>
Motivation: 手动制作幻灯片耗时且需要专业知识，现有基于自然语言的生成方法难以捕捉视觉和结构细节。

Method: 提出Slide2Code任务，引入SlideCoder框架，结合颜色梯度分割算法和分层检索增强生成方法。
...
### [170] [SpaCE-10: A Comprehensive Benchmark for Multimodal Large Language Models in Compositional Spatial Intelligence](https://arxiv.org/abs/2506.07966)
...
Method: 定义10种原子空间能力和8种组合能力，采用分层标注流程生成高质量QA对，覆盖点云输入和多选QA等设置。

Result: 测试发现最先进的MLLMs仍显著落后于人类，计数能力的不足尤其限制其组合空间能力。

Conclusion: SpaCE-10为MLLM社区提供了重要发现和评估工具，代码和数据集已开源。
...
### [171] [CyberV: Cybernetics for Test-time Scaling in Video Understanding](https://arxiv.org/abs/2506.07971)
*Jiahao Meng,Shuyang Sun,Yue Tan,Lu Qi,Yunhai Tong,Xiangtai Li,Longyin Wen*

Main category: cs.CV

TL;DR: CyberV框架通过引入控制论循环（传感器、控制器和MLLM推理系统），提升多模态大语言模型（MLLMs）对长视频或复杂视频的理解能力，无需重新训练。实验显示性能显著提升，甚至超越GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs因前馈处理方式在长视频或复杂视频理解上存在计算需求高、鲁棒性差和准确性不足的问题，尤其是参数较少的模型。

Method: 提出CyberV框架，包含传感器监控MLLM中间过程（如注意力漂移），控制器触发自校正和动态资源分配，形成自适应系统。

Result: CyberV显著提升模型性能（如Qwen2.5-VL-7B提升8.3%），超越GPT-4o，并在通用基准测试中表现一致。

Conclusion: CyberV通过控制论循环有效增强MLLMs的动态视频理解能力，具有鲁棒性和泛化性，代码已开源。
...
### [172] [OneIG-Bench: Omni-dimensional Nuanced Evaluation for Image Generation](https://arxiv.org/abs/2506.07977)
*Jingjing Chang,Yixiao Fang,Peng Xing,Shuhan Wu,Wei Cheng,Rui Wang,Xianfang Zeng,Gang Yu,Hai-Bao Chen*

Main category: cs.CV

TL;DR: OneIG-Bench是一个全面的文本到图像（T2I）模型评估框架，专注于多维度细粒度评估，包括提示-图像对齐、文本渲染精度、推理生成内容、风格化和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有T2I模型评估系统不够全面，未能充分评估推理能力、文本渲染和风格等关键维度。

Method: 设计OneIG-Bench框架，支持灵活评估，用户可选择特定维度生成图像并完成对应评估。

Result: OneIG-Bench提供了公开的代码库和数据集，支持可重复的评估研究和跨模型比较。

Conclusion: OneIG-Bench填补了T2I模型评估的空白，帮助研究人员全面分析模型性能。
...
### [173] [Real-time Localization of a Soccer Ball from a Single Camera](https://arxiv.org/abs/2506.07981)
...
### [174] [CXR-LT 2024: A MICCAI challenge on long-tailed, multi-label, and zero-shot disease classification from chest X-ray](https://arxiv.org/abs/2506.07984)
...
TL;DR: CXR-LT 2024是一个社区驱动的项目，旨在通过扩展数据集（377,110张胸部X光片和45种疾病标签）和改进任务设计（包括零样本学习）来提升肺部疾病分类的性能和可测量性。


<details>
  <summary>Details</summary>
Motivation: 解决开放长尾肺部疾病分类中的挑战，并提升现有技术的可测量性，以推动临床实用的诊断模型发展。

Method: 通过扩展数据集、引入零样本学习任务，以及整合多模态模型、生成方法和零样本学习策略来优化分类性能。

Result: 提供了一个更全面的数据集和任务设计，支持罕见疾病检测和零样本泛化，为未来研究提供了宝贵资源。

Conclusion: CXR-LT 2024通过社区合作和技术创新，推动了胸部X光诊断模型的临床实用性和泛化能力。
...
### [175] [Rethinking Crowd-Sourced Evaluation of Neuron Explanations](https://arxiv.org/abs/2506.07985)
...
Method: 引入重要性采样选择最有价值的输入样本，并提出贝叶斯方法聚合评分以减少噪声。

Result: 实现了约30倍的成本降低和约5倍的评分效率提升，并比较了不同解释方法的质量。

Conclusion: 新方法显著提高了评估效率和准确性，为神经元解释研究提供了实用工具。
...
### [176] [Rethinking Cross-Modal Interaction in Multimodal Diffusion Transformers](https://arxiv.org/abs/2506.07986)
*Zhengyao Lv,Tianlin Pan,Chenyang Si,Zhaoxi Chen,Wangmeng Zuo,Ziwei Liu,Kwan-Yee K. Wong*

Main category: cs.CV

TL;DR: 论文提出了一种名为TACA的方法，通过动态调整跨模态注意力的温度和时序感知权重，解决了MM-DiT模型中文本与图像对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 当前MM-DiT模型（如FLUX）在文本驱动视觉生成中表现优异，但仍存在文本提示与生成内容对齐不精确的问题。

Method: 提出了TACA方法，通过温度缩放和时序依赖调整动态平衡跨模态交互，并结合LoRA微调。

Result: 在T2I-CompBench基准测试中，TACA显著提升了文本-图像对齐效果，且在FLUX和SD3.5等模型上验证了其有效性。
...
### [177] [PairEdit: Learning Semantic Variations for Exemplar-based Image Editing](https://arxiv.org/abs/2506.07992)
*Haoguang Lu,Jiacheng Chen,Zhenguo Yang,Aurele Tohokantche Gnanha,Fu Lee Wang,Li Qing,Xudong Mao*

Main category: cs.CV

TL;DR: PairEdit是一种无需文本指导即可从少量图像对中学习复杂编辑语义的视觉编辑方法。
...
Result: PairEdit成功学习复杂语义并显著提升内容一致性。

Conclusion: PairEdit在无需文本指导的情况下有效学习编辑语义，优于基线方法。
...
TL;DR: MADFormer是一种结合自回归（AR）和扩散模型的混合Transformer，通过分块生成和分层混合优化高分辨率图像生成。
...
Method: MADFormer分块生成图像，AR层用于全局条件，扩散层用于局部细化。

Result: 实验表明分块生成显著提升高分辨率图像性能，混合分层优化质量与效率平衡。
...
Method: 采用自回归模型处理结构化3D场景，提出统一的LLM框架，并详细探讨了数据表示和模态特定目标等关键设计选择。

Result: 在四个核心3D任务（渲染、识别、指令跟随和问答）及四个3D数据集上评估性能，并在真实世界3D物体识别任务中展示了模型的有效性。

Conclusion: 通过量化形状编码丰富了3D模态，证明了模型在复杂3D物体形状重建和识别任务中的有效性。
...
TL;DR: MTV是一个音频同步视频生成框架，通过分离音频为语音、音效和音乐轨道，实现精细控制和高质量视频生成。


<details>
  <summary>Details</summary>
Motivation: 音频与视觉世界紧密同步，是可控视频生成的理想控制信号，但现有方法在复杂音频类型下难以实现高质量同步。

Method: MTV框架将音频分离为语音、音效和音乐轨道，分别控制唇部动作、事件时间和视觉情绪，并引入DEMIX数据集支持多阶段训练。

Result: MTV在视频质量、文本-视频一致性和音频-视频对齐等六个标准指标上达到最先进性能。

Conclusion: MTV通过音频分离和多阶段训练，实现了高质量、语义对齐的视频生成。
...
TL;DR: ZeroVO是一种新型视觉里程计算法，无需预定义或静态相机校准，通过几何感知网络、语言先验和半监督训练实现零样本泛化。


<details>
  <summary>Details</summary>
Motivation: 现有视觉里程计方法依赖预定义或静态相机校准，限制了其泛化能力。ZeroVO旨在克服这一限制，实现跨相机和环境的零样本泛化。

Method: 1. 设计无需校准的几何感知网络；2. 引入语言先验增强特征提取；3. 开发半监督训练范式适应新场景。

Result: 在KITTI、nuScenes和Argoverse 2等基准测试中性能提升30%以上，并在GTA合成数据集上验证了泛化能力。
...
Method: 设计分层世界抽象，结合物理模拟器的精细控制和生成模型的逼真输出。

Result: Dreamland在图像质量上提升50.8%，可控性提升17.9%，并支持现成生成模型的使用。

Conclusion: Dreamland在可控性和生成质量上表现优异，对AI代理训练有潜力。
...
TL;DR: 该论文研究了视觉语言模型（VLMs）在视觉任务中的表现，发现其性能显著低于其视觉编码器，并分析了失败原因。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型如何整合视觉和语言信息，并评估其在视觉任务中的表现。

Method: 通过一系列视觉中心基准测试（如深度估计、对应关系）比较VLMs与其视觉编码器的性能，并分析失败原因。

Result: VLMs在视觉任务中表现远低于视觉编码器，接近随机水平，主要瓶颈在于未能有效利用视觉信息。

Conclusion: VLMs在视觉任务中的表现受限于语言模型的先验知识，未能充分利用视觉信息，研究为未来改进提供了诊断和评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 解决自回归视频扩散模型中的曝光偏差问题，即在推理时模型需基于自身不完美输出生成序列。

Method: 采用自回归展开和KV缓存策略，训练时基于自生成输出生成帧，并引入整体视频级损失和滚动KV缓存机制。

Result: 实验表明，该方法在单GPU上实现亚秒级延迟的实时视频生成，性能优于或匹配更慢的非因果扩散模型。
...
TL;DR: 论文研究了Vision Transformers中高范数令牌导致噪声注意力图的机制，提出了一种无需重新训练的方法来缓解这一问题。
...
Method: 通过将高范数激活转移到额外的未训练令牌中，模拟注册令牌的效果。

Result: 方法能生成更清晰的注意力和特征图，提升下游视觉任务性能，效果接近显式训练注册令牌的模型。

Conclusion: 测试时注册令牌为预训练模型提供了一种无需训练的解决方案，提高了模型的可解释性。
...
TL;DR: ViGaL是一种通过玩街机游戏提升多模态大语言模型（MLLM）泛化推理能力的新方法，显著提升了模型在数学和多学科问题上的表现。


<details>
  <summary>Details</summary>
Motivation: 受认知科学启发，游戏能促进可迁移的认知技能，因此提出通过游戏训练提升MLLM的泛化推理能力。

Method: 使用强化学习（RL）在街机游戏（如Snake）上对7B参数的MLLM进行后训练，无需接触具体问题或图表。

Result: 模型在多模态数学和多学科基准测试中表现优异，甚至超越专门调优的模型，同时保持基础模型在通用视觉任务上的性能。

Conclusion: 规则化游戏可作为可控且可扩展的预训练任务，解锁MLLM的泛化多模态推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 多任务密集预测需要大量标注，而部分任务标注的局限性促使研究零样本学习。

Method: 采用扩散模型，结合任务编码、任务条件化和统一潜在损失，设计多流模型和任务注意力机制。

Result: 在8个基准测试的7个任务上表现优于基线方法。

Conclusion: StableMTL通过潜在回归和任务注意力机制，实现了高效的多任务学习。
...
TL;DR: 4DGT是一种基于4D高斯和Transformer的动态场景重建模型，通过单目视频训练，统一静态与动态组件，支持高效渲染。


<details>
  <summary>Details</summary>
Motivation: 动态场景重建需要处理复杂的时间变化环境，现有方法效率低且难以扩展。

Method: 使用4D高斯作为归纳偏置，提出密度控制策略，支持长时空输入，采用滚动窗口处理64帧。

Result: 4DGT在真实视频中显著优于其他高斯网络，跨域视频上与优化方法精度相当，重建时间从小时级降至秒级。

Conclusion: 4DGT通过高效的前馈推理和密度控制，实现了动态场景的高质量快速重建。
...
### [190] [On geodesic disks enclosing many points](https://arxiv.org/abs/2506.06477)
*Prosenjit Bose,Guillermo Esteban,David Orden,Rodrigo Silveira,Tyler Tuttle*

Main category: cs.CG

TL;DR: 论文研究了在多边形中点集的几何性质，给出了关于包含点数的上下界，并探讨了特殊情况和两色问题的结果。


<details>
  <summary>Details</summary>
Motivation: 研究多边形中点集的几何性质，特别是关于测地圆包含点数的上下界，以解决相关问题。

Method: 通过几何分析和数学推导，建立点集在多边形中的测地圆包含点数的上下界，并分析特殊情况。

Result: 给出了Π(n)的上下界，特殊情况下给出了紧界，两色问题也得到了上下界和具体结果。

Conclusion: 论文在多边形中点集的几何性质上取得了重要进展，为相关问题提供了理论支持。

Abstract: Let $ \Pi(n) $ be the largest number such that for every set $ S $ of $ n $
...
\frac{n}{11.3}\rceil+1$ points both inside and outside the disk.

</details>


### [191] [Rectangular Duals on the Cylinder and the Torus](https://arxiv.org/abs/2506.07170)
*Therese Biedl,Philipp Kindermann,Jonathan Klawitter*

Main category: cs.CG

TL;DR: 本文研究了在圆柱和平环面上图的矩形对偶表示，给出了圆柱上图的矩形对偶的完整特征，并提出了在平环面上测试和构造矩形对偶的高效方法。


<details>
  <summary>Details</summary>
Motivation: 研究非平面表面（如圆柱和平环面）上图的矩形对偶表示，扩展了平面图矩形对偶的理论和应用范围。

Method: 通过正则边标记（regular edge labeling）描述矩形邻接关系，测试和构造圆柱和平环面上的矩形对偶。

Result: 完全刻画了圆柱上图的可矩形对偶性，并提出了平环面上矩形对偶的高效测试和构造方法。

Conclusion: 本文为非平面表面上的矩形对偶表示提供了理论基础和实用工具，扩展了相关研究的适用范围。

Abstract: A rectangular dual of a plane graph $G$ is a contact representation of $G$ by
...
dual, if applicable, can be done efficiently.

</details>


### [192] [An $O(n\log n)$ Algorithm for Single-Source Shortest Paths in Disk Graphs](https://arxiv.org/abs/2506.07571)
*Mark de Berg,Sergio Cabello*

Main category: cs.CG

TL;DR: 论文证明了在盘图上单源最短路径问题可以在O(n log n)时间内解决，而在胖三角形交集图上可以在O(n log² n)时间内解决。


<details>
  <summary>Details</summary>
Motivation: 研究在特定几何图（如盘图和胖三角形交集图）上高效解决单源最短路径问题的算法。

Method: 通过分析图的几何特性，设计针对盘图和胖三角形交集图的优化算法。

Result: 在盘图上达到O(n log n)时间复杂度，在胖三角形交集图上达到O(n log² n)时间复杂度。

Conclusion: 证明了在特定几何图上可以高效解决单源最短路径问题，为相关领域提供了新的算法思路。

Abstract: We prove that the single-source shortest-path problem on disk graphs can be
...
### [193] [Vid2Sim: Generalizable, Video-based Reconstruction of Appearance, Geometry and Physics for Mesh-free Simulation](https://arxiv.org/abs/2506.06440)
...
<details>
  <summary>Details</summary>
Motivation: 从视频中忠实重建纹理形状和物理属性是一个具有挑战性的问题，现有方法依赖复杂优化且通用性差。

Method: Vid2Sim结合前馈神经网络和轻量优化管道，利用Linear Blend Skinning实现高效无网格模拟。
...
### [194] [Splat and Replace: 3D Reconstruction with Repetitive Elements](https://arxiv.org/abs/2506.06462)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如NeRF和3DGS）在训练视角不足时，对未见过或被遮挡部分的渲染质量较低。环境中常存在重复元素，可以用于改善这一问题。

Method: 提出一种方法，在3DGS重建中分割每个重复实例，将它们对齐并共享信息，同时考虑实例间的外观变化。

Result: 在合成和真实场景中验证，新视角合成质量显著提升。

Conclusion: 通过利用重复元素，有效改善了场景重建和新视角合成的质量。
...
### [195] [Noise Consistency Regularization for Improved Subject-Driven Image Synthesis](https://arxiv.org/abs/2506.06483)
...
<details>
  <summary>Details</summary>
Motivation: 现有微调方法存在欠拟合（无法可靠捕捉主体身份）和过拟合（记忆主体图像并减少背景多样性）问题。

Method: 提出两种辅助一致性损失：先验一致性正则化损失（保持非主体图像的扩散噪声一致性）和主体一致性正则化损失（增强模型对噪声调制潜码的鲁棒性）。
...
### [196] [JGS2: Near Second-order Converging Jacobi/Gauss-Seidel for GPU Elastodynamics](https://arxiv.org/abs/2506.06494)
*Lei Lan,Zixuan Lu,Chun Yuan,Weiwei Xu,Hao Su,Huamin Wang,Chenfanfu Jiang,Yin Yang*

Main category: cs.GR

TL;DR: 提出一种新型GPU算法，在保持Jacobi方法良好并行性的同时，达到与全空间牛顿法相当的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 并行模拟中，收敛性和并行性常被视为冲突目标，本文旨在解决这一问题。

Method: 通过分析过冲现象，提出理论上的二阶最优解，并结合Cubature采样和全坐标公式化，实现高效预计算。

Result: 算法运行时成本略高于Jacobi方法，但收敛速度接近牛顿法，实验显示收敛性能优于现有GPU方法50至100倍。

Conclusion: 该方法在刚性和软材料模拟中均实现二阶收敛，提供高质量仿真效果。
...
### [197] [CrossGen: Learning and Generating Cross Fields for Quad Meshing](https://arxiv.org/abs/2506.07020)
...
Method: 使用自动编码器网络架构，将点云表面编码为稀疏体素网格，解码为基于SDF的几何和交叉场，并结合扩散模型处理部分输入。

Result: 方法能在秒级内生成高质量交叉场，具有高几何保真度、噪声鲁棒性和快速推理能力。
...
### [198] [Accelerating 3D Gaussian Splatting with Neural Sorting and Axis-Oriented Rasterization](https://arxiv.org/abs/2506.07069)
...
TL;DR: 论文提出了一种架构与算法协同设计方法，通过轴定向光栅化、神经排序和可重构处理阵列优化3D高斯溅射（3DGS）的实时渲染效率，显著提升速度和能效。


<details>
  <summary>Details</summary>
Motivation: 尽管3DGS在高质量视图合成中表现优异，但在资源受限设备上实现实时渲染仍面临挑战，主要由于功耗和面积限制。

Method: 1. 轴定向光栅化减少冗余计算；2. 神经排序替代硬件排序；3. 可重构处理阵列支持光栅化和神经网络推理；4. 使用π轨迹瓦片调度优化内存访问。

Result: 实验显示设计在保持渲染质量的同时，速度提升23.4~27.8倍，能耗降低28.8~51.4倍。

Conclusion: 该设计显著提升了3DGS在资源受限设备上的实时渲染效率，计划开源以推动领域发展。
...
### [199] [HOI-PAGE: Zero-Shot Human-Object Interaction Generation with Part Affordance Guidance](https://arxiv.org/abs/2506.07209)
*Lei Li,Angela Dai*
...
Method: 引入部分功能图（PAGs）作为结构化HOI表示，指导三阶段合成：分解3D物体为几何部分，从文本生成参考视频并提取运动约束，优化4D HOI序列以满足部分级接触约束。

Result: 实验表明，该方法灵活，能生成复杂多物体或多人的交互序列，显著提升了零样本4D HOI生成的现实感和文本对齐。

Conclusion: HOI-PAGE通过部分级功能推理实现了更真实、多样化的4D HOI合成，优于现有方法。
...
### [200] [Immersive Visualization of Flat Surfaces Using Ray Marching](https://arxiv.org/abs/2506.07558)
*Fabian Lander,Diaaeldin Taha*

Main category: cs.GR

TL;DR: 提出了一种基于光线步进的高效可视化方法，用于探索多种几何表面，并展示了其实际应用和实现细节。


<details>
  <summary>Details</summary>
Motivation: 为几何表面（如平移表面、镜像房间等）提供直观且高效的可视化方法。

Method: 使用光线步进技术，结合计算效率优化，实现几何表面的可视化。

Result: 通过多个示例验证了方法的实用性，并提供了代码实现。

Conclusion: 该方法不仅高效，还适用于教育和推广，相关资源已公开。
...
### [201] [PIG: Physically-based Multi-Material Interaction with 3D Gaussians](https://arxiv.org/abs/2506.07657)
*Zeyu Xiao,Zhenyi Wu,Mingyang Sun,Qipeng Yan,Yufan Guo,Zhuoer Liang,Lihua Zhang*

Main category: cs.GR

TL;DR: PIG方法通过结合3D高斯分割与物理模拟，解决了3D高斯场景中物体交互的精度问题，提升了视觉质量和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 3D高斯场景中物体交互存在分割不准确、变形不精确和渲染伪影问题，需改进。
...
### [202] [GaussianVAE: Adaptive Learning Dynamics of 3D Gaussians for High-Fidelity Super-Resolution](https://arxiv.org/abs/2506.07897)
*Shuja Khalid,Mohamed Ibrahim,Yang Liu*

Main category: cs.GR

TL;DR: 提出一种轻量级生成模型，通过Hessian辅助采样策略提升3D高斯泼溅的分辨率和几何保真度，突破输入分辨率限制。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS方法受限于输入分辨率，无法生成比训练视图更精细的细节。

Method: 使用轻量级生成模型预测并细化额外的3D高斯分布，采用Hessian辅助采样策略智能识别需要密集化的区域。

Result: 在单消费级GPU上实时运行（0.015秒/次），几何精度和渲染质量显著优于现有方法。
...
### [203] [Speedy Deformable 3D Gaussian Splatting: Fast Rendering and Compression of Dynamic Scenes](https://arxiv.org/abs/2506.07917)
*Allen Tu,Haiyang Ying,Alex Hanson,Yonghan Lee,Tom Goldstein,Matthias Zwicker*

Main category: cs.GR

TL;DR: SpeeDe3DGS通过时间敏感剪枝和GroupFlow技术，显著提升动态3DGS和4DGS的渲染速度，减少计算和内存需求。


<details>
  <summary>Details</summary>
Motivation: 动态3DGS中每帧对每个高斯进行神经推理导致渲染速度慢、计算和内存需求高，亟需优化。
...
### [204] [Squeeze3D: Your 3D Generation Model is Secretly an Extreme Neural Compressor](https://arxiv.org/abs/2506.07932)
...
Result: 实验显示，Squeeze3D在纹理网格、点云和辐射场上的压缩比分别达到2187x、55x和619x，且视觉质量接近现有方法。
...
### [205] [Tactile MNIST: Benchmarking Active Tactile Perception](https://arxiv.org/abs/2506.06361)
...
<details>
  <summary>Details</summary>
Motivation: 触觉感知能增强机器人操作的灵活性，但其局部性限制了全局场景理解。主动感知技术可以弥补这一不足，但目前缺乏标准化基准。

Method: 引入Tactile MNIST Benchmark Suite，包含多样化的模拟场景和真实数据集，支持触觉感知任务的标准化评估。

Result: 提供了13,500个合成3D MNIST数字模型和153,600个真实触觉样本，并训练了CycleGAN以实现逼真的触觉模拟渲染。

Conclusion: 该基准套件通过标准化协议和可复现的评估框架，推动了触觉感知和主动感知领域的系统性进展。
...
### [206] [CPS-Guard: Framework for Dependability Assurance of AI- and LLM-Based Cyber-Physical Systems](https://arxiv.org/abs/2506.06381)
...
TL;DR: CPS-Guard是一个通过多角色编排自动化AI驱动的CPS保证过程的框架，有效检测漏洞并支持自适应恢复策略。


<details>
  <summary>Details</summary>
Motivation: 传统验证方法难以应对AI组件的动态性和不可预测性，需要一种新方法来确保CPS的可靠性和安全性。

Method: 采用多角色编排，在模拟环境中分配专用代理（如安全监控、故障注入等）持续评估和优化AI行为。

Result: 通过自动驾驶车辆的案例研究，CPS-Guard成功检测漏洞、管理性能影响并支持自适应恢复策略。

Conclusion: CPS-Guard为安全和安全关键系统提供了一种结构化且可扩展的严格验证与验证解决方案。
...
### [207] [Active Illumination Control in Low-Light Environments using NightHawk](https://arxiv.org/abs/2506.06394)
*Yash Turkar,Youngjin Kim,Karthik Dantu*

Main category: cs.RO

TL;DR: NightHawk框架通过结合主动照明和曝光控制，优化了地下环境中的图像质量，提高了特征检测和匹配的可靠性。
...
Method: 提出NightHawk框架，通过在线贝叶斯优化确定最佳光照强度和曝光时间，并使用基于特征检测的指标作为优化目标。

Result: 实验表明，特征检测和匹配性能提升了47-197%，在恶劣光照条件下实现了更可靠的视觉估计。

Conclusion: NightHawk有效解决了地下环境中的视觉挑战，为机器人导航提供了实用解决方案。
...
### [208] [Edge-Enabled Collaborative Object Detection for Real-Time Multi-Vehicle Perception](https://arxiv.org/abs/2506.06474)
...
<details>
  <summary>Details</summary>
Motivation: 传统车载感知系统因遮挡和盲区精度有限，云端方案延迟高，无法满足实时需求。

Method: ECOD框架整合PACE（多车数据聚合）和VOTE（共识投票分类）算法，利用边缘计算实现低延迟处理。

Result: 实验表明ECOD分类准确率提升75%，且满足实时性要求。

Conclusion: 边缘计算可显著提升CAV协作感知能力，适用于延迟敏感场景。
...
### [209] [Enhancing Situational Awareness in Underwater Robotics with Multi-modal Spatial Perception](https://arxiv.org/abs/2506.06476)
*Pushyami Kaveti,Ambjorn Grimsrud Waldum,Hanumant Singh,Martin Ludvigsen*

Main category: cs.RO

TL;DR: 论文提出了一种多模态感知方法，融合摄像头、IMU和声学设备数据，以提升水下SLAM的鲁棒性和实时性。


<details>
  <summary>Details</summary>
Motivation: 水下环境（如光线衰减、散射和低对比度）导致传统视觉SLAM失效，且现有方法多依赖单目或双目输入，难以适应多摄像头配置。

Method: 结合几何与学习技术及语义分析，利用多传感器数据（摄像头、IMU、声学设备）进行实时SLAM。

Result: 实验证明在视觉挑战性水下环境中实现可靠状态估计和高质量3D重建的可行性。

Conclusion: 系统存在传感器校准和学习方法局限性等问题，需进一步研究以支持大规模水下作业。
...
### [210] [BeliefMapNav: 3D Voxel-Based Belief Map for Zero-Shot Object Navigation](https://arxiv.org/abs/2506.06487)
*Zibo Zhou,Yue Hu,Lingkai Zhang,Zonglin Li,Siheng Chen*

Main category: cs.RO

TL;DR: 论文提出了一种基于3D体素信念地图的零样本物体导航方法（BeliefMapNav），通过结合LLM的语义推理和视觉嵌入，实现了高效的目标定位和全局导航。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本物体导航方法依赖通用模型（如LLM和VLM），但缺乏全局环境理解和空间推理能力，导致导航效率低下。

Method: 提出了一种3D体素信念地图，结合LLM语义先验和实时观测，构建目标位置的全局后验信念，并设计了BeliefMapNav导航系统。

Result: 在HM3D、MP3D和HSSD基准测试中，BeliefMapNav取得了SOTA的SR和SPL，SPL比之前最佳方法提升了46.4%。

Conclusion: BeliefMapNav通过3D体素地图和路径规划，显著提升了零样本物体导航的效率和精确性。
...
### [211] [MapleGrasp: Mask-guided Feature Pooling for Language-driven Efficient Robotic Grasping](https://arxiv.org/abs/2506.06535)
*Vineet Bhat,Naman Patel,Prashanth Krishnamurthy,Ramesh Karri,Farshad Khorrami*

Main category: cs.RO

TL;DR: 论文提出了一种基于掩码引导特征池化的轻量级增强方法，用于语言驱动的机器人抓取（LDRG），通过两阶段训练策略和开放词汇抓取数据集RefGraspNet，显著提升了抓取性能。


<details>
  <summary>Details</summary>
Motivation: 解决自然语言驱动的机器人抓取中未见物体的挑战，提升抓取效率和泛化能力。

Method: 采用两阶段训练策略：1) 使用视觉语言模型生成特征图和分割掩码；2) 通过掩码区域的特征池化预测抓取姿态。结合深度映射和逆运动学扩展到3D。

Result: 在OCID-VLG基准上提升12%，LIBERO仿真基准表现媲美VLA模型，真实实验中成功率57%，超越基线7%。

Conclusion: 掩码引导特征池化方法显著提升了语言驱动抓取的性能和泛化能力，RefGraspNet数据集进一步增强了模型表现。
...
### [212] [Semantics-aware Predictive Inspection Path Planning](https://arxiv.org/abs/2506.06560)
*Mihir Dharmadhikari,Kostas Alexis*

Main category: cs.RO

TL;DR: 本文提出了一种名为“语义感知预测规划”（SPP）的新型语义感知检测路径规划方法，针对工业环境中重复语义结构的检测需求，通过识别和预测语义模式，优化检测路径。


<details>
  <summary>Details</summary>
Motivation: 工业环境中（如船舶压载水舱）的语义结构通常具有重复性，需要高效检测方法。

Method: 提出算法识别语义场景图中的重复模式，并预测未探索区域的语义演化；设计两种针对压载水舱检测的路径规划策略。

Result: 仿真和实验表明，该方法在检测时间和语义覆盖率上优于现有技术。

Conclusion: SPP方法显著提升了检测效率，适用于复杂工业环境。
...
### [213] [Towards Terrain-Aware Task-Driven 3D Scene Graph Generation in Outdoor Environments](https://arxiv.org/abs/2506.06562)
*Chad R Samuelson,Timothy W McLain,Joshua G Mangelson*

Main category: cs.RO

TL;DR: 论文探讨了3D场景图（3DSGs）在户外环境中的应用，提出了一种生成任务无关的度量语义点云的方法，并改进了室内3DSG生成技术以适应户外需求。


<details>
  <summary>Details</summary>
Motivation: 传统3D场景表示（如点云和占用网格）缺乏语义组织，限制了机器人的高级推理能力。3DSGs通过整合几何、拓扑和语义关系，提供了结构化表示，但现有研究主要集中于室内环境。

Method: 提出了一种生成户外度量语义点云的方法，并改进了室内3DSG生成技术以适应户外环境。
...
### [214] [NeSyPack: A Neuro-Symbolic Framework for Bimanual Logistics Packing](https://arxiv.org/abs/2506.06567)
*Bowei Li,Peiqi Yu,Zhenran Tang,Han Zhou,Yifan Sun,Ruixuan Liu,Changliu Liu*

Main category: cs.RO

TL;DR: NeSyPack是一个结合数据驱动模型和符号推理的神经符号框架，用于双手机器人物流包装任务，具有可解释性、通用性和高效性。


<details>
  <summary>Details</summary>
Motivation: 解决传统端到端模型需要大规模重新训练的问题，同时提高系统的鲁棒性和适应性。

Method: 通过分层推理将任务分解为子任务，并使用符号技能图管理原子技能，选择参数、配置和控制策略。

Result: NeSyPack在2025年IEEE机器人自动化会议的WBCD竞赛中获得一等奖。

Conclusion: NeSyPack的模块化设计在性能和效率上优于端到端模型。
...
### [215] [Enhancing Robot Safety via MLLM-Based Semantic Interpretation of Failure Data](https://arxiv.org/abs/2506.06570)
*Aryaman Gupta,Yusuf Umut Ciftci,Somil Bansal*

Main category: cs.RO

TL;DR: 提出一种利用多模态大语言模型（MLLMs）自动聚类机器人失败数据的方法，以无监督方式从失败中学习，并提升策略改进和实时适应能力。


<details>
  <summary>Details</summary>
Motivation: 机器人系统在复杂环境中易失败，手动分析失败数据不切实际，需自动化方法从失败中提取可扩展的学习信号。

Method: 利用MLLMs从原始感知轨迹推断高级失败原因，将无标注失败日志组织为语义聚类，揭示潜在模式和失败原因。

Result: 语义聚类能指导针对性数据收集以优化策略，并用于在线失败检测，提升机器人学习和鲁棒性。

Conclusion: 该框架将现实失败转化为可操作的信号，促进机器人自适应和安全性提升。
...
### [216] [Underwater Multi-Robot Simulation and Motion Planning in Angler](https://arxiv.org/abs/2506.06612)
*Akshaya Agrawal,Evan Palmer,Zachary Kingston,Geoffrey A. Hollinger*

Main category: cs.RO

TL;DR: 扩展了Angler框架以支持多机器人水下仿真和运动规划，提供模块化架构、碰撞避免和环境生成工具。


<details>
  <summary>Details</summary>
Motivation: 水下多机器人系统部署成本高且耗时，仿真框架需接近现实以支持算法和软件开发。

Method: 扩展Angler框架，创建非冲突通信通道，集成Gazebo、ArduSub SITL和MAVROS，支持多机器人同时仿真，并引入OMPL和碰撞避免模块。

Result: 实现了多机器人水下仿真和运动规划，支持动态环境中的开发和基准测试。

Conclusion: 该扩展为水下多机器人系统的开发和测试提供了高效、接近现实的仿真工具。
...
### [217] [SMaRCSim: Maritime Robotics Simulation Modules](https://arxiv.org/abs/2506.07781)
*Mart Kartašev,David Dörner,Özer Özkahraman,Petter Ögren,Ivan Stenius,John Folkesson*

Main category: cs.RO

TL;DR: SMaRCSim是一套仿真工具包，旨在解决水下机器人开发中的快速测试和团队协作问题。


<details>
  <summary>Details</summary>
Motivation: 现有仿真工具缺乏对学习型方法、多类型机器人团队协作及任务规划的支持，限制了水下机器人功能的开发。

Method: 开发了SMaRCSim仿真工具包，支持学习型方法、多类型机器人协作及任务规划集成。

Result: SMaRCSim为水下机器人开发提供了快速测试和团队协作的解决方案。

Conclusion: SMaRCSim填补了现有仿真工具的不足，为水下机器人功能开发提供了高效支持。
...
### [218] [Attention-Based Convolutional Neural Network Model for Human Lower Limb Activity Recognition using sEMG](https://arxiv.org/abs/2506.06624)
...
Method: 使用多通道sEMG数据，设计轻量级注意力DNN（62,876参数），采用留一法验证。

Result: 验证集准确率86.74%，测试集85.38%，优于现有模型。

Conclusion: 该模型适合实时部署，有望用于人机交互系统。
...
### [219] [Active Test-time Vision-Language Navigation](https://arxiv.org/abs/2506.06630)
*Heeju Ko,Sungjune Kim,Gyeongrok Oh,Jeongyoon Yoon,Honglak Lee,Sujin Jang,Seungryong Kim,Sangpil Kim*

Main category: cs.RO

TL;DR: ATENA是一个测试时主动学习框架，通过人类-机器人交互优化导航策略，提升不确定性的校准能力。


<details>
  <summary>Details</summary>
Motivation: 离线训练的视觉语言导航策略在陌生环境中性能下降，熵最小化方法易因错误累积导致过度自信。

Method: 提出混合熵优化和自主动学习策略，结合动作和伪专家分布优化不确定性校准。

Result: 在多个VLN基准测试中表现优异，克服了测试时的分布偏移问题。

Conclusion: ATENA通过主动学习和不确定性优化，显著提升了导航策略的适应性和性能。
...
### [220] [Self-Adapting Improvement Loops for Robotic Learning](https://arxiv.org/abs/2506.06658)
*Calvin Luo,Zilai Zeng,Mingxi Jia,Yilun Du,Chen Sun*

Main category: cs.RO

TL;DR: 论文提出了一种名为SAIL的自适应改进循环方法，通过结合互联网规模的预训练视频模型和自收集的行为数据，持续提升视频生成模型在新任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 解决视频生成模型在新任务中泛化能力不足的问题，同时探索如何通过在线自收集数据持续改进模型性能。

Method: 提出SAIL方法，结合互联网规模的预训练视频模型和自收集的行为数据，迭代更新模型性能。

Result: 在MetaWorld任务和真实机器人任务中，SAIL持续提升性能，且对数据过滤和初始演示质量表现出鲁棒性。

Conclusion: SAIL通过结合互联网规模数据和在线学习，为机器人任务提供了一种高效的自改进视频模型方法。
...
### [221] [DriveSuprim: Towards Precise Trajectory Selection for End-to-End Planning](https://arxiv.org/abs/2506.06659)
...
<details>
  <summary>Details</summary>
Motivation: 解决基于回归的单一路径预测方法无法显式评估轨迹安全性的问题，以及基于选择的方法在优化和区分安全关键差异上的挑战。

Method: 提出粗到细的候选轨迹筛选、旋转增强方法和自蒸馏框架。

Result: 在NAVSIM v1和v2上分别达到93.5% PDMS和87.1% EPDMS，表现出卓越的安全关键能力和轨迹质量。

Conclusion: DriveSuprim在复杂驾驶场景中实现了先进的安全性和性能。
...
### [222] [Generalized Trajectory Scoring for End-to-end Multimodal Planning](https://arxiv.org/abs/2506.06664)
*Zhenxin Li,Wenhao Yao,Zi Wang,Xinglong Sun,Joshua Chen,Nadine Chang,Maying Shen,Zuxuan Wu,Shiyi Lan,Jose M. Alvarez*

Main category: cs.RO

TL;DR: GTRS提出了一种统一的端到端多模态规划框架，结合粗粒度和细粒度轨迹评估，解决了现有轨迹评分方法的泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹评分方法在泛化能力上存在局限，静态词汇表难以进行细粒度适应，动态生成方法无法捕捉更广泛的轨迹分布。
...
### [223] [RoboCerebra: A Large-scale Benchmark for Long-horizon Robotic Manipulation Evaluation](https://arxiv.org/abs/2506.06677)
*Songhao Han,Boxiang Qiu,Yue Liao,Siyuan Huang,Chen Gao,Shuicheng Yan,Si Liu*

Main category: cs.RO

TL;DR: RoboCerebra是一个新基准，用于评估机器人操作中的高级推理能力，结合了VLM规划器和VLA控制器，并提供了长时程任务的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注反应性策略，未充分利用VLM的语义推理和长时程规划能力，因此需要新的基准来探索这些能力。

Method: 提出RoboCerebra基准，包括大规模仿真数据集、分层框架（VLM规划器+VLA控制器）和结构化评估协议。数据集通过GPT生成任务指令并由人类执行。

Result: RoboCerebra具有更长的动作序列和更密集的标注，优于现有基准，并展示了VLM在System 2模块中的潜力。

Conclusion: RoboCerebra推动了更通用和高效的机器人规划器的发展，填补了长时程推理评估的空白。
...
### [224] [RoboPARA: Dual-Arm Robot Planning with Parallel Allocation and Recomposition Across Tasks](https://arxiv.org/abs/2506.06683)
*Shiying Duan,Pei Ren,Nanxiang Jiang,Zhengping Che,Jian Tang,Yifan Sun,Zhaoxin Fan,Wenjun Wu*

Main category: cs.RO

TL;DR: RoboPARA是一个基于大语言模型的双臂机器人任务并行规划框架，通过两阶段优化任务并行性，显著提升效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在任务规划中未能充分利用双臂协作的并行性潜力，限制了效率。

Method: RoboPARA采用两阶段方法：依赖图规划候选生成和图重遍历双臂并行规划，优化任务并行性。

Result: 在X-DAPT数据集上，RoboPARA显著优于现有方法，尤其在复杂任务组合中表现优异。

Conclusion: RoboPARA为双臂机器人任务并行规划提供了高效可靠的解决方案，并发布了首个相关数据集。
...
TL;DR: SpikePingpong系统结合尖峰视觉与模仿学习，通过SONIC和IMPACT模块解决高速乒乓球机器人控制中的视觉预测和策略规划问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 高速物体控制在机器人领域仍具挑战性，乒乓球作为测试平台，需要快速拦截和精确轨迹调整，推动机器人控制技术的发展。

Method: 系统采用20 kHz尖峰相机进行高精度球追踪，结合SONIC模块补偿现实不确定性，IMPACT模块实现精确落点规划。
...
### [226] [SARAL-Bot: Autonomous Robot for Strawberry Plant Care](https://arxiv.org/abs/2506.06798)
*Arif Ahmed,Ritvik Agarwal,Gaurav Srikar,Nathaniel Rose,Parikshit Maini*

Main category: cs.RO

TL;DR: 团队SARAL开发了一款自主机器人，用于草莓种植中的导航、病叶检测和移除，以解决劳动力短缺问题并支持可持续农业。


<details>
  <summary>Details</summary>
Motivation: 草莓种植需要大量劳动力监测和维护植物健康，劳动力短缺和高成本是主要问题。

Method: 开发了一款具备导航、视觉检测和病叶移除功能的自主机器人。

Result: 系统展示了机器人技术在草莓种植现代化和智能农业中的潜力。

Conclusion: 该机器人技术有望推动草莓种植的规模化和智能化发展。
...
### [227] [IRS: Instance-Level 3D Scene Graphs via Room Prior Guided LiDAR-Camera Fusion](https://arxiv.org/abs/2506.06804)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在开放环境中的适应性有限，视觉基础模型（VFMs）的出现为开放词汇识别和自然语言查询提供了可能，推动了3D场景图构建的发展。

Method: 通过LiDAR的广视角和远距离感知能力获取几何先验，结合多级VFMs提升语义提取精度，利用房间级分割实现并行处理，融合几何与语义线索增强鲁棒性。

Result: 相比现有方法，构建速度提升了一个数量级，同时保持了高语义精度。

Conclusion: 实验验证了方法的有效性，并在语言引导的语义导航任务中展示了其实际应用潜力。
...
### [228] [RF-Source Seeking with Obstacle Avoidance using Real-time Modified Artificial Potential Fields in Unknown Environments](https://arxiv.org/abs/2506.06811)
*Shahid Mohammad Mulla,Aryan Kanakapudi,Lakshmi Narasimhan,Anuj Tiwari*

Main category: cs.RO

TL;DR: 提出了一种结合RF源寻向和自适应人工势场的无人机实时轨迹规划方法，显著提高了目标到达成功率和轨迹效率。


<details>
  <summary>Details</summary>
Motivation: 现有障碍物避障算法（如APF）无法适应不同障碍物配置的环境，且目标位置可能不精确，需结合RF源寻向技术。

Method: 采用基于采样的方法实时调整APF参数，结合RF信号计算目标方位角，实现自适应避障。

Result: RF寻向算法平均角度误差仅1.48度，导航算法使目标到达成功率提升46%，轨迹长度减少1.2%。

Conclusion: 该方法在未知环境中表现出高效性和适应性，适用于灾难响应和基础设施监测。
...
### [229] [Multimodal Spatial Language Maps for Robot Navigation and Manipulation](https://arxiv.org/abs/2506.06862)
*Chenguang Huang,Oier Mees,Andy Zeng,Wolfram Burgard*

Main category: cs.RO

TL;DR: 提出了一种多模态空间语言地图（VLMaps和AVLMaps），通过融合预训练多模态特征与3D环境重建，实现自然语言命令到空间目标的翻译，并支持多模态目标导航。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用环境地图的几何精度和多模态信息，限制了导航代理的语言理解能力。

Method: 构建视觉-语言地图（VLMaps）和音频-视觉-语言地图（AVLMaps），结合预训练多模态特征与3D环境重建，并通过大语言模型（LLMs）实现自然语言命令的翻译。

Result: 实验表明，该方法在仿真和真实环境中实现了零样本空间和多模态目标导航，并在模糊场景中召回率提高了50%。

Conclusion: 多模态空间语言地图为机器人导航和交互提供了视觉、音频和空间线索的统一支持，显著提升了目标定位和导航能力。
...
### [230] [Hierarchical Intention Tracking with Switching Trees for Real-Time Adaptation to Dynamic Human Intentions during Collaboration](https://arxiv.org/abs/2506.07004)
*Zhe Huang,Ye-Ji Mun,Fatemeh Cheraghi Pouria,Katherine Driggs-Campbell*

Main category: cs.RO

TL;DR: 提出了一种分层意图跟踪（HIT）算法，用于协作机器人实时跟踪动态和分层的人类意图，显著提升了协作效率和用户体验。


<details>
  <summary>Details</summary>
Motivation: 人类在协作任务中的意图是多层次且动态变化的，协作机器人需要准确跟踪这些意图以适应变化并纠正错误估计。

Method: HIT算法通过贝叶斯滤波、向上测量传播和向下后验传播，以任意深度的意图树表示人类意图，并动态切换交互任务和验证任务树。

Result: 实验表明，HIT系统在效率、物理负荷和用户舒适度之间取得平衡，同时增强了用户信任并减少了任务中断。

Conclusion: HIT算法有效提升了协作机器人对多层次人类意图的理解能力，为协作任务提供了更优的解决方案。
...
### [231] [CARoL: Context-aware Adaptation for Robot Learning](https://arxiv.org/abs/2506.07006)
*Zechen Hu,Tong Xu,Xuesu Xiao,Xuan Wang*

Main category: cs.RO

TL;DR: CARoL框架通过上下文感知和知识适配，提升机器人学习新任务的效率，适用于多种RL算法，并在仿真和实际实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 利用先验知识提升机器人学习效率，但需解决知识相关性和适应性整合的挑战。

Method: 提出CARoL框架，通过分析状态转移识别任务相似性，并适配先验知识。

Result: 在仿真和实际实验中，CARoL表现出更快收敛和更高奖励，并能将仿真策略快速迁移到现实任务。

Conclusion: CARoL能高效学习新任务，具有广泛适用性和实际应用潜力。
...
### [232] [Prime the search: Using large language models for guiding geometric task and motion planning by warm-starting tree search](https://arxiv.org/abs/2506.07062)
*Dongryung Lee,Sejune Joo,Kimin Lee,Beomjoon Kim*

Main category: cs.RO

TL;DR: 论文提出了一种利用大型语言模型（LLM）结合蒙特卡洛树搜索（MCTS）解决几何任务与运动规划（G-TAMP）问题的方法，通过LLM的常识知识引导任务规划，并在MCTS中优化搜索效率。


<details>
  <summary>Details</summary>
Motivation: 传统G-TAMP方法依赖领域无关启发式或学习经验，计算资源或数据需求高。人类则利用常识直觉解决问题，因此作者尝试利用LLM的常识知识改进任务规划。

Method: 设计基于谓词的提示，将几何信息编码为LLM输入，生成任务计划；结合MCTS在混合动作空间中搜索可行解，利用LLM预热搜索过程。

Result: 在六个G-TAMP问题上，该方法优于之前的LLM规划器和纯搜索算法。

Conclusion: LLM与MCTS结合的方法在G-TAMP问题中表现出色，既利用了LLM的常识知识，又通过MCTS优化了搜索效率。
...
### [233] [Robotic Policy Learning via Human-assisted Action Preference Optimization](https://arxiv.org/abs/2506.07127)
*Wenke xia,Yichu Yang,Hongtao Wu,Xiao Ma,Tao Kong,Di Hu*

Main category: cs.RO

TL;DR: 论文提出了一种名为HAPO的人辅助动作偏好优化方法，旨在解决VLA模型依赖专家演示的问题，通过人类干预优化动作偏好，提升模型从失败中学习的能力。


<details>
  <summary>Details</summary>
Motivation: VLA模型依赖专家演示，限制了其从失败中学习和纠正的能力，因此需要一种方法优化动作偏好以提升适应性。

Method: 提出HAPO方法，结合人机协作框架收集干预轨迹，并通过自适应重加权算法优化动作偏好，解决不可逆交互和概率不匹配问题。

Result: 实验证明HAPO在模拟和现实场景中具有优越的泛化性和鲁棒性，适用于多种操作任务。

Conclusion: HAPO通过人类干预和偏好优化，显著提升了VLA模型的可靠性和从失败中学习的能力。
...
### [234] [Improving Traffic Signal Data Quality for the Waymo Open Motion Dataset](https://arxiv.org/abs/2506.07150)
*Xintao Yan,Erdao Liang,Jiawei Wang,Haojie Zhu,Henry X. Liu*

Main category: cs.RO

TL;DR: 本文提出了一种自动化的方法，利用车辆轨迹数据和交通领域知识，修复Waymo Open Motion Dataset中缺失或不准确的交通信号状态，显著提升了数据集质量。
...
Method: 结合车辆轨迹数据和交通领域知识，开发了一种鲁棒且灵活的方法，用于补全和修正交通信号状态。

Result: 在360,000多个涉及交通信号的场景中，成功补全了71.7%的缺失或未知信号状态，并将红灯违规率从15.7%降至2.9%。

Conclusion: 该方法显著提升了自动驾驶数据集的质量，为AI和自动驾驶研究提供了更可靠的数据支持，并开源了代码和改进后的数据。
...
### [235] [MorphoCopter: Design, Modeling, and Control of a New Transformable Quad-Bi Copter](https://arxiv.org/abs/2506.07204)
*Harsh Modi,Hao Su,Xiao Liang,Minghui Zheng*

Main category: cs.RO

TL;DR: 论文介绍了一种新型变形四旋翼无人机MorphoCopter，通过单一旋转关节实现快速变形为超窄形态，解决了传统四旋翼硬件配置固定的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统四旋翼无人机硬件配置固定，限制了其在某些环境中的适应性。MorphoCopter通过动态变形设计填补了这一空白。

Method: 采用单一旋转关节设计，支持从X形态快速折叠为堆叠双旋翼形态，并开发了适应不同形态的自适应控制系统。

Result: 原型机宽度可从447毫米缩减至138毫米（减少近70%），并通过仿真和飞行实验验证了其性能。

Conclusion: MorphoCopter通过创新的变形设计和控制系统，显著提升了四旋翼无人机的环境适应能力。
...
### [236] [Machine Learning-Based Self-Localization Using Internal Sensors for Automating Bulldozers](https://arxiv.org/abs/2506.07271)
...
Method: 使用机器学习模型从内部传感器估计局部速度，并将其输入扩展卡尔曼滤波器进行全局定位。

Result: 实验表明，该方法比基于运动学的方法更能抑制位置误差的累积，尤其是在打滑情况下。推土机专用传感器（如铲刀位置传感器和液压压力传感器）有助于提高定位精度。

Conclusion: 提出的机器学习方法在推土机自定位中表现优越，尤其在信号丢失或打滑情况下具有显著优势。
...
### [237] [Model Analysis And Design Of Ellipse Based Segmented Varying Curved Foot For Biped Robot Walking](https://arxiv.org/abs/2506.07283)
*Boyang Chen,Xizhe Zang,Chao Song,Yue Zhang,Jie Zhao*

Main category: cs.RO

TL;DR: 本文提出了一种基于椭圆的分段变曲率（ESVC）足部设计，用于双足机器人，旨在提高步态能量效率并保持控制器的解析可操作性。通过实验验证，ESVC足部在多种行走任务中显著降低了能耗。


<details>
  <summary>Details</summary>
Motivation: 受人类足部分段曲率滚动形状的启发，设计ESVC足部以提升双足机器人的能量效率，同时保持足部位置控制的解析可操作性。

Method: 1. 推导ESVC足部的完整解析接触模型；2. 使用非线性规划优化椭圆参数；3. 引入误差补偿方法；4. 将ESVC足部与基于混合线性倒立摆模型的行走控制器集成。

Result: 实验表明，ESVC足部在标记时间、矢状面和横向行走任务中能耗显著降低，横向行走中能耗最高减少18.52%。

Conclusion: ESVC足部为双足机器人提供了一种实用且节能的解决方案，并为未来数据驱动的足部形状优化奠定了基础。
...
### [238] [Very Large-scale Multi-Robot Task Allocation in Challenging Environments via Robot Redistribution](https://arxiv.org/abs/2506.07293)
*Seabin Lee,Joonyeol Sim,Changjoo Nam*

Main category: cs.RO

TL;DR: 提出了一种考虑机器人路径的多机器人任务分配方法，通过避免碰撞和死锁来最小化任务完成时间。


<details>
  <summary>Details</summary>
Motivation: 在密集障碍和狭窄通道的环境中，传统方法因机器人冲突和路径规划不足而效率低下。

Method: 使用广义Voronoi图构建路线图，并通过分区和推-拉机制重新分配机器人以减少冲突。
...
### [239] [BR-MPPI: Barrier Rate guided MPPI for Enforcing Multiple Inequality Constraints with Learned Signed Distance Field](https://arxiv.org/abs/2506.07325)
*Hardik Parwana,Taekyung Kim,Kehan Long,Bardh Hoxha,Hideki Okamoto,Georgios Fainekos,Dimitra Panagou*

Main category: cs.RO

TL;DR: 将MPPI控制器与CBF结合，通过CBF条件指导MPPI的采样过程，提出一种增强安全性和采样效率的算法。


<details>
  <summary>Details</summary>
Motivation: 解决MPPI在无约束最优控制问题中的局限性，同时利用CBF的严格不等式约束能力，实现更安全的控制。

Method: 将CBF条件转化为等式约束，通过参数化线性类K函数和状态增强系统设计控制输入，结合状态变换和控制投影操作。

Result: 仿真和四旋翼实验表明，算法在采样效率和接近安全边界的能力上优于传统MPPI。

Conclusion: 提出的方法有效整合了MPPI和CBF，提升了控制的安全性和性能。
...
### [240] [Real-Time Execution of Action Chunking Flow Policies](https://arxiv.org/abs/2506.07339)
*Kevin Black,Manuel Y. Galliker,Sergey Levine*

Main category: cs.RO

TL;DR: 论文提出了一种实时分块（RTC）算法，用于解决AI系统在高频控制任务中的延迟问题，通过异步执行动作分块策略实现平滑操作。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统在实时交互中存在高延迟问题，现有动作分块方法无法完全解决延迟导致的动作不连贯问题。

Method: 提出RTC算法，适用于任何基于扩散或流的视觉语言动作模型，无需重新训练，通过异步生成和执行动作分块。

Result: 在Kinetix模拟器和真实世界双手机器人任务中测试，RTC显著提高了任务吞吐量和成功率，尤其在存在延迟的情况下表现优异。

Conclusion: RTC是一种高效、鲁棒的实时动作分块方法，显著提升了AI系统在动态任务中的性能。
...
### [241] [Reproducibility in the Control of Autonomous Mobility-on-Demand Systems](https://arxiv.org/abs/2506.07345)
...
TL;DR: 本文研究了自动驾驶按需出行（AMoD）研究中可重复性的问题，提出了评估和改进可重复性的框架和具体指南。
...
Method: 通过系统分析AMoD研究流程中的关键组件（如系统建模、控制问题、仿真设计等），识别不可重复性的常见来源，并提出结构化框架和检查表。

Result: 提出了具体的可重复性指南和检查表，支持未来研究实现可复制、可比较和可扩展的结果。

Conclusion: 本文为AMoD及其他依赖网络化自主性和数据驱动控制的网络物理系统奠定了更透明和可重复的研究文化基础。
...
### [242] [UruBots Autonomous Cars Challenge Pro Team Description Paper for FIRA 2025](https://arxiv.org/abs/2506.07348)
...
TL;DR: UruBots团队开发了一款用于2025 FIRA自动驾驶汽车挑战赛的自主小车，结合机械电子组件和深度学习算法，成功完成赛道任务。


<details>
  <summary>Details</summary>
Motivation: 参与2025 FIRA自动驾驶汽车挑战赛，开发一款小型电动自动驾驶车辆，实现基于视觉输入的实时导航。

Method: 设计机械电子组件，使用深度学习模型（CNN）处理摄像头图像，控制车辆的转向和油门。训练数据集包含上万张图像。

Result: 车辆在30秒内完成赛道，速度约0.4米/秒，并能避开障碍物。

Conclusion: 项目成功展示了小型自动驾驶车辆在复杂环境中的实时导航能力，验证了深度学习算法的有效性。
...
TL;DR: MapBERT是一个新框架，利用BitVAE和掩码变换器高效建模未观察区域的语义分布，提升空间感知能力。
...
Method: 结合BitVAE编码语义图为紧凑比特令牌，使用掩码变换器推断缺失区域，并提出对象感知掩码策略增强推理。
...
### [244] [Language-Grounded Hierarchical Planning and Execution with Multi-Robot 3D Scene Graphs](https://arxiv.org/abs/2506.07454)
...
TL;DR: 多机器人系统结合3D场景图，实现自然语言指令下的复杂任务执行。
...
Method: 构建共享3D场景图，结合开放集对象地图，利用大型语言模型（LLM）将指令转化为PDDL目标。

Result: 系统在大型户外环境中成功执行复杂任务。

Conclusion: 3D场景图和LLM的结合为多机器人系统提供了高效的任务执行能力。
...
### [245] [RAPID Hand: A Robust, Affordable, Perception-Integrated, Dexterous Manipulation Platform for Generalist Robot Autonomy](https://arxiv.org/abs/2506.07490)
*Zhaoliang Wan,Zetong Bi,Zida Zhou,Hao Ren,Yiming Zeng,Yihan Li,Lu Qi,Xu Yang,Ming-Hsuan Yang,Hui Cheng*

Main category: cs.RO

TL;DR: 本文提出了一种低成本、高灵活性的机器人手平台RAPID Hand，用于收集多指机器人操作数据，支持通用机器人自主性。该平台通过硬件和软件协同优化，实现了紧凑设计、高精度感知和高效远程操作。


<details>
  <summary>Details</summary>
Motivation: 解决低成本、高灵活性机器人手平台稀缺的问题，以支持通用机器人自主性的数据收集需求。

Method: 提出RAPID Hand平台，结合紧凑的20自由度手设计、全手感知系统和高效远程操作接口，通过协同优化硬件与软件实现。

Result: 实验验证了平台的硬件、感知和远程操作性能，基于收集数据训练的扩散策略表现优于现有方法。

Conclusion: RAPID Hand平台低成本、高性能，适合高质量数据收集，并将公开以促进可重复性和广泛应用。
...
### [246] [Taking Flight with Dialogue: Enabling Natural Language Control for PX4-based Drone Agent](https://arxiv.org/abs/2506.07509)
*Shoon Kit Lim,Melissa Jia Ying Chong,Jing Huey Khor,Ting Yang Ling*

Main category: cs.RO

TL;DR: 论文提出了一种开源框架，用于实现无人机的自然语言控制，结合了PX4飞行控制、ROS 2中间件和本地托管模型。


<details>
  <summary>Details</summary>
Motivation: 当前无人机多模态视觉语言系统依赖闭源模型，限制了资源不足的组织使用，作者希望通过开源框架实现无人机控制的民主化。

Method: 框架整合了PX4飞行控制、ROS 2中间件和本地托管模型（Ollama），并在仿真和自定义四旋翼平台上评估了四种LLM和三种VLM的性能。

Result: 通过实验评估了不同LLM和VLM在命令生成和场景理解方面的表现。

Conclusion: 开源框架为无人机自然语言控制提供了可行方案，推动了该领域的民主化发展。
...
### [247] [BitVLA: 1-bit Vision-Language-Action Models for Robotics Manipulation](https://arxiv.org/abs/2506.07530)
*Hongyu Wang,Chuyan Xiong,Ruiping Wang,Xilin Chen*

Main category: cs.RO

TL;DR: BitVLA是首个1位三元参数（-1,0,1）的视觉-语言-动作（VLA）模型，通过蒸馏感知训练策略压缩视觉编码器至1.58位，内存占用仅为29.8%，性能接近4位量化模型。
...
Result: 在LIBERO基准测试中，性能接近4位量化模型OpenVLA-OFT，内存占用仅为29.8%。
...
### [248] [Fractional Collisions: A Framework for Risk Estimation of Counterfactual Conflicts using Autonomous Driving Behavior Simulations](https://arxiv.org/abs/2506.07540)
*Sreeja Roy-Singh,Sarvesh Kolekar,Daniel P. Bonny,Kyle Foss*

Main category: cs.RO

TL;DR: 提出了一种基于传感器数据估计碰撞风险的方法，通过模拟场景评估两车冲突，预测碰撞严重性，并在合成环境中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统（ADS）和自然驾驶数据中碰撞风险的量化问题，以改进安全性能。

Method: 检测和分类冲突类型，识别角色（发起者或响应者），建模行为期望为概率轨迹，计算速度差异并结合碰撞模型估计损失。

Result: 在合成环境中预测碰撞误差在1%以内，ADS将自然碰撞减少4倍，碰撞风险降低62%。

Conclusion: 该方法有效量化碰撞风险，显著提升ADS安全性，适用于大规模传感器数据分析。
...
### [249] [Blending Participatory Design and Artificial Awareness for Trustworthy Autonomous Vehicles](https://arxiv.org/abs/2506.07633)
*Ana Tanevska,Ananthapathmanabhan Ratheesh Kumar,Arabinda Ghosh,Ernesto Casablanca,Ginevra Castellano,Sadegh Soudjani*

Main category: cs.RO

TL;DR: 本文提出了一种数据驱动的人类驾驶员模型，用于增强多智能体系统中的情境感知架构，并通过大规模用户研究验证了自动驾驶车辆透明度和用户行为之间的关系。


<details>
  <summary>Details</summary>
Motivation: 当前机器人代理（如自动驾驶车辆和无人机）需要处理不确定的现实环境，同时与人类用户互动，因此需要建立信任和透明的交互模型。

Method: 通过大规模用户中心研究收集数据，研究自动驾驶车辆的透明度与用户行为的关系，并基于数据构建马尔可夫链模型。

Result: 研究发现，自动驾驶车辆的透明度、场景环境和用户人口统计特征显著影响模型的状态转移。

Conclusion: 本文的数据驱动模型为多智能体系统中的人类交互提供了理论基础，并强调了透明度和用户特征在建模中的重要性。
...
### [250] [Fast ECoT: Efficient Embodied Chain-of-Thought via Thoughts Reuse](https://arxiv.org/abs/2506.07639)
*Zhekai Duan,Yuan Zhang,Shikai Geng,Gaowen Liu,Joschka Boedecker,Chris Xiaoxuan Lu*

Main category: cs.RO

TL;DR: Fast ECoT通过缓存和并行化推理步骤，显著降低ECoT的推理延迟，无需修改模型或额外训练。


<details>
  <summary>Details</summary>
Motivation: ECoT的顺序推理导致高延迟，限制了实时部署，Fast ECoT旨在解决这一问题。

Method: 利用ECoT的结构化和重复性，缓存高层推理并并行化模块化推理步骤，同时引入异步调度器。

Result: 实验显示延迟降低7.5%，任务成功率和推理准确性保持或提升。

Conclusion: Fast ECoT使ECoT策略更接近实时部署，适用于仿真和现实任务。
...
### [251] [A Communication-Latency-Aware Co-Simulation Platform for Safety and Comfort Evaluation of Cloud-Controlled ICVs](https://arxiv.org/abs/2506.07696)
...
TL;DR: 本文提出了一种延迟感知的协同仿真平台，用于评估云控智能网联车辆在真实V2C延迟条件下的安全性和舒适性。


<details>
  <summary>Details</summary>
Motivation: 测试云控智能网联车辆需要模拟真实车辆行为和通信延迟，现有仿真环境难以满足需求。

Method: 集成CarMaker和Vissim，引入基于5G实测数据的Gamma分布延迟模型，并提出主动冲突模块（PCM）动态生成安全关键场景。

Result: 实验表明PCM能有效提升驾驶环境关键性，V2C延迟主要影响舒适性。

Conclusion: 该平台能系统评估云控智能网联车辆在不同测试条件下的表现。
...
### [252] [Primal-Dual iLQR for GPU-Accelerated Learning and Control in Legged Robots](https://arxiv.org/abs/2506.07823)
*Lorenzo Amatucci,João Sousa-Pinto,Giulio Turrisi,Dominique Orban,Victor Barasuol,Claudio Semini*

Main category: cs.RO

TL;DR: 论文提出了一种基于GPU并行化的新型MPC实现，用于腿式机器人运动控制，显著提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决传统MPC在腿式机器人运动控制中计算复杂度高的问题，通过并行化技术优化求解过程。

Method: 采用并行关联扫描求解KKT系统，实现时间和状态空间的并行化，降低计算复杂度。

Result: 相比现有求解器（acados和crocoddyl），运行时提升60%（WB-MPC）和700%（SRBD-MPC），并可支持16个机器人的集中控制。

Conclusion: 该方法高效且可扩展，支持GPU大规模并行化，为机器人运动控制和学习提供了新可能。
...
### [253] [Versatile Loco-Manipulation through Flexible Interlimb Coordination](https://arxiv.org/abs/2506.07876)
*Xinghao Zhu,Yuxin Chen,Lingfeng Sun,Farzad Niroui,Simon Le CleacH,Jiuguang Wang,Kuan Fang*

Main category: cs.RO

TL;DR: ReLIC是一种通过强化学习实现灵活肢体协调的方法，支持多功能运动与操作任务。


<details>
  <summary>Details</summary>
Motivation: 现有运动与操作研究受限于特定任务或固定肢体配置，难以适应非结构化环境。

Method: 采用自适应控制器，动态分配肢体用于操作或运动，并通过强化学习在仿真中训练。

Result: 在12项真实任务中平均成功率达78.9%，支持多种任务规范（如轨迹、接触点、自然语言）。

Conclusion: ReLIC展示了多功能性和鲁棒性，为复杂环境中的自主机器人操作提供了新思路。
...
### [254] [Design and Implementation of a Peer-to-Peer Communication, Modular and Decentral YellowCube UUV](https://arxiv.org/abs/2506.07924)
...
<details>
  <summary>Details</summary>
Motivation: 现有UUV难以集成或升级传感器，限制了其灵活性和适应性。

Method: 设计并实现模块化、去中心化的UUV YellowCube，采用P2P通信机制替代集中式架构。

Result: 通过实验室和海上试验验证了UUV的性能。

Conclusion: YellowCube的模块化和P2P设计为UUV的灵活性和适应性提供了有效解决方案。
...
### [255] [BridgeVLA: Input-Output Alignment for Efficient 3D Manipulation Learning with Vision-Language Models](https://arxiv.org/abs/2506.07961)
*Peiyan Li,Yixiang Chen,Hongtao Wu,Xiao Ma,Xiangnan Wu,Yan Huang,Liang Wang,Tao Kong,Tieniu Tan*

Main category: cs.RO

TL;DR: BridgeVLA是一种新型3D视觉-语言-动作模型，通过将3D输入投影为2D图像并利用2D热图进行动作预测，显著提高了样本效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法未充分利用3D数据的空间结构，导致样本效率低下。

Method: 将3D输入投影为2D图像，利用2D热图预测动作，并提出可扩展的预训练方法。

Result: 在多个仿真基准测试和真实机器人实验中表现优异，平均成功率显著提升，样本效率极高。

Conclusion: BridgeVLA在3D操作任务中高效且有效，具有强大的泛化能力和样本效率。
...
### [256] [Experimental Evaluation of Static Image Sub-Region-Based Search Models Using CLIP](https://arxiv.org/abs/2506.06938)
*Bastian Jäckl,Vojtěch Kloda,Daniel A. Keim,Jakub Lokoč*

Main category: cs.MM

TL;DR: 研究探讨了在多模态文本-图像模型中，通过添加基于位置的提示来增强模糊文本查询在高度同质化专业领域的检索性能。


<details>
  <summary>Details</summary>
Motivation: 在高度同质化的专业领域（如水下场景），用户通常只能提供模糊的文本描述，缺乏专家知识区分同质实体，导致检索性能不佳。

Method: 收集了741个人工标注的数据集，包含短/长文本描述和感兴趣区域的边界框，评估CLIP模型在不同静态子区域（如3x3分区和5网格重叠）的检索效果。

Result: 结果显示，简单的3x3分区和5网格重叠显著提高了检索效果，并对标注框的扰动保持鲁棒性。

Conclusion: 基于位置的提示可以有效补充模糊文本查询，提升在高度同质化专业领域的检索性能。
...
### [257] [QForce-RL: Quantized FPGA-Optimized Reinforcement Learning Compute Engine](https://arxiv.org/abs/2506.07046)
*Anushka Jha,Tanushree Dewangan,Mukul Lokhande,Santosh Kumar Vishvakarma*

Main category: cs.AR

TL;DR: QForce-RL利用量化和轻量级架构提升FPGA上的RL性能，减少资源消耗和能耗，性能提升2.3倍。


<details>
  <summary>Details</summary>
Motivation: FPGA部署RL资源消耗大，QForce-RL旨在通过量化和轻量架构解决这一问题。

Method: 结合E2HRL减少RL动作，利用QuaRL的量化SIMD硬件加速，优化模型大小和计算操作。

Result: 性能提升2.3倍，FPS提升2.6倍，适用于资源受限设备。

Conclusion: QForce-RL为资源受限设备提供了高效、灵活的部署方案。
...
### [258] [A Culturally-diverse Multilingual Multimodal Video Benchmark & Model](https://arxiv.org/abs/2506.07032)
...
TL;DR: 本文介绍了ViMUL-Bench，一个多语言视频LMM基准测试，旨在评估14种语言的视频理解能力，并提出了ViMUL模型，以平衡高低资源语言的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大多语言视频LMM（Large Multimodal Models）主要针对英语，缺乏对其他语言和文化包容性的研究。本文旨在填补这一空白。

Method: 提出ViMUL-Bench基准测试，包含14种语言的8k样本，涵盖15个文化多样性类别；同时开发了ViMUL模型，并提供了120万样本的多语言视频训练集。

Result: ViMUL模型在高低资源语言之间取得了更好的平衡，ViMUL-Bench为多语言视频LMM研究提供了标准化评估工具。

Conclusion: ViMUL-Bench和ViMUL模型为未来开发更具文化包容性的多语言视频LMM奠定了基础，相关资源将公开共享。
...
### [259] [Lingshu: A Generalist Foundation Model for Unified Multimodal Medical Understanding and Reasoning](https://arxiv.org/abs/2506.07044)
...
TL;DR: 论文提出了一种针对医学领域的多模态大语言模型Lingshu，通过数据优化和多阶段训练解决现有医学MLLM的局限性，并在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有医学MLLM在知识覆盖、幻觉问题和推理能力方面存在不足，限制了其在医学应用中的效果。

Method: 提出数据优化流程和多阶段训练策略，构建医学多模态数据集，并探索强化学习增强推理能力。

Result: Lingshu在多项医学任务中表现优于现有开源多模态模型。

Conclusion: Lingshu通过数据优化和训练策略改进，显著提升了医学MLLM的性能。
...
### [260] [Flattery in Motion: Benchmarking and Analyzing Sycophancy in Video-LLMs](https://arxiv.org/abs/2506.07180)
*Wenrui Zhou,Shu Yang,Qingsong Yang,Zikun Guo,Lijie Hu,Di Wang*

Main category: cs.CL

TL;DR: 论文提出了VISE基准，用于评估视频大语言模型（Video-LLMs）在误导性用户输入下的迎合行为，并探索了关键帧选择作为缓解策略。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在实际应用中需要可靠的多模态推理能力，但其迎合用户输入的行为（即使与视觉证据矛盾）影响了可信度。目前缺乏针对视频语言领域的系统性评估。

Method: 提出VISE基准，涵盖多种问题格式、提示偏见和视觉推理任务，首次将语言领域的迎合行为分析引入视觉领域。

Result: VISE能够细粒度分析多种迎合行为类型和交互模式，并发现关键帧选择是一种无需训练的可解释缓解策略。

Conclusion: VISE填补了视频语言领域迎合行为研究的空白，为减少模型偏见提供了潜在路径。
...
### [261] [Unblocking Fine-Grained Evaluation of Detailed Captions: An Explaining AutoRater and Critic-and-Revise Pipeline](https://arxiv.org/abs/2506.07631)
*Brian Gordon,Yonatan Bitton,Andreea Marzoca,Yasumasa Onoe,Xiao Wang,Daniel Cohen-Or,Idan Szpektor*

Main category: cs.CL

TL;DR: 论文提出了DOCCI-Critique基准和VNLI-Critique模型，用于评估和改进视觉语言模型（VLM）生成段落长度图像描述的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法难以捕捉VLM生成段落描述中的细粒度错误，缺乏验证数据集。

Method: 引入DOCCI-Critique基准（含1,400条标注）和VNLI-Critique模型，用于自动化事实分类和错误分析。

Result: VNLI-Critique在多个基准测试中表现优异，AutoRater与人类评估高度一致，Critic-and-Revise流程显著提升描述准确性。

Conclusion: 研究提供了关键基准和工具，旨在提升VLM图像理解的细粒度评估标准。
...
### [262] [Benchmarking Early Agitation Prediction in Community-Dwelling People with Dementia Using Multimodal Sensors and Machine Learning](https://arxiv.org/abs/2506.06306)
*Ali Abedi,Charlene H. Chu,Shehroz S. Khan*

Main category: eess.SP

TL;DR: 研究开发了基于多模态传感器数据的机器学习方法，用于早期预测社区居住的老年痴呆患者的激越行为，并引入新的上下文特征。最佳模型AUC-ROC达0.9720。


<details>
  <summary>Details</summary>
Motivation: 激越行为在老年痴呆患者中常见，及时预测可减轻护理负担并提高生活质量。

Method: 使用多模态传感器数据，评估多种机器学习和深度学习模型，包括二元分类和异常检测。

Result: 最佳模型（轻梯度提升机）在二元分类中表现优异，AUC-ROC为0.9720。

Conclusion: 该方法支持准确、可解释的激越行为预测，有助于主动护理和居家养老。
...
### [263] [An Open-Source Python Framework and Synthetic ECG Image Datasets for Digitization, Lead and Lead Name Detection, and Overlapping Signal Segmentation](https://arxiv.org/abs/2506.06315)
*Masoud Rahimi,Reza Karbasi,Abdol-Hossein Vahabie*

Main category: eess.SP

TL;DR: 开源Python框架生成合成ECG图像数据集，支持深度学习任务如ECG数字化、导联区域和名称检测及波形分割。


<details>
  <summary>Details</summary>
Motivation: 推动ECG分析中的深度学习任务，提供多样化的合成数据集以支持研究。

Method: 基于PTB-XL信号数据集，生成四种开放数据集，包括图像与信号配对、YOLO格式标注、单导联分割掩码等。

Result: 发布了开源框架和四种数据集，支持ECG数字化、检测及分割任务。

Conclusion: 该框架和数据集为ECG深度学习研究提供了实用工具和资源。
...
### [264] [Heart Rate Classification in ECG Signals Using Machine Learning and Deep Learning](https://arxiv.org/abs/2506.06349)
*Thien Nhan Vo,Thanh Xuan Truong*

Main category: eess.SP

TL;DR: 该研究通过传统机器学习和深度学习方法对心电图信号进行分类，发现基于手工特征的LightGBM模型表现最佳，优于基于图像的CNN方法。


<details>
  <summary>Details</summary>
Motivation: 解决心电图信号中心跳分类的问题，比较传统手工特征提取与深度学习图像转换方法的性能差异。

Method: 1. 传统机器学习：提取HRV、均值、方差等特征，训练SVM、随机森林等分类器；2. 深度学习方法：将ECG信号转换为图像（GAF、MTF、RP），使用CNN分类。

Result: LightGBM模型表现最佳（准确率99%，F1分数0.94），优于CNN方法（F1分数0.85）。

Conclusion: 手工特征能更好地捕捉ECG信号的时空变化，未来可结合多导联信号和时序依赖提升分类性能。
...
### [265] [Towards Data-Driven Model-Free Safety-Critical Control](https://arxiv.org/abs/2506.06931)
*Zhe Shen,Yitaek Kim,Christoffer Sloth*

Main category: eess.SY

TL;DR: 提出了一种基于数据驱动的无模型控制屏障函数（CBFs）框架，用于实现机器人系统的安全速度控制，通过神经网络学习Lyapunov函数并估计速度控制器的最大衰减率，结合Chernoff边界增强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决无模型CBFs中设计参数（如alpha）依赖速度控制器衰减率的问题，避免手动调参。

Method: 使用神经网络从数据中学习Lyapunov函数，估计速度控制器的最大衰减率，并推导结合Chernoff边界的概率安全条件。

Result: 在UR5e机器人上测试，验证了框架在无模型CBFs中实现安全速度控制的有效性。

Conclusion: 该框架通过数据驱动方法解决了无模型CBFs的参数依赖问题，提升了安全控制的鲁棒性。
...
### [266] [Deep Equivariant Multi-Agent Control Barrier Functions](https://arxiv.org/abs/2506.07755)
*Nikolaos Bousias,Lars Lindemann,George Pappas*

Main category: eess.SY

TL;DR: 论文提出了一种基于对称性的分布式控制屏障函数方法，解决了现有学习方法的可扩展性、泛化性和采样效率问题。


<details>
  <summary>Details</summary>
Motivation: 随着多智能体系统在复杂环境中的自主部署，确保数据驱动策略的安全性至关重要，但现有方法忽视了系统的几何结构。

Method: 引入对称性注入的分布式控制屏障函数，通过可学习的基于图的安全证书满足内在对称性，并提出了一种构建等变网络的方法。

Result: 在模拟实验中，该方法在安全性、可扩展性和任务成功率上优于现有基线。

Conclusion: 嵌入对称性对于安全分布式神经策略的重要性得到了验证。
...
### [267] [LLMs as World Models: Data-Driven and Human-Centered Pre-Event Simulation for Disaster Impact Assessment](https://arxiv.org/abs/2506.06355)
*Lingyao Li,Dawei Li,Zhenhui Ou,Xiaoran Xu,Jingxiao Liu,Zihui Ma,Runlong Yu,Min Deng*

Main category: cs.CY

TL;DR: 本研究利用大语言模型（LLMs）模拟地震影响，结合多模态数据预测地震烈度，结果显示与真实报告高度一致。


<details>
  <summary>Details</summary>
Motivation: 通过高效模拟提升对突发性灾害（如地震）的主动准备能力。

Method: 利用多模态数据集（地理空间、社会经济、建筑和街景图像数据）训练LLMs，生成地震烈度预测。

Result: 在2014年纳帕和2019年里奇克莱斯特地震中，预测结果与实际报告高度相关（0.88），RMSE为0.77。

Conclusion: LLMs在灾害影响模拟中具有潜力，可增强灾前规划能力。
...
### [268] [ResPF: Residual Poisson Flow for Efficient and Physically Consistent Sparse-View CT Reconstruction](https://arxiv.org/abs/2506.06400)
...
Method: 基于PFGM++，引入条件引导和残差融合模块，跳过冗余初始步骤并保持数据一致性。

Result: 在合成和临床数据集上，ResPF在重建质量、推理速度和鲁棒性上优于现有方法。

Conclusion: ResPF是首个将Poisson Flow模型应用于稀疏视图CT的方法，显著提升了重建性能。
...
### [269] [SPC to 3D: Novel View Synthesis from Binary SPC via I2I translation](https://arxiv.org/abs/2506.06890)
*Sumit Sharma,Gopi Raju Matta,Kaushik Mitra*

Main category: eess.IV

TL;DR: 提出了一种两阶段框架，将二进制SPC图像转换为高质量彩色新视图，显著提升了感知质量和几何一致性。


<details>
  <summary>Details</summary>
Motivation: SPAD和SPC技术虽能高效捕获光子信息，但二进制图像导致纹理和颜色信息丢失，传统3D合成技术难以处理。

Method: 第一阶段使用Pix2PixHD进行图像到图像转换，第二阶段采用NeRF或3DGS进行3D场景重建。

Result: 实验验证表明，该方法在感知质量和几何一致性上优于基线。
...
### [270] [Optimal Transport Driven Asymmetric Image-to-Image Translation for Nuclei Segmentation of Histological Images](https://arxiv.org/abs/2506.07023)
*Suman Mahapatra,Pradipta Maji*

Main category: eess.IV

TL;DR: 提出了一种新的深度生成模型，用于从组织学图像中分割细胞核，通过嵌入空间处理信息不对称问题，并利用可逆生成器降低网络复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决图像到图像翻译模型在信息不对称情况下表现不佳的问题，提高细胞核分割的准确性和效率。

Method: 引入嵌入空间处理信息不对称，结合最优传输和测度理论设计可逆生成器，无需显式循环一致性损失，并加入空间约束压缩操作。

Result: 模型在公开数据集上表现优于现有方法，实现了网络复杂度和性能的更好平衡。

Conclusion: 提出的深度生成模型在细胞核分割任务中表现出色，为信息不对称问题提供了有效解决方案。
...
### [271] [SiliCoN: Simultaneous Nuclei Segmentation and Color Normalization of Histological Images](https://arxiv.org/abs/2506.07028)
*Suman Mahapatra,Pradipta Maji*

Main category: eess.IV

TL;DR: 提出了一种新型深度生成模型，同时实现组织学图像中细胞核结构的分割和染色颜色归一化，结合截断正态分布和空间注意力机制。


<details>
  <summary>Details</summary>
Motivation: 解决组织学图像中颜色变异对细胞核分割的影响，同时提升颜色归一化的准确性。

Method: 采用深度生成模型，假设颜色外观信息与细胞核分割图及嵌入信息独立，利用截断正态分布混合作为颜色外观代码的先验，并引入空间注意力机制。

Result: 在公开标准数据集上验证了模型性能，并与现有先进算法进行了比较。

Conclusion: 该模型通过解耦表示和空间注意力机制，实现了对颜色变异鲁棒的细胞核分割和颜色归一化。
...
### [272] [Transfer Learning and Explainable AI for Brain Tumor Classification: A Study Using MRI Data from Bangladesh](https://arxiv.org/abs/2506.07228)
*Shuvashis Sarker*

Main category: eess.IV

TL;DR: 该研究开发了一种基于深度学习的自动脑肿瘤分类系统，利用MRI数据和XAI技术提高分类准确性和可解释性，适用于医疗资源有限的地区。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤的及时诊断对改善患者预后至关重要，尤其是在医疗基础设施受限的国家如孟加拉国。手动MRI分析效率低且易出错，因此需要自动化解决方案。

Method: 研究使用VGG16、VGG19和ResNet50等深度学习模型对脑肿瘤进行分类，并结合Grad-CAM和Grad-CAM++等XAI技术提升模型可解释性。

Result: VGG16模型表现最佳，准确率达99.17%。XAI技术增强了系统的透明度和稳定性。

Conclusion: 深度学习与XAI结合可有效提升脑肿瘤检测能力，尤其适用于医疗资源匮乏地区。
...
### [273] [A Comprehensive Analysis of COVID-19 Detection Using Bangladeshi Data and Explainable AI](https://arxiv.org/abs/2506.07234)
*Shuvashis Sarker*

Main category: eess.IV

TL;DR: 研究利用深度学习模型VGG19在CXR图像中检测COVID-19，准确率达98%，并采用LIME和SMOTE提升模型透明度和数据平衡。


<details>
  <summary>Details</summary>
Motivation: COVID-19全球大流行对公共卫生和经济造成巨大冲击，孟加拉国病例和死亡人数显著，亟需高效检测方法。

Method: 使用4,350张CXR图像数据集，分为四类，应用ML、DL和TL模型，VGG19为主模型，结合LIME解释预测，SMOTE解决数据不平衡。

Result: VGG19模型达到98%准确率，LIME揭示了影响分类的关键区域，SMOTE有效平衡了数据。

Conclusion: 研究强调了可解释AI在提升模型透明度和可靠性中的重要性，为CXR图像检测提供了高效解决方案。
...
### [274] [A Narrative Review on Large AI Models in Lung Cancer Screening, Diagnosis, and Treatment Planning](https://arxiv.org/abs/2506.07236)
*Jiachen Zhong,Yiting Wang,Di Zhu,Ziwei Wang*

Main category: eess.IV

TL;DR: 本文综述了大型AI模型在肺癌筛查、诊断、预后和治疗中的应用，分类并评估了现有模型，探讨了其临床潜力与局限性。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球高发且致命的疾病，亟需精准及时的诊断和治疗。大型AI模型的进步为医学影像理解和临床决策提供了新工具。

Method: 系统综述了现有大型AI模型（如CLIP、BLIP等），分类为模态特定编码器、编码器-解码器框架和联合编码架构，并评估了其在多模态学习任务中的表现。

Result: 这些模型在肺结节检测、基因突变预测、多组学整合和个性化治疗规划中表现出色，部分已进入临床验证阶段。

Conclusion: 大型AI模型在肺癌诊疗中具有变革潜力，但仍需解决泛化性、可解释性和合规性问题，未来应发展可扩展、可解释且临床集成的AI系统。
...
### [275] [Text-guided multi-stage cross-perception network for medical image segmentation](https://arxiv.org/abs/2506.07475)
*Gaoyu Chen*

Main category: eess.IV

TL;DR: 论文提出了一种文本引导的多阶段交叉感知网络（TMC），用于解决医学图像分割中目标区域语义表达弱的问题，通过多阶段交叉注意力模块和多阶段对齐损失提升性能。
...
### [276] [Fine-Grained Motion Compression and Selective Temporal Fusion for Neural B-Frame Video Coding](https://arxiv.org/abs/2506.07709)
*Xihua Sheng,Peilin Chen,Meng Wang,Li Zhang,Shiqi Wang,Dapeng Oliver Wu*

Main category: eess.IV

TL;DR: 论文提出了一种改进的神经B帧编码方法，通过细粒度运动压缩和选择性时间融合提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有神经B帧编码工具直接沿用P帧方法，未解决B帧独特挑战，导致性能不佳。

Method: 设计细粒度运动压缩方法（交互式双分支运动自编码器）和选择性时间融合方法（预测双向融合权重）。

Result: 实验表明，该方法优于现有神经B帧编码器，性能接近H.266/VVC参考软件。

Conclusion: 提出的方法有效解决了B帧编码的独特问题，显著提升了压缩性能。
...
### [277] [HotelMatch-LLM: Joint Multi-Task Training of Small and Large Language Models for Efficient Multimodal Hotel Retrieval](https://arxiv.org/abs/2506.07296)
*Arian Askari,Emmanouil Stergiadis,Ilya Gusev,Moran Beladev*

Main category: cs.IR

TL;DR: HotelMatch-LLM是一种多模态密集检索模型，用于旅游领域的自然语言搜索，解决了传统搜索引擎的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统旅游搜索引擎需要用户从目的地开始并编辑搜索参数，限制了用户体验。HotelMatch-LLM旨在通过自然语言搜索提升效率。

Method: 1. 多任务优化；2. 非对称密集检索架构；3. 图像处理技术。

Result: 在四个测试集上，HotelMatch-LLM显著优于现有模型（如VISTA和MARVEL），主查询类型得分0.681（MARVEL为0.603）。
...
### [278] [Curriculum Learning With Counterfactual Group Relative Policy Advantage For Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2506.07548)
...
<details>
  <summary>Details</summary>
Motivation: 现有MARL方法通常在固定对手策略下训练，限制了适应性和策略优化，需解决动态环境中的不稳定性和稀疏奖励问题。

Method: 结合动态CL框架和CGRPA机制，自适应调整对手强度，并通过反事实优势函数分配个体贡献。

Result: 实验表明，该方法提升了训练稳定性和最终性能，优于现有方法。

Conclusion: 动态CL与CGRPA结合有效解决了MARL中的适应性和稳定性问题，具有竞争力。
...
### [279] [Contextual Experience Replay for Self-Improvement of Language Agents](https://arxiv.org/abs/2506.06698)
*Yitao Liu,Chenglei Si,Karthik Narasimhan,Shunyu Yao*

Main category: cs.AI

TL;DR: 论文提出了Contextual Experience Replay (CER)框架，通过动态记忆缓冲区积累和合成过去的经验，使语言模型代理能够在复杂任务中自我改进，提升适应性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型代理在复杂任务（如网页导航）中缺乏环境特定经验，且无法在推理时持续学习，限制了其表现。
...
Conclusion: CER有效提升了语言模型代理的适应性和性能，证明了其在复杂任务中的潜力。
...
### [280] [VisioMath: Benchmarking Figure-based Mathematical Reasoning in LMMs](https://arxiv.org/abs/2506.06727)
*Can Li,Ting Zhang,Mei Wang,Hua Huang*

Main category: cs.AI

TL;DR: VisioMath是一个评估多模态数学推理能力的基准数据集，专注于图像选项的数学问题，现有大模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有大模型在多模态数学推理中，尤其是图像选项场景下的能力尚未充分探索，VisioMath填补了这一空白。

Method: 提出VisioMath数据集，包含8,070张图像和1,800道多选题，答案选项为图像，评估现有大模型的性能。

Result: 即使是GPT-4o等先进模型，准确率仅为45.9%，显示模型在视觉相似选项推理上的局限性。

Conclusion: VisioMath为未来多模态推理研究提供了严格测试基准，推动领域进步。
...
### [281] [Meta-Adaptive Prompt Distillation for Few-Shot Visual Question Answering](https://arxiv.org/abs/2506.06905)
*Akash Gupta,Amos Storkey,Mirella Lapata*

Main category: cs.AI

TL;DR: 本文提出一种元学习方法，通过任务相关的软提示和注意力映射模块，提升小规模多模态模型（LMMs）在少样本学习中的表现，优于传统上下文学习（ICL）和提示调优方法。


<details>
  <summary>Details</summary>
Motivation: 现有LMMs依赖上下文学习（ICL）时性能不稳定，尤其是小模型，可能因图像嵌入信息过多而影响任务表现。

Method: 提出元学习方法，通过任务相关的软提示和注意力映射模块（集成于LLaVA v1.5架构），在少样本场景下实现任务适应。

Result: 在VL-ICL Bench上验证，该方法优于ICL和提示调优，对图像扰动鲁棒，提升了视觉问答任务的表现。

Conclusion: 该方法为LMMs在低数据场景下的任务适应提供了有效解决方案，性能显著提升。
...
### [282] [Long-Tailed Learning for Generalized Category Discovery](https://arxiv.org/abs/2506.06965)
*Cuong Manh Hoang*

Main category: cs.AI

TL;DR: 论文提出了一种新框架，用于在长尾分布中实现广义类别发现（GCD），通过自引导标记和表示平衡技术提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法在平衡数据集上表现良好，但在现实世界中的不平衡数据集上效果不佳，因此需要改进。

Method: 提出自引导标记技术和表示平衡过程，前者生成伪标签以减少偏差，后者通过挖掘样本邻域提升尾部类别的关注度。

Result: 在公开数据集上的实验表明，该框架优于现有方法。

Conclusion: 新框架有效解决了GCD在长尾分布中的问题，性能优于现有技术。
...
### [283] [Mitigating Behavioral Hallucination in Multimodal Large Language Models for Sequential Images](https://arxiv.org/abs/2506.07184)
*Liangliang You,Junchi Yao,Shu Yang,Guimin Hu,Lijie Hu,Di Wang*

Main category: cs.AI

TL;DR: 多模态大语言模型存在行为幻觉问题，本文提出SHE框架和BEACH指标来检测和缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 行为幻觉在多模态模型中研究较少，限制了其可靠性和扩展性。本文旨在填补这一空白。

Method: 提出SHE框架，通过自适应时间窗口检测幻觉，并通过正交投影缓解幻觉。同时提出BEACH指标量化行为幻觉。

Result: SHE在标准基准测试中将行为幻觉减少10%以上，同时保持描述准确性。

Conclusion: SHE框架和BEACH指标有效解决了行为幻觉问题，提升了多模态模型的可靠性。
...
### [284] [Reinforcing Multimodal Understanding and Generation with Dual Self-rewards](https://arxiv.org/abs/2506.07963)
...
Method: 基于理解与生成是对偶任务的观察，提出自监督双奖励机制，通过反转输入-输出来计算对偶似然作为自我奖励。
...
### [285] [GUI-Reflection: Empowering Multimodal GUI Models with Self-Reflection Behavior](https://arxiv.org/abs/2506.08012)
*Penghao Wu,Shengnan Ma,Bo Wang,Jiaheng Yu,Lewei Lu,Ziwei Liu*

Main category: cs.AI

TL;DR: GUI-Reflection框架通过自反思和错误纠正能力增强多模态GUI模型，实现更鲁棒的GUI自动化。
...
Method: 提出GUI-Reflection框架，包括GUI特定预训练、离线监督微调和在线反思调优三个阶段，自动生成反思数据并学习错误纠正能力。

Result: 框架成功赋予GUI代理自反思和纠正能力，提升了GUI自动化的鲁棒性和适应性。
...
### [286] [Pendulum Tracker -- SimuFísica: A Web-based Tool for Real-time Measurement of Oscillatory Motion](https://arxiv.org/abs/2506.07301)
*Marco P. M. de Souza,Juciane G. Maia,Lilian N. de Andrade*

Main category: physics.ed-ph

TL;DR: Pendulum Tracker是一个基于计算机视觉的实时测量物理摆运动的工具，适用于教育平台SimuFísica，支持多设备使用。


<details>
  <summary>Details</summary>
Motivation: 为教育实验物理提供一种实时测量和分析摆运动的工具，提升教学效果。

Method: 利用OpenCV.js库和摄像头自动检测摆的位置，实时显示角度-时间图和周期估计。

Result: 实验验证了其测量周期、重力加速度和分析阻尼振荡的准确性，与理论预测一致。

Conclusion: Pendulum Tracker是一个准确且多功能的工具，适用于物理实验教学。
...
### [287] [MedChat: A Multi-Agent Framework for Multimodal Diagnosis with Large Language Models](https://arxiv.org/abs/2506.07400)
*Philip Liu,Sparsh Bansal,Jimmy Dinh,Aditya Pawar,Ramani Satishkumar,Shail Desai,Neeraj Gupta,Xin Wang,Shu Hu*

Main category: cs.MA

TL;DR: MedChat是一个多代理诊断框架，结合专业视觉模型和角色特定的LLM代理，解决通用LLM在医学影像中的幻觉和解释性问题。
...
Method: 提出MedChat框架，结合专业视觉模型和多个角色特定的LLM代理，由导演代理协调，增强可靠性和交互性。

Result: MedChat减少了幻觉风险，提升了诊断报告的可靠性，并支持临床审查和教育用途的交互界面。

Conclusion: MedChat通过多代理协作，显著改善了医学影像诊断的准确性和实用性。
...
### [288] [Active Lubrication of Transluminal Medical Instruments](https://arxiv.org/abs/2506.07225)
...
TL;DR: 论文提出了一种通过超声波振动主动润滑腔内手术器械的方法，显著减少摩擦，提升手术安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 腔内微创手术中器械与组织摩擦可能导致并发症，如穿孔或器械弯曲，需减少摩擦以提升安全性。

Method: 利用超声波振动在器械表面生成压力流体层，动态润滑接触界面，减少摩擦。

Result: 原型导管测试中，摩擦减少42%（软组织）和82%（硬质表面），且温度安全，有效防止导管弯曲。

Conclusion: 主动润滑技术可显著提升腔内手术的安全性和效果。
...
### [289] [Towards Infant Sleep-Optimized Driving: Synergizing Wearable and Vehicle Sensing in Intelligent Cruise Control](https://arxiv.org/abs/2506.06459)
*Ruitao Chen,Mozhang Guo,Jinge Li*

Main category: cs.LG

TL;DR: 论文提出了一种结合强化学习的智能巡航控制框架，通过个性化驾驶行为优化婴儿睡眠质量，同时保持出行效率。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶对乘客舒适度的影响，尤其是婴儿睡眠质量，尚未充分研究。

Method: 整合LSTM和Transformer神经网络与强化学习，动态计算最佳驾驶策略。

Result: 仿真结果显示，该方法显著提高了婴儿睡眠质量，同时保持了出行效率。

Conclusion: 提出的框架有效平衡了乘客舒适度和出行效率，为自动驾驶的个性化优化提供了新思路。
...
TL;DR: 论文提出了一种基于大语言模型（LLM）的分层协作方法，用于多无人机系统的运动与通信控制，在动态约束环境中表现优异。


<details>
  <summary>Details</summary>
Motivation: 多无人机系统在动态和受限环境中的控制与优化仍具挑战性，需解决碰撞避免和交通流维护问题。

Method: 采用分层协作框架，HAPS上的LLM负责无人机接入控制，无人机上的LLM处理运动规划与控制。

Result: 实验表明，该方法系统奖励更高、运营成本更低，且无人机碰撞率显著降低。

Conclusion: 基于LLM的知识驱动范式为下一代3D空中高速公路系统发展提供了潜力。
...
### [291] [Safety-Aware Reinforcement Learning for Control via Risk-Sensitive Action-Value Iteration and Quantile Regression](https://arxiv.org/abs/2506.06954)
...
<details>
  <summary>Details</summary>
Motivation: 传统近似动作值迭代强化学习算法在高方差随机环境中存在过估计偏差，且难以满足安全约束。

Method: 提出了一种结合分位数回归和CVaR的风险正则化算法，确保安全约束并避免复杂架构。

Result: 理论证明了风险敏感分布Bellman算子的收敛性，仿真实验显示在动态任务中表现更优。

Conclusion: 该方法在安全性和性能上优于风险中性方法，适用于高方差环境。
...
### [292] [Graph-Assisted Stitching for Offline Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.07744)
*Seungho Baek,Taegeon Park,Jongchan Park,Seungjun Oh,Yusung Kim*

Main category: cs.LG

TL;DR: GAS提出了一种基于图搜索的子目标选择框架，替代传统的高层策略学习，通过TDR空间嵌入和TE指标优化，显著提升了离线分层强化学习的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线分层强化学习方法在任务时间跨度增加时效率下降，且缺乏跨轨迹状态转移的有效策略。

Method: GAS将子目标选择建模为图搜索问题，利用TDR空间嵌入状态并聚类，通过最短路径算法选择子目标序列，同时引入TE指标优化图质量。

Result: GAS在运动、导航和操作任务中表现优异，尤其在关键任务中得分88.3，远超之前的最佳得分1.0。

Conclusion: GAS通过图搜索和状态嵌入，显著提升了离线分层强化学习的效率和性能，为复杂任务提供了新思路。
...
### [293] [CellCLIP -- Learning Perturbation Effects in Cell Painting via Text-Guided Contrastive Learning](https://arxiv.org/abs/2506.06290)
*Mingyu Lu,Ethan Weinberger,Chanwoo Kim,Su-In Lee*

Main category: cs.LG

TL;DR: CellCLIP是一种用于高内涵筛选数据的跨模态对比学习框架，通过预训练图像编码器和新型通道编码方案，显著提升了跨模态检索和下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 高内涵筛选数据（如Cell Painting）可以揭示扰动与细胞形态变化的关系，但现有方法难以处理其语义差异和多类扰动表示。

Method: 结合预训练图像编码器、新型通道编码方案和自然语言编码器，构建跨模态对比学习框架CellCLIP。

Result: CellCLIP在跨模态检索和生物学任务中表现最佳，同时显著减少计算时间。
...
### [294] [NeurNCD: Novel Class Discovery via Implicit Neural Representation](https://arxiv.org/abs/2506.06412)
*Junming Wang,Yi Shi*

Main category: cs.LG

TL;DR: NeurNCD是一种用于开放世界新类发现的数据高效框架，通过结合Embedding-NeRF模型和KL散度，替代传统显式3D分割图，提升了语义嵌入和视觉嵌入空间的性能。
...
Method: 提出NeurNCD框架，结合Embedding-NeRF模型和KL散度，通过特征查询、调制和聚类等关键组件，实现语义嵌入与隐式神经表示的高效信息交换。

Result: 在NYUv2和Replica数据集上显著优于现有方法，无需密集标注数据或人工干预。

Conclusion: NeurNCD在开放和封闭世界场景中均表现出卓越的分割性能，为新类发现提供了高效解决方案。
...
### [295] [Vision-QRWKV: Exploring Quantum-Enhanced RWKV Models for Image Classification](https://arxiv.org/abs/2506.06633)
*Chi-Sheng Chen*

Main category: cs.LG

TL;DR: 论文提出了一种混合量子-经典架构Vision-QRWKV，用于图像分类任务，通过引入变分量子电路增强RWKV的非线性特征变换能力。
...
Result: 量子增强模型在多数数据集上优于经典模型，尤其在具有细微或噪声类别区分的数据集（如ChestMNIST、RetinaMNIST）上表现突出。

Conclusion: 研究首次系统地将量子增强RWKV应用于视觉领域，为轻量高效的量子视觉模型提供了架构权衡和未来潜力的见解。
...
### [296] [Non-Intrusive Load Monitoring Based on Image Load Signatures and Continual Learning](https://arxiv.org/abs/2506.06637)
...
<details>
  <summary>Details</summary>
Motivation: 传统非侵入式负载监测方法在复杂多变的负载组合和应用环境中面临特征鲁棒性差和模型泛化能力不足的挑战。

Method: 将多维电力信号转换为图像负载特征，结合深度卷积神经网络进行设备识别与分类，并引入自监督预训练和持续在线学习策略。

Result: 在高采样率负载数据集上的实验表明，该方法在识别精度上有显著提升。

Conclusion: 该方法通过图像特征和持续学习有效解决了传统方法的局限性，适用于新负载的出现。
...
### [297] [The OCR Quest for Generalization: Learning to recognize low-resource alphabets with model editing](https://arxiv.org/abs/2506.06761)
*Adrià Molina Rodríguez,Oriol Ramos Terrades,Josep Lladós*

Main category: cs.LG

TL;DR: 论文提出了一种通过模型编辑技术增强低资源语言识别能力的方法，显著提升了新字母表和跨域评估的性能。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言（如古代手稿和非西方语言）在识别系统中因数据不足而被忽视的问题，提升模型的泛化能力。

Method: 利用模型编辑技术，结合领域合并策略，避免对大规模预训练或原型设计的依赖。

Result: 实验显示，即使在相同训练数据下，该方法在字母表迁移学习和跨域评估中表现显著优于现有方法。

Conclusion: 该方法为低资源语言的文档识别提供了新思路，扩展了模型的文化和上下文适应性。
...
### [298] [Feature-Based Instance Neighbor Discovery: Advanced Stable Test-Time Adaptation in Dynamic World](https://arxiv.org/abs/2506.06782)
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络在训练和测试域之间的分布变化下性能下降，影响应用体验。现有测试时适应方法难以应对动态、多变的测试分布。

Method: FIND方法包括三个关键组件：层间特征解耦（LFD）、特征感知批量归一化（FABN）和选择性FABN（S-FABN）。LFD通过构建图结构稳定捕获相似分布特征，FABN结合源统计和测试分布统计优化特征表示，S-FABN决定哪些层需要特征分区。

Result: 实验表明，FIND在动态场景中显著优于现有方法，准确率提升30%，同时保持计算效率。

Conclusion: FIND通过特征解耦和自适应归一化策略，有效解决了测试时分布变化问题，为动态场景下的神经网络性能优化提供了新思路。
...
### [299] [FREE: Fast and Robust Vision Language Models with Early Exits](https://arxiv.org/abs/2506.06884)
*Divya Jyoti Bajpai,Manjesh Kumar Hanawal*

Main category: cs.LG

TL;DR: 论文提出了一种名为FREE的对抗训练方法，用于在视觉语言模型（VLMs）中实现早期退出（EE），以提高推理速度并减少性能损失。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在任务中表现优异，但其大尺寸导致推理延迟问题，限制了实际应用。
...
Conclusion: FREE方法有效提升了视觉语言模型的推理效率和鲁棒性。
...
### [300] [Rewriting the Budget: A General Framework for Black-Box Attacks Under Cost Asymmetry](https://arxiv.org/abs/2506.06933)
*Mahdi Salmani,Alireza Abdollahpoorrostam,Seyed-Mohsen Moosavi-Dezfooli*

Main category: cs.LG

TL;DR: 本文提出了一种针对图像分类器的非对称黑盒攻击框架，通过优化搜索策略和梯度估计过程，显著降低了高成本查询的依赖，从而在非对称查询成本下实现更低的攻击总成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设所有查询成本相同，而实际应用中某些查询可能成本更高（如触发额外审核或处罚）。针对这一非对称成本场景，现有算法效果有限，因此需要开发更有效的攻击方法。

Method: 提出非对称搜索（AS）和非对称梯度估计（AGREST），分别优化搜索策略和梯度估计过程，以减少高成本查询的使用。通过平衡不同类型查询，设计高效算法以最小化总攻击成本。

Result: 在标准图像分类基准测试中，该方法在不同成本机制下均表现优异，总查询成本和扰动幅度均低于现有方法，某些场景下提升达40%。

Conclusion: 本文提出的非对称黑盒攻击框架通过优化核心组件，显著提升了在非对称查询成本场景下的攻击效率，为实际应用提供了更实用的解决方案。
...
### [301] [Towards Physics-informed Diffusion for Anomaly Detection in Trajectories](https://arxiv.org/abs/2506.06999)
*Arun Sharma,Mingzhou Yang,Majid Farhadloo,Subhankar Ghosh,Bharat Jayaprakash,Shashi Shekhar*

Main category: cs.LG

TL;DR: 论文提出了一种基于物理知识的扩散模型，用于检测异常轨迹（如GPS欺骗），解决了现有方法因忽略时空依赖性和物理知识而导致的假阳性问题。


<details>
  <summary>Details</summary>
Motivation: 非法活动（如非法捕鱼和石油走私）在国际水域频发，而AI生成的深度伪造技术（如虚假轨迹）增加了检测难度，且缺乏足够的标记数据。

Method: 提出了一种物理知识引导的扩散模型，结合运动学约束，识别不符合物理规律的轨迹。

Result: 在真实数据集（海事和城市领域）上的实验表明，该方法在异常检测和轨迹生成方面具有更高的准确性和更低的误差率。

Conclusion: 该方法有效解决了异常轨迹检测中的假阳性问题，为遏制非法活动提供了技术支持。
...
### [302] [Advancing Multimodal Reasoning Capabilities of Multimodal Large Language Models via Visual Perception Reward](https://arxiv.org/abs/2506.07218)
*Tong Xiao,Xin Xu,Zhenya Huang,Hongyu Gao,Quan Liu,Qi Liu,Enhong Chen*

Main category: cs.LG

TL;DR: 论文提出Perception-R1方法，通过引入视觉感知奖励提升多模态大语言模型（MLLMs）的感知和推理能力，解决了现有RLVR方法在增强多模态感知能力上的不足。
...
### [303] [Variational Supervised Contrastive Learning](https://arxiv.org/abs/2506.07413)
*Ziwen Wang,Jiajun Fan,Thao Nguyen,Heng Ji,Ge Liu*

Main category: cs.LG

TL;DR: VarCon是一种变分监督对比学习方法，通过变分推断优化类感知匹配，解决了传统对比学习的分布控制和泛化问题，在多个数据集上表现优异。
...
Result: 在CIFAR-10、CIFAR-100、ImageNet-100和ImageNet-1K上，VarCon达到SOTA性能（如ImageNet-1K上79.36% Top-1准确率），并展现出更清晰的决策边界和语义组织。

Conclusion: VarCon通过变分推断显著提升了对比学习的性能和泛化能力，适用于少样本学习和多种增强策略。
...
### [304] [Language Embedding Meets Dynamic Graph: A New Exploration for Neural Architecture Representation Learning](https://arxiv.org/abs/2506.07735)
*Haizhao Jing,Haokui Zhang,Zhenhao Shang,Rong Xiao,Peng Wang,Yanning Zhang*

Main category: cs.LG

TL;DR: LeDG-Former结合语言嵌入和动态图表示学习，解决了现有方法忽视硬件属性和静态邻接矩阵的局限性，实现了跨硬件平台的零样本预测和更优的神经网络建模。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视硬件属性信息且依赖静态邻接矩阵，限制了模型的实用性和编码效果。

Method: 提出LeDG-Former框架，结合语言嵌入（通过LLM处理）和动态图Transformer，实现硬件和架构的统一表示。

Result: 在NNLQP、NAS-Bench-101和NAS-Bench-201数据集上表现优异，首次实现跨硬件延迟预测。

Conclusion: LeDG-Former通过创新框架解决了现有问题，提升了神经网络架构建模的实用性和性能。
...
### [305] [Identifiable Object Representations under Spatial Ambiguities](https://arxiv.org/abs/2506.07806)
*Avinash Kori,Francesca Toni,Ben Glocker*

Main category: cs.LG

TL;DR: 提出了一种多视角概率方法，解决空间模糊性问题，无需视角标注，并在实验中验证了其鲁棒性和扩展性。
...
Result: 解决了空间模糊性，提供了可识别性的理论保证，并在实验中表现出鲁棒性和扩展性。

Conclusion: 该方法在无需视角标注的情况下，有效解决了空间模糊性问题，具有理论和实践优势。
...
### [306] [Diffusion Counterfactual Generation with Semantic Abduction](https://arxiv.org/abs/2506.07883)
*Rajat Rasal,Avinash Kori,Fabio De Sousa Ribeiro,Tian Xia,Ben Glocker*

Main category: cs.LG

TL;DR: 论文提出了一种基于扩散模型的因果图像生成框架，通过引入空间、语义和动态推理，实现了高保真和身份保留的反事实图像编辑。
...
Method: 提出了一套基于扩散模型的因果机制，结合Pearl因果理论，将语义表示融入扩散模型，实现反事实推理的图像编辑。

Result: 首次实现了扩散模型中高级语义身份保留的反事实编辑，展示了语义控制在因果控制和身份保留之间的权衡。

Conclusion: 该框架为反事实图像生成提供了新的解决方案，平衡了因果控制的忠实性和身份保留的需求。
...
### [307] [Diffuse Everything: Multimodal Diffusion Models on Arbitrary State Spaces](https://arxiv.org/abs/2506.07903)
*Kevin Rojas,Yuchen Zhu,Sichen Zhu,Felix X. -F. Ye,Molei Tao*

Main category: cs.LG

TL;DR: 本文提出了一种新的多模态扩散模型框架，支持原生生成不同模态的耦合数据，无需依赖外部预处理协议。
...
Method: 提出了一种新颖的框架，支持在任意状态空间上构建多模态扩散模型，并引入解耦的噪声调度策略。

Result: 在文本-图像生成和混合类型表格数据合成任务中表现优异。

Conclusion: 该框架实现了多模态数据的原生生成，具有竞争力。
...
### [308] [Generative Modeling of Weights: Generalization or Memorization?](https://arxiv.org/abs/2506.07998)
*Boya Zeng,Yida Yin,Zhiqiu Xu,Zhuang Liu*

Main category: cs.LG

TL;DR: 生成模型在图像和视频生成中表现优异，但用于生成神经网络权重时，现有方法主要通过记忆训练检查点，而非真正生成新颖权重。


<details>
  <summary>Details</summary>
Motivation: 探索生成模型在合成高性能神经网络权重方面的潜力，并评估其是否能生成与训练数据不同的新权重。

Method: 研究了四种代表性方法，分析其生成权重的能力，并与简单基线（如添加噪声或权重集成）进行比较。

Result: 现有方法主要通过记忆训练检查点生成权重，无法超越简单基线，且记忆问题难以通过常见缓解措施解决。

Conclusion: 研究揭示了当前生成模型的局限性，强调了在新领域评估生成模型时需更谨慎。
