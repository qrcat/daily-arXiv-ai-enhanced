...
TL;DR: 介绍了EaGERS，一个无需训练的文档视觉问答管道，通过生成自然语言理由并将其定位到空间子区域来提高DocVQA的准确性和透明度


<details>
  <summary>Details</summary>
Motivation: 现有的文档视觉问答(DocVQA)系统缺乏透明度和可重现性，需要一种能够提供明确理由并定位到相关区域的方法，同时避免额外的模型微调

Method: 提出EaGERS管道，包含三个步骤：(1)通过视觉语言模型生成自然语言理由；(2)通过计算可配置网格上的多模态嵌入相似性并使用多数投票将理由定位到空间子区域；(3)仅从掩码图像中选定的相关区域生成响应

Result: 在DocVQA数据集上的实验表明，最佳配置在精确匹配准确率和平均归一化Levenshtein相似性指标上都优于基础模型，同时提高了DocVQA的透明度和可重现性

Conclusion: EaGERS作为一个完全无需训练且模型无关的管道，成功提升了文档视觉问答的性能，并通过提供空间定位的理由增强了系统的透明度和可解释性，无需额外的模型微调
...
Method: MindJourney通过耦合VLM和基于视频扩散的可控世界模型，迭代生成相机轨迹并合成多视角图像，供VLM推理。
...
TL;DR: 论文提出Mono-InternVL和Mono-InternVL-1.5两种单模态大语言模型，通过视觉参数空间嵌入和渐进学习提升性能，显著降低训练和推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有单模态大语言模型存在优化不稳定和灾难性遗忘问题，需改进视觉知识学习方式。

Method: 嵌入视觉参数空间，采用多模态专家混合架构和渐进式视觉预训练（EViP++）。

Result: Mono-InternVL在15个基准测试中12个领先，Mono-InternVL-1.5显著降低成本并保持性能。

Conclusion: 提出的方法有效解决了单模态大语言模型的挑战，性能优越且成本更低。
...
TL;DR: 本文综述了大尺度像素级作物分类的工作流程，比较了传统监督方法和新兴迁移学习方法，并评估了预处理、模型选择和样本量对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 研究旨在确定最优的作物分类工作流程，以提升大尺度作物分类的准确性和适应性。

Method: 通过系统实验比较了六种预处理方法和十一种监督分类模型，并评估了迁移学习技术在不同领域偏移下的表现。

Result: 研究发现，精细预处理与Transformer模型表现最佳；迁移学习提升了工作流程的适应性，而样本量决定了监督学习或迁移学习的适用性。

Conclusion: 工作流程的选择取决于样本量，监督学习在样本充足时更优，而迁移学习在样本不足时是可行的替代方案。
...
Method: 提出CT-ScanGaze数据集，开发CT-Searcher预测器处理3D输入，并利用2D数据集预训练模型。
...
TL;DR: MS-DGCNN++是一种改进的多尺度动态图卷积网络，通过层次化多尺度融合和语义特征提取，显著提升了树种类分类和3D物体识别的准确率。
...
Result: 在树种类分类和3D物体识别任务中均取得显著提升，准确率分别达到94.96%和93.15%。
...
TL;DR: NeuraLeaf是一种用于3D植物叶片建模的神经参数模型，通过分离2D基础形状和3D变形，解决了叶片形状多样性和变形灵活性的挑战。


<details>
  <summary>Details</summary>
Motivation: 植物叶片在农业和计算机图形学中具有重要意义，但其多样性和变形特性为建模带来挑战。
...
Method: 结合HAR特征嵌入和上下文元数据，评估22种骨干模型。

Result: 最高准确率63.9%，优于守门员决策。

Conclusion: 数据集和模型为体育预测任务提供了通用解决方案。
...
TL;DR: Funnel-HOI框架通过编码器阶段引入HOI特定线索，利用非对称共注意力机制和多模态信息提升交互检测性能，在零样本和长尾分布问题上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决人-物交互检测（HOID）中因对象-动作组合数量庞大导致的标注数据有限和长尾分布问题，提出在编码器阶段引入HOI特定线索以增强场景理解。

Method: 提出Funnel-HOI框架，先检测对象（明确概念），再关联动作（抽象概念），利用非对称共注意力机制和多模态信息挖掘线索，并设计新损失函数优化交互分类器。

Result: 在HICO-DET和V-COCO数据集上，Funnel-HOI在完全监督和零样本设置中表现优异，未见和罕见HOI类别分别提升12.4%和8.4%。

Conclusion: Funnel-HOI通过编码器阶段的HOI特定线索和新型损失函数，显著提升了交互检测性能，尤其在零样本和长尾分布场景中表现突出。
...
Conclusion: CogNVS通过自监督和零样本微调，为动态场景新视角合成提供了高效且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，但现有风险分层方法常无法检测早期亚临床变化。研究旨在填补视网膜微血管特征与脂质代谢在早期心血管疾病发病机制中的关联空白。

Method: 采用深度学习图像处理技术量化视网膜表型，结合超高效液相色谱-电喷雾电离高分辨质谱（UHPLC ESI HRMS）分析血清脂质组学数据，进行大规模协变量调整和分层相关性分析。
...
Conclusion: SOD-YOLO为无人机图像中的小目标检测提供了实用高效的解决方案，代码和模型已开源。
...
Result: 总结了当前研究中的最佳实践，并提出了开发可靠、工业级模型的指导。

Conclusion: 为研究者和从业者提供了结构化指导，推动基于Transformer的空间定位模型的进一步发展。
...
Result: 在多个基准数据集（DAD、CCD、A3D）上验证了更高的预测准确性、响应速度、计算效率和可解释性。

Conclusion: 该方法在交通事故预测领域设定了新的性能基准。
...
Method: HairShifter框架包含图像发型转换模块（IHT）和多尺度门控SPADE解码器，实现精确的逐帧转换和时空一致性。
...
Method: Mamba Snake将多轮廓演化建模为分层状态空间图谱，结合蛇形视觉状态空间模块（MEB）和能量图形状先验，优化复杂形态的自适应细化。
...
TL;DR: 本文提出Think-Before-Draw框架，通过Chain-of-Thought将抽象情感标签转化为面部肌肉运动描述，并结合渐进式去噪策略优化微表情动态，显著提升了情感说话头生成的自然性。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本的情感说话头生成方法依赖离散情感标签，简化了面部肌肉运动的动态复杂性，导致生成结果不够自然。

Method: 引入Chain-of-Thought将情感标签转化为肌肉运动描述，并提出渐进式去噪策略，通过全局情感定位和局部肌肉控制优化微表情动态。
...
TL;DR: 提出了一种结合生成场景增强和自适应时序推理的框架，以解决自动驾驶系统中事故预测的数据稀缺和关键线索缺失问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要可靠的事故预测能力，但面临数据稀缺和环境干扰的挑战。

Method: 开发了基于领域提示的视频生成管道和动态预测模型，结合图卷积和时序操作。

Result: 实验表明，该框架提高了事故预测的准确性和提前时间。

Conclusion: 该框架为自动驾驶的安全关键应用提供了稳健的解决方案。
...
TL;DR: 本文介绍了一种自主无人机视觉系统，用于动态海洋环境中鲨鱼的连续实时跟踪，通过多无人机协作扩展覆盖范围。
...
Method: 集成机载计算机、稳定RGB-D摄像头和定制OSTrack管道，实现视觉识别和多无人机无缝交接。

Result: 在5200帧鲨鱼数据集上，实时跟踪成功率达81.9%，多无人机交接覆盖率达82.9%。

Conclusion: 证明了协调无人机操作在海洋跟踪中的可行性，为自主监测奠定了基础。
...
Method: 提出任务无关动作范式ATARA和AnyPos模型，结合自监督框架和逆动力学模型，优化数据收集和学习效率。

Result: 实验表明，AnyPos-ATARA管道在测试准确率上提升51%，下游任务成功率提高30-40%。

Conclusion: 任务无关范式结合自监督和逆动力学模型显著提升了动作模型的泛化能力和效率。
...
Result: 实验结果显示，ReToM在FID上提升了6.2%，CLIP分数更高，同时推理时间相当。
...
Method: 在Transformer块的MLP层中进行输入/输出通道选择，并结合理论泛化边界指导剪枝。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型（LVLMs）主要针对室内场景，难以适应户外大规模场景的多视角、多模态需求。

Method: 构建SVM-City数据集，包含多尺度、多视角和多模态数据；设计City-VLM模型，通过联合概率分布空间实现多模态融合。

Result: City-VLM在问答任务中平均性能提升18.14%，展现了跨户外场景的实用性和泛化能力。
...
TL;DR: DeQA-Doc是一个基于多模态大语言模型（MLLM）的文档质量评估框架，通过软标签策略和分辨率调整，显著提升了文档质量评分的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有文档质量评估方法在准确性和鲁棒性上表现不足，限制了其实际应用。MLLM在图像质量评估中的成功启发了将其扩展到文档领域的尝试。

Method: 提出DeQA-Doc框架，利用MLLM的视觉语言能力和软标签策略回归连续质量分数，通过分辨率调整和集成方法优化性能。
...
TL;DR: 提出了一种名为Sage的新方法，通过结合文本模态的语义指导来优化视觉编码器的微调，以解决长尾学习中的分布不一致问题。


<details>
  <summary>Details</summary>
Motivation: 长尾场景中类别样本量的差异导致低频类别性能下降，现有方法忽略了视觉与文本模态的对齐。

Method: 提出SG-Adapter，利用类描述作为语义指导，通过注意力机制增强视觉与文本对齐；并设计分布不匹配感知补偿因子来纠正预测偏差。

Result: 在基准数据集上的实验验证了Sage在长尾学习中的有效性。

Conclusion: Sage通过多模态对齐和分布补偿显著提升了长尾学习的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频问答方法主要依赖事件中心的问题-答案对，缺乏对视频更广泛上下文的理解，限制了模型的泛化和高级推理能力。

Method: FIQ方法通过生成基于视频描述的问题-答案对，并引入VQ-CAlign模块，将任务特定问题嵌入与视觉特征对齐。

Result: 在SUTD-TrafficQA数据集上，FIQ方法达到了最先进的性能。

Conclusion: FIQ通过增强对视频基础信息的理解，显著提升了模型的泛化和推理能力。
...
TL;DR: MCoT-RE是一种无需训练的零样本CIR框架，通过多角度思维链和重排序提升检索效果。
...
Method: 利用多角度思维链生成两种描述，结合多粒度重排序进行检索。

Result: 在FashionIQ和CIRR数据集上分别提升6.24%和8.58%的召回率。

Conclusion: MCoT-RE在无需训练的方法中达到最优效果。
...
TL;DR: FAR-Net提出了一种多阶段融合框架，通过增强语义对齐和自适应协调模块，解决了现有早期和晚期融合方法在组合图像检索中的不足。
...
TL;DR: 论文介绍了SMOT4SB挑战，通过时间信息解决单帧检测的局限性，提出了新数据集、新评估指标SO-HOTA，并展示了挑战赛结果。


<details>
  <summary>Details</summary>
Motivation: 解决小目标多目标跟踪（SMOT）在目标像素极少时的检测和关联不可靠问题，推动无人机场景下的应用。

Method: 提出SMOT4SB数据集（211个无人机视频序列）、新评估指标SO-HOTA，并举办MVA2025挑战赛。

Result: 挑战赛获胜方法比基线提升5.1倍，SO-HOTA指标有效缓解IoU对小位移的敏感性。

Conclusion: 为无人机场景下的SMOT研究奠定基础，应用包括鸟击避免、农业、渔业和生态监测。
...
Method: 提出了一种针对DINOv2粗粒度特征的关键点检测器，结合几何特征增强表示，并使用基于Transformer的匹配器和可微分位姿估计层。

Result: 在TartanAir和KITTI数据集上优于现有帧间视觉里程计方法，在EuRoC数据集上表现竞争性，运行效率达72 FPS且内存占用小于1GB。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在细粒度控制和可靠评估方面存在不足，需要改进多模态字幕生成的可控性和评估方法。

Method: ACM框架利用用户指令和模态特征改进基础模型的字幕生成，无需重新训练基础模型。同时构建了ACD数据集和AnyCapEval评估基准。

Result: ACM显著提升了字幕质量，在AnyCapEval上，ACM-8B将GPT-4o的内容得分提高了45%，风格得分提高了12%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法受限于固定知识库和推理能力，缺乏经验知识的有效整合和进化能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究目标是开发一种无需数据集微调即可适应不同分辨率和视差范围的通用立体匹配模型。

Method: 采用多分辨率Transformer进行长距离匹配，结合新型损失函数集中概率于可行匹配，实现视差、遮挡和置信度的联合估计。

Result: $S^2M^2$在Middlebury v3和ETH3D基准测试中达到最新技术水平，显著优于现有方法。

Conclusion: $S^2M^2$成功解决了全局匹配架构的计算和内存成本问题，同时实现了高精度和高效性。
...
<details>
  <summary>Details</summary>
Motivation: CLIP在领域泛化中难以关注跨领域的任务相关区域（即领域不变区域），导致在未见目标领域上表现不佳。

Method: 提出SRE方法：1）通过数据增强模拟领域偏移生成模拟目标域；2）通过注意力重聚焦减少领域偏移；3）利用集成学习增强领域不变注意力图的捕捉能力。
...
TL;DR: VITA是一种视觉到动作的流匹配策略，通过将潜在视觉表示映射为潜在动作，简化了传统流匹配和扩散策略的复杂条件机制，并在ALOHA平台上表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统流匹配和扩散策略需要额外的条件机制（如交叉注意力）来生成基于视觉信息的动作，导致时间和空间开销。VITA旨在消除这些模块，同时保持生成建模能力。

Method: VITA将潜在图像作为流源，通过自编码器创建结构化的动作潜在空间，并通过流匹配和动作重构损失进行端到端学习。

Result: VITA在ALOHA平台的复杂双手机器人任务中表现优于或匹配现有生成策略，同时将推理延迟降低了50-130%。

Conclusion: VITA是首个仅使用MLP层即可解决复杂双手机器人任务的流匹配策略，展示了其高效性和简单性。
...
TL;DR: 提出了一种名为SCORE的开放词汇遥感实例分割框架，通过整合多粒度场景上下文（区域和全局上下文）提升视觉和文本表示，解决了现有方法在遥感图像中的局限性。
...
Method: SCORE框架通过区域感知集成（细化类嵌入）和全局上下文适应（丰富文本嵌入）增强视觉和文本表示。

Result: 实验表明，SCORE在开放词汇遥感实例分割任务中达到了SOTA性能。
...
Conclusion: HRSeg通过高效的高分辨率感知和增强机制，显著提升了推理分割任务的性能。
...
Method: 利用嵌入项链式设备的干电极捕捉颈部组织阻抗变化，结合解剖学先验和深度学习框架估计头部姿态。

Result: 在7名参与者中测试，平均顶点误差为25.9毫米，性能与视觉方法相当。

Conclusion: NeckSense展示了生物阻抗传感在头部跟踪中的潜力，提供了一种紧凑且无需视线的解决方案。
...
TL;DR: 提出一种基于摄像头、用户无感知的情绪识别方法，整合凝视模式与环境语义及时态动态，实现实时、连续的情绪识别。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式信号（如面部表情、语音或手势），忽略环境背景影响，且生理信号方法需复杂传感器，限制自然行为和扩展性。

Method: 利用标准高清摄像头捕捉用户眼部外观和头部运动，估计时空凝视轨迹，建模凝视行为的空间、语义和时态维度。

Result: 方法能无感知、实时、连续地识别情绪，具有高通用性和低部署成本。

Conclusion: 情绪是人与环境互动的复杂结果，新方法通过动态凝视与环境交互揭示了这一点。
...
Method: 论文首先将五种图像分类的性能估计方法适配到车道检测任务中作为基线。然后提出LanePerf框架，结合图像和车道特征，利用预训练图像编码器和DeepSets架构处理零车道检测和大域偏移情况。
...
Method: 提出Fed-Cyclic（循环拓扑）和Fed-Star（星形拓扑）两种算法，分别通过局部训练和预聚合解决统计异构问题。

Result: 实验证明新算法在自建数据集上优于现有基线。

Conclusion: 新数据集和算法为联邦学习领域提供了实用工具，并展示了性能优势。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D点云重建方法存在信息丢失问题，如纹理缺失或复杂结构导致的细节失真，多视角图像能弥补这些不足。

Method: Argus融合多视角图像和相机位姿生成场景特征，与3D特征交互，形成全面的3D感知场景嵌入。

Result: 实验表明，Argus在多种下游任务中优于现有3D-LMMs。

Conclusion: Argus通过多模态输入扩展LLMs的3D任务能力，显著提升了3D场景理解的性能。
...
TL;DR: 提出了一种结合LES和PTS的DMQ方法，有效解决了扩散模型在低比特量化中的性能退化问题，显著提升了图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型计算成本高，现有PTQ方法在低比特量化时因忽略异常值导致性能下降。

Method: 结合LES优化通道缩放因子，引入自适应时间步加权方案，并使用PTS处理高方差通道，增强量化鲁棒性。

Result: 在低比特（如W4A6、W4A8）下显著优于现有方法，保持高质量图像生成和模型稳定性。

Conclusion: DMQ方法通过LES和PTS有效解决了量化问题，为资源受限环境提供了高效解决方案。
...
TL;DR: 提出了一种弱监督跨模态行人重识别方法，仅需单模态样本标签，通过异构专家协作一致性学习框架提升跨模态识别能力。
...
Conclusion: MUPM通过共享预训练和轻量微调实现不确定性关系的量化，为临床应用中多模态数据的鲁棒分析提供了实用工具。
...
Method: 使用FlexFormer将视频和文本联合压缩为统一潜在表示，支持可变长度输入和单查询令牌设计。

Result: 实验验证了LoViC在多种任务中的有效性和多功能性。

Conclusion: LoViC通过创新的设计和统一的范式，成功生成了长且连贯的视频。
...
TL;DR: cIDIR是一种基于隐式神经表示（INRs）的新型可变形图像配准（DIR）框架，通过条件化正则化超参数优化配准过程，避免了传统方法需要多次训练的问题。


<details>
  <summary>Details</summary>
Motivation: 传统DIR框架中，正则化参数的调优计算成本高且需多次训练，cIDIR旨在解决这一问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式几何先验，常导致跨重演伪影且难以捕捉细微情感，同时缺乏多人动画支持。
...
TL;DR: 研究通过多模态方法（结合X光图像和患者元数据）及细粒度识别策略，在有限数据集上提高了手腕病理诊断的准确性。


<details>
  <summary>Details</summary>
Motivation: 手腕病理诊断耗时且依赖专家，计算机视觉因数据稀缺和单一模态限制而面临挑战。

Method: 采用细粒度识别策略、融合患者元数据与X光图像，并使用细粒度数据集预训练权重。

Result: 诊断准确性在小数据集上提升2%，在大骨折数据集上提升超过10%。
...
TL;DR: 该论文提出了一种基于预训练RGB扩散模型的两阶段方法，用于从RGB图像重建高光谱图像（HSI），通过分离不可观测特征并学习其与RGB的联合分布，实现了先进的性能。
...
Method: 扩展RGB预训练的潜在扩散模型（RGB-LDM）为不可观测特征LDM（ULDM），通过两阶段流程：1）光谱结构表示学习；2）光谱-空间联合分布学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有样本选择方法在冷启动场景或噪声对应处理上存在不足，需要更高效且准确的方法。
...
TL;DR: 该论文提出了一种基于涂鸦标注的弱监督图像篡改定位方法，通过自监督训练和动态特征调整模块，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统深度学习方法依赖大规模像素级标注数据的问题，同时提升弱监督方法的性能。

Method: 采用涂鸦标注监督，结合自监督训练、先验感知特征调制模块（PFMM）、门控自适应融合模块（GAFM）和置信感知熵最小化损失（${\mathcal{L}}_{ {CEM }}$）。

Result: 实验表明，该方法在分布内和分布外数据上均优于现有全监督方法。

Conclusion: 涂鸦标注监督结合动态特征调整和不确定性正则化，显著提升了图像篡改定位的性能和效率。
...
TL;DR: 该研究改进了Masked AutoRegressive (MAR)模型，提出MaskGIL架构，通过双向注意力和2D RoPE提升图像生成质量，性能媲美AR模型，且推理步骤大幅减少。


<details>
  <summary>Details</summary>
Motivation: 传统MAR模型在图像生成中表现不如AR模型，研究旨在通过改进MAR架构提升其性能。

Method: 评估多种图像标记器，引入双向注意力和2D RoPE，构建MaskGIL模型，并扩展至文本驱动生成和多任务应用。

Result: MaskGIL在ImageNet 256x256基准测试中FID得分3.71，仅需8步推理，性能与AR模型相当。

Conclusion: MaskGIL不仅提升了MAR模型的性能，还扩展了其应用范围，如实时语音转图像。
...
TL;DR: 提出了一种基于成本体积的运动特征表示方法，通过减少子网络数量提升效率，实现了实时性能和高精度。


<details>
  <summary>Details</summary>
Motivation: 在安全关键机器人应用中，实时准确检测动态对象至关重要。现有方法因使用多个子网络导致计算成本高，影响实时性能。

Method: 提出Channel-wise Motion Features，仅需Pose Network提取深度特征和3D运动信息，减少计算负担。

Result: 在KITTI和Cityscapes数据集上，FPS提升约4倍，参数减少至25%，同时保持同等精度。

Conclusion: 新方法通过简化网络结构，显著提升了运动分割的效率和实时性能。
...
<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测（OWOD）需要检测未知对象并增量学习新类别，但缺乏未知对象的标注数据。PROB模型虽无需伪标签，但存在对象性与类别预测的冲突。

Method: 提出Decoupled PROB，引入ETOP终止对象性预测，避免冲突；采用TDQI高效提取已知和未知对象特征。

Result: 在OWOD基准测试中，Decoupled PROB性能显著优于现有方法。

Conclusion: Decoupled PROB通过解耦对象性和类别预测，有效提升了开放世界目标检测的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割中标注变异性大，传统深度学习方法无法捕捉标注者偏见，现有方法仅关注单一视角。
...
TL;DR: 论文提出了一种名为GLAD的框架，通过LoRA和梯度正则化技术解决少样本学习中的过拟合问题，并在多个基准数据集上表现出色。
...
Conclusion: GLAD提供了一种简单通用的解决方案，显著提升了少样本学习的鲁棒性和泛化能力。
...
TL;DR: 提出了一种基于深度学习的自动化胎儿肺成熟度评估流程，结合了肺分割模型和模型拟合评估方法，结果显示与手动分割结果无显著差异。
...
Method: 使用3D nnU-Net模型进行肺分割，并结合模型拟合评估IVIM参数。

Result: 分割模型平均Dice系数为82.14%，自动化与手动分割结果无差异。
...
TL;DR: 提出了一种参数高效的终身视觉概念学习框架R²MoE，通过路由蒸馏、冗余专家移除和分层注意力机制，显著减少遗忘率和参数开销。
...
Method: 结合混合专家框架、路由蒸馏、冗余专家移除和分层注意力机制。

Result: 在CustomConcept 101数据集上，遗忘率降低87.8%，参数减少63.3%，生成图像概念保真度更高。

Conclusion: R²MoE在终身视觉概念学习中表现出色，平衡了性能和参数效率。
...
TL;DR: 提出了一种基于注册的异常检测框架，结合多原型对齐和聚类差异分析，用于高分辨率3D点云中的精确异常定位。
...
Method: 通过将测试样本注册到多个正常原型以进行直接结构比较，并采用关键点引导的聚类策略，在特征丰富的区域进行局部相似性计算。

Result: 在Real3D-AD基准测试中，该方法在对象级和点级异常检测上均达到最先进性能，仅使用原始特征。

Conclusion: 该方法通过多原型对齐和关键点引导的聚类，有效解决了3D点云异常检测中的挑战，实现了高精度定位。
...
Method: 提出RS-TinyNet，包含多维度协作注意力模块（MDCA）、辅助可逆分支（ARB）和渐进式融合检测头（PFDH），用于增强目标显著性和特征完整性。

Result: 在AI-TOD数据集上AP和AP75分别提升4.0%和6.5%，在DIOR数据集上验证了其广泛适用性。
...
TL;DR: 该论文提出了一种简化的自动驾驶世界模型，无需额外监督或传感器，性能优于现有方法，尤其在复杂场景中表现突出。通过对比实验，发现连续自回归模型优于离散令牌模型。
...
Method: 采用简单设计，无需额外监督或传感器，构建混合分词器对比离散令牌模型与连续自回归模型。
...
TL;DR: 该论文提出了一种利用Midjourney生成合成图像的方法，用于解决建筑工人检测中数据不足的问题，并在真实和合成数据集上取得了良好的检测效果。
...
Result: 在真实数据集上AP为0.937（IoU=0.5）和0.642（IoU=0.5-0.95），在合成数据集上AP为0.994和0.919。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频质量的提升，检测此类内容以防止虚假信息、保护隐私和安全变得至关重要。现有方法主要针对DeepFakes，而通用视频检测方法尚不完善。
...
TL;DR: 论文提出了一种近似正交微调（AOFT）策略，通过生成近似正交向量构建低秩权重矩阵，以增强预训练视觉变换器（ViT）在下游任务中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 观察到预训练ViT的权重矩阵具有近似正交性，而低秩适应矩阵（如LoRA和Adapter）缺乏这一特性，这可能导致泛化能力不足。因此，研究如何通过保持正交性提升微调模型的泛化能力。

Method: 提出AOFT策略，使用单个可学习向量生成近似正交向量，构建低秩适应矩阵（down/up-projection），使其与预训练权重矩阵具有相似的正交性。

Result: 实验表明，AOFT在多个下游图像分类任务中表现优异，验证了其通过正交性增强泛化能力的有效性。

Conclusion: AOFT通过保持低秩矩阵的正交性，显著提升了ViT在下游任务中的泛化性能，为参数高效微调提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 解决RPE基准在可重复性和评估质量上的问题，促进公平一致的定量评估。

Method: 通过细致的视觉匹配改进GT标注，并公开这些标注作为开源资源。

Result: 改进了RPE基准的可靠性和可重复性，减少了手动匹配的错误。

Conclusion: 公开的改进标注资源有助于未来在人体姿态感知多模态推理领域的研究。
...
TL;DR: 提出了一种实时检测手-物体交互的高效方法，结合动作识别和物体检测模块，在30fps下性能优异。
...
Result: 动作识别模块在ENIGMA-51上达到38.52% p-AP，物体检测模块达到85.13% AP。
...
Method: 1. 使用高压缩变分自编码器（VAE）降低输入数据维度；2. 引入KD引导的三级剪枝策略缩小模型尺寸；3. 开发针对DiT的对抗性步骤蒸馏技术，减少推理步骤至4步。

Result: 优化后的模型在iPhone 16 Pro Max上实现每秒10帧以上的生成速度，证明了移动设备上实时高质量视频生成的可行性。

Conclusion: 通过优化方法，成功在移动平台上实现了实时视频生成，解决了DiT的高计算成本问题。
...
TL;DR: 提出了一种滑动迭代去噪方法，用于提升4D扩散模型在稀疏视角视频输入下的时空一致性，从而改善高保真视图合成质量。
...
Conclusion: 该方法显著提升了4D一致性，且GPU内存消耗可控。
...
<details>
  <summary>Details</summary>
Motivation: 视觉生成任务中复杂概念的响应和组合缺乏稳定性且易出错，这一问题尚未得到充分研究。

Method: 通过精心设计的实验探索概念响应差的原因，并提出了一种在线概念均衡损失函数（IMBA损失），无需离线数据处理且代码改动少。
...
<details>
  <summary>Details</summary>
Motivation: 探索利用3DShape2VecSet的几何表达能力，实现部件级别的3D对象生成。

Method: 基于3DShape2VecSet的潜在空间，自回归生成部件，逐步预测并组合成完整对象。

Result: 在3D部件生成任务中达到最先进性能，生成的部件可直接组装无需额外优化。

Conclusion: AutoPartGen展示了在部件级3D生成任务中的高效性和优越性。
...
Method: 提出VisionThink，动态决定是否需高分辨率图像，结合强化学习和LLM-as-Judge策略优化决策。
...
Method: 通过小批量耦合逐步调整分层流匹配中各层分布的复杂度。
...
Method: 采用VidThinker管道，包括指令条件化的片段级标注生成、指令引导的视频片段检索和细粒度帧选择。
...
### [90] [WaFusion: A Wavelet-Enhanced Diffusion Framework for Face Morph Generation](https://arxiv.org/abs/2507.12493)
*Seyed Rasoul Hosseini,Omid Ahmadieh,Jeremy Dawson,Nasser Nasrabadi*

Main category: cs.GR

TL;DR: WaFusion结合小波分解和扩散模型，高效生成高质量的人脸融合图像，优于现有方法。
...
Conclusion: WaFusion为生物特征安全系统提供了高效且先进的解决方案，设定了新标准。
...
### [91] [Wavelet-GS: 3D Gaussian Splatting with Wavelet Decomposition](https://arxiv.org/abs/2507.12498)
*Beizhen Zhao,Yifan Zhou,Sicheng Yu,Zijian Wang,Hao Wang*

Main category: cs.GR

TL;DR: 提出了一种基于小波分解的解耦优化框架，改进3D高斯泼溅（3DGS）在复杂场景重建中的表现，解决了全局结构不完整和局部光照效果不清晰的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在复杂场景重建中表现不佳，存在全局结构不完整和局部光照效果不清晰的问题，需要一种更有效的优化方法。

Method: 通过3D小波分解将点云分为高频和低频成分，低频优化全局结构和高斯分布，高频恢复细节并加入重光照模块；同时利用2D小波分解模拟辐射变化以指导细节重建。

Result: 在多个数据集上实验表明，该方法在各项指标上均达到最优性能，超越了现有方法。

Conclusion: 提出的解耦优化框架显著提升了3DGS在复杂场景重建中的表现，推动了3D场景重建领域的发展。
...
### [92] [HairFormer: Transformer-Based Dynamic Neural Hair Simulation](https://arxiv.org/abs/2507.12600)
...
Conclusion: 该方法在广泛场景下表现出优异的通用性和高保真效果，解决了头发动态模拟的关键挑战。
...
### [93] [VolSegGS: Segmentation and Tracking in Dynamic Volumetric Scenes via Deformable 3D Gaussians](https://arxiv.org/abs/2507.12667)
*Siyuan Yao,Chaoli Wang*

Main category: cs.GR

TL;DR: VolSegGS是一种基于高斯泼溅的框架，支持动态体素场景的交互式分割和跟踪，用于探索性可视化和分析。
...
Method: 利用可变形3D高斯表示动态场景，结合视图无关颜色和亲和力场网络实现分割，嵌入分割结果以支持跟踪。

Result: 在多个时变数据集上验证了有效性，实时交互且计算需求低。

Conclusion: VolSegGS为时变体素数据分析和可视化提供了强大且高效的解决方案。
...
### [94] [Physically Based Neural LiDAR Resimulation](https://arxiv.org/abs/2507.12489)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR模拟方法在传感器特定效果（如滚动快门和激光功率变化）方面表现不足，需要更精确的模型来提升模拟质量。

Method: 通过显式建模LiDAR传感器的特性（如滚动快门、激光功率变化和强度衰减），提出了一种新的LiDAR模拟方法。

Result: 定量和定性比较以及消融研究表明，该方法在LiDAR模拟准确性上优于现有技术，并展示了高级重模拟能力（如生成高分辨率LiDAR扫描）。
...
### [95] [FOUNDER: Grounding Foundation Models in World Models for Open-Ended Embodied Decision Making](https://arxiv.org/abs/2507.12496)
*Yucen Wang,Rui Yu,Shenghua Wan,Le Gan,De-Chuan Zhan*

Main category: cs.RO

TL;DR: FOUNDER框架整合基础模型（FMs）和世界模型（WMs）的互补优势，通过无奖励方式在具身环境中实现开放任务解决。


<details>
  <summary>Details</summary>
Motivation: 利用FMs的通用知识与WMs的动态建模能力，解决复杂观测或领域差距下的任务泛化问题。

Method: 学习映射函数将FM表示嵌入WM状态空间，通过想象学习目标条件策略，并利用预测的时间距离作为奖励信号。

Result: 在多任务离线视觉控制基准上表现优异，尤其在复杂观测或领域差距场景中优于现有方法。

Conclusion: FOUNDER通过结合FMs和WMs的优势，实现了高效的任务泛化和奖励一致性验证。
...
### [96] [ReAL-AD: Towards Human-Like Reasoning in End-to-End Autonomous Driving](https://arxiv.org/abs/2507.12499)
...
### [97] [VLMgineer: Vision Language Models as Robotic Toolsmiths](https://arxiv.org/abs/2507.12644)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何利用基础模型的常识和创造力自动设计工具，以解决机器人智能中的物理问题。

Method: 结合视觉语言模型的代码生成能力和进化搜索，迭代设计工具和操作策略。

Result: VLMgineer在多样化任务中表现优异，优于人工设计的工具和基于人类规格的VLM生成设计。

Conclusion: VLMgineer为自动化工具发明提供了新方向，并发布了基准和代码以促进未来研究。
...
### [98] [MoistureMapper: An Autonomous Mobile Robot for High-Resolution Soil Moisture Mapping at Scale](https://arxiv.org/abs/2507.12716)
*Nathaniel Rose,Hannah Chuang,Manuel A Andrade-Rodriguez,Rishi Parashar,Dani Or,Parikshit Maini*

Main category: cs.RO

TL;DR: 设计并部署了自主移动机器人MoistureMapper，用于土壤湿度传感，结合自适应采样策略，显著减少旅行距离和地图方差。


<details>
  <summary>Details</summary>
Motivation: 现有土壤湿度传感方法成本高且不适合大规模应用，需开发低成本、高效的解决方案。

Method: 使用配备TDR传感器和直接推钻机制的机器人，结合高斯过程建模的自适应采样策略。

Result: 自适应采样策略比贪婪基准方法减少30%旅行距离和5%地图方差。

Conclusion: MoistureMapper和自适应采样策略为土壤湿度传感提供了高效、低成本的解决方案。
...
### [99] [Learning to Predict Mobile Robot Stability in Off-Road Environments](https://arxiv.org/abs/2507.12731)
*Nathaniel Rose,Arif Ahmed,Emanuel Gutierrez-Cornejo,Parikshit Maini*

Main category: cs.RO

TL;DR: 论文提出了一种基于学习的轻量级神经网络IMUnet，直接从本体感受数据估计机器人平台稳定性，无需地形模型或力传感。同时开发了一种基于视觉的ArUco跟踪方法，量化稳定性（C3分数）。
...
Conclusion: 该方法为机器人任务（如农业和太空应用中的移动操作）提供了稳定性估计的新途径，并为感知导航和规划提供了监督机制。
...
### [100] [ASC-SW: Atrous strip convolution network with sliding windows for visual-assisted map navigation](https://arxiv.org/abs/2507.12744)
...
Method: 使用MobileNetV2作为主干网络，设计ASCSPP模块提取DLO特征，结合Atrous Strip Convolution和滑动窗口后处理模块提高识别精度。
...
### [101] [Refining Motion for Peak Performance: Identifying Optimal Gait Parameters for Energy-Efficient Quadrupedal Bounding](https://arxiv.org/abs/2507.12751)
...
Method: 建模Unitree A1机器人，开发可独立调整步态参数的控制器，并在Gazebo中模拟不同速度下的步态。

Result: 优化步态参数可显著降低能量消耗，提升四足机器人运动的整体效率。

Conclusion: 该研究为四足机器人提供了节能控制策略，适用于商业化平台。
...
### [102] [osmAG-LLM: Zero-Shot Open-Vocabulary Object Navigation via Semantic Maps and Large Language Models Reasoning](https://arxiv.org/abs/2507.12753)
*Fujing Xie,Sören Schwertfeger,Hermann Blum*

Main category: cs.RO

TL;DR: 论文提出了一种面向动态环境的机器人导航系统，结合LLM语义先验和在线主动导航，显著提高了动态或未映射对象的检索成功率。


<details>
  <summary>Details</summary>
Motivation: 现有高细节物体地图易过时，动态环境中对象位置变化频繁，需解决这一问题。

Method: 开发了基于环境背景和LLM语义先验的导航系统，采用在线主动策略，不依赖高保真地图。

Result: 在静态和动态对象查询中，检索成功率更高且路径更短，尤其在动态或未映射对象上表现远超先前方法。

Conclusion: 系统通过结合环境背景与语义推理，有效解决了动态环境中对象导航的挑战。
...
### [103] [FFI-VTR: Lightweight and Robust Visual Teach and Repeat Navigation based on Feature Flow Indicator and Probabilistic Motion Planning](https://arxiv.org/abs/2507.12800)
*Jikai Wang,Yunqi Cheng,Zonghai Chen*

Main category: cs.RO

TL;DR: 提出了一种无需精确定位和密集重建的轻量级、鲁棒的视觉重复导航方法，通过特征流映射和概率运动规划实现高效导航。
...
Conclusion: 提出的方法在无需精确定位的情况下实现了高效、鲁棒的视觉重复导航。
...
### [104] [Enter the Mind Palace: Reasoning and Planning for Long-term Active Embodied Question Answering](https://arxiv.org/abs/2507.12846)
...
TL;DR: 论文提出了一种长期主动具身问答（LA-EQA）任务，通过结构化记忆系统和基于信息价值的停止准则，显著提升了机器人在复杂环境中的问答性能。


<details>
  <summary>Details</summary>
Motivation: 研究长期运行的机器人如何利用积累的环境知识更有效地协助人类，解决传统EQA方法在长期记忆和主动探索方面的不足。

Method: 提出基于场景图的世界实例结构化记忆系统，结合推理和规划算法，并引入基于信息价值的停止准则以平衡探索与记忆检索。

Result: 在真实世界实验和新基准测试中，该方法显著优于现有基线，提高了答案准确性和探索效率。

Conclusion: 结构化记忆系统和信息价值准则为长期具身问答提供了有效解决方案，推动了机器人在复杂环境中的实际应用。
...
### [105] [DEMONSTRATE: Zero-shot Language to Robotic Control via Multi-task Demonstration Learning](https://arxiv.org/abs/2507.12855)
...
### [106] [LaViPlan : Language-Guided Visual Path Planning with RLVR](https://arxiv.org/abs/2507.12911)
*Hayeon Oh*

Main category: cs.RO

TL;DR: 论文提出LaViPlan框架，利用强化学习与可验证奖励（RLVR）优化视觉语言模型（VLM），解决其在自动驾驶中高层决策与低层轨迹预测的错位问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中，分布外（OOD）场景可能导致危险行为，现有VLM虽能识别场景但决策常缺乏上下文感知。

Method: 采用RLVR优化VLM，通过规划导向指标解决视觉-语言-动作的错位问题。

Result: 实验表明，LaViPlan提升了OOD条件下的情境感知和决策能力。

Conclusion: 该工作为自动驾驶中的VLM提供了一种有前景的后训练范式。
...
### [107] [MoCap2GT: A High-Precision Ground Truth Estimator for SLAM Benchmarking Based on Motion Capture and IMU Fusion](https://arxiv.org/abs/2507.12920)
...
Result: 实验表明，MoCap2GT优于现有方法，显著提升了SLAM基准测试的精确性。

Conclusion: MoCap2GT为SLAM评估提供了更精确的地面真实轨迹，推动了SLAM技术的发展。
...
### [108] [Non-differentiable Reward Optimization for Diffusion-based Autonomous Motion Planning](https://arxiv.org/abs/2507.12977)
*Giwon Lee,Daehee Park,Jaewoo Jeong,Kuk-Jin Yoon*

Main category: cs.RO

TL;DR: 论文提出了一种基于强化学习的扩散运动规划模型训练方案，以优化不可微分目标（如安全性和有效性），并通过奖励加权动态阈值算法提升性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在运动规划中表现优异，但无法直接优化不可微分目标（如安全性和有效性），因此需要新的训练方法。

Method: 采用强化学习训练扩散模型，引入奖励加权动态阈值算法，以优化安全性和有效性目标。

Result: 在行人数据集（CrowdNav, ETH-UCY）上实现了最先进的性能，优于其他基线方法。

Conclusion: 该方法为安全有效的运动规划提供了一种通用且高效的解决方案。
...
### [109] [Robustness Requirement Coverage using a Situation Coverage Approach for Vision-based AI Systems](https://arxiv.org/abs/2507.12986)
*Sepeedeh Shahbeigi,Nawshin Mannan Proma,Victoria Hodge,Richard Hawkins,Boda Li,Valentina Donzella*

Main category: cs.RO

TL;DR: 提出了一种结合相机噪声因素识别与情境覆盖分析的框架，用于系统化地制定AI感知系统的鲁棒性安全需求，特别针对汽车领域的相机退化问题。


<details>
  <summary>Details</summary>
Motivation: 在复杂动态环境中，AI感知系统的传感器退化会直接影响输入数据质量，进而损害AI推理能力。传统方法难以全面覆盖所有可能的退化场景，导致安全需求存在漏洞。

Method: 整合相机噪声因素识别与情境覆盖分析，结合领域、传感器和安全专家的知识，并纳入操作设计域规范，扩展退化模型以涵盖影响AI性能的噪声因素。

Result: 提出了一种系统性框架，支持鲁棒性需求的制定和完整性评估，特别适用于相机退化的AI感知系统。

Conclusion: 该框架为相机退化场景下的AI感知系统提供了一种基于噪声因素和情境覆盖的鲁棒性需求分析方法，是迈向系统化安全需求制定的初步尝试。
...
### [110] [Rethinking the Embodied Gap in Vision-and-Language Navigation: A Holistic Study of Physical and Visual Disparities](https://arxiv.org/abs/2507.13019)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有视觉与语言导航（VLN）方法在物理机器人部署中的理想化假设问题，提出更现实的评估平台。

Method: 引入VLN-PE平台，评估多种方法，包括分类模型、扩散模型和基于LLM的无训练路径规划。

Result: 结果显示性能显著下降，原因是机器人观测空间有限、环境光照变化及物理挑战（如碰撞和跌倒）。
...
### [111] [What Can Robots Teach Us About Trust and Reliance? An interdisciplinary dialogue between Social Sciences and Social Robotics](https://arxiv.org/abs/2507.13041)
...
TL;DR: 本文主张通过跨学科方法研究人机交互中的信任问题，结合社会科学与机器人学的见解。


<details>
  <summary>Details</summary>
Motivation: 随着机器人融入日常生活，信任问题日益重要，但现有研究缺乏系统性且社会科学与机器人学之间对话不足。

Method: 通过结合社会科学与机器人学的视角，探讨信任的形成、测试与表现。

Result: 提出了一种更全面、适应性更强的信任理解框架。

Conclusion: 跨学科对话有助于建立更扎实的人机信任理论。
...
### [112] [Efficient Online Learning and Adaptive Planning for Robotic Information Gathering Based on Streaming Data](https://arxiv.org/abs/2507.13053)
*Sanjeev Ramkumar Sudha,Joel Jose,Erlend M. Coates*

Main category: cs.RO

TL;DR: 本文提出了一种高效的适应性信息规划方法，用于映射连续标量场，解决了高斯过程在实时性能和大数据集上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有解决方案通常假设环境已知，但实际环境可能是未知或时变的，适应性规划和增量在线映射成为研究重点。

Method: 采用流式稀疏高斯过程（GPs）进行高效适应性信息规划，并通过仿真和真实数据集验证。

Result: 实验表明，该方法在保持与基线相似映射精度的同时，降低了长任务的计算复杂度。

Conclusion: 该方法为未知或时变环境中的信息采集提供了高效解决方案。
...
### [113] [ZipMPC: Compressed Context-Dependent MPC Cost via Imitation Learning](https://arxiv.org/abs/2507.13088)
*Rahel Rickenbach,Alan A. Lahoud,Erik Schaffernicht,Melanie N. Zeilinger,Johannes A. Stork*

Main category: cs.RO

TL;DR: ZipMPC通过为短预测时域MPC学习压缩且上下文相关的成本函数，模仿长时域MPC行为，提升性能并保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决模型预测控制（MPC）在实时系统（如机器人）中因计算负担而限制应用的问题，尤其是短预测时域对控制性能和长期目标设计的影响。

Method: 利用可微分MPC和神经网络传播模仿损失的梯度，学习压缩且上下文相关的成本函数。

Result: 在仿真和真实自动驾驶赛车实验中，ZipMPC表现优于基线方法，尤其在优化长期目标、计算效率、约束满足和泛化能力方面。

Conclusion: ZipMPC能够有效模仿长时域MPC行为，提升控制性能，适用于未见过的环境，且计算成本低。
...
### [114] [GraspGen: A Diffusion-based Framework for 6-DOF Grasping with On-Generator Training](https://arxiv.org/abs/2507.13097)
...
TL;DR: GraspGen是一种基于扩散过程的6-DOF抓取生成框架，通过DiffusionTransformer和高效判别器提升抓取性能，并在模拟和真实机器人中表现优异。


<details>
  <summary>Details</summary>
Motivation: 尽管学习型6-DOF抓取方法取得进展，但仍难以泛化到不同机器人和实际场景，因此需要一种更通用的解决方案。

Method: 提出GraspGen框架，结合DiffusionTransformer和高效判别器，并引入新的训练方法。同时发布包含5300万次抓取的模拟数据集。

Result: GraspGen在模拟和真实机器人中表现优于现有方法，并在FetchBench基准测试中达到最优。
...
### [115] [Aligning Humans and Robots via Reinforcement Learning from Implicit Human Feedback](https://arxiv.org/abs/2507.13171)
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在稀疏奖励条件下效果不佳，且依赖复杂的人工设计奖励函数。现有基于人类反馈的强化学习方法（RLHF）需要显式反馈，增加了用户认知负担。

Method: 提出RLIHF框架，利用非侵入式EEG信号（如错误相关电位ErrPs）生成概率奖励，结合预训练解码器，实现连续隐式反馈。

Result: 在MuJoCo仿真环境中，使用Kinova Gen2机械臂进行复杂抓取任务，结果显示基于EEG反馈的代理性能接近密集人工奖励训练的代理。
...
### [116] [Few-shot transfer of tool-use skills using human demonstrations with proximity and tactile sensing](https://arxiv.org/abs/2507.13200)
...
Method: 通过仿真预训练基础策略捕获工具使用中的常见接触状态，再通过少量真实世界人类演示微调以弥合领域差距。

Result: 验证了该框架能通过少量演示教会机器人使用不同物理和几何特性的工具完成表面跟随任务。

Conclusion: 机器人通过迁移预训练策略中的工具-环境接触关系识别能力学习新技能，多模态传感器组合提升了接触状态和环境几何的识别。
...
### [117] [Signal Temporal Logic Compliant Co-design of Planning and Control](https://arxiv.org/abs/2507.13225)
*Manas Sashank Juvvi,Tushar Dilip Kurne,Vaishnavi J,Shishir Kolathaya,Pushpak Jagtap*

Main category: cs.RO

TL;DR: 提出了一种新颖的协同设计策略，将轨迹规划与控制结合，以处理自主机器人中基于STL的任务。
...
Method: 分两阶段：学习时空运动基元以封装机器人约束，并构建符合STL的运动计划。使用强化学习构建控制策略库，再通过采样生成STL合规运动计划。
...
### [118] [Evaluating Reinforcement Learning Algorithms for Navigation in Simulated Robotic Quadrupeds: A Comparative Study Inspired by Guide Dog Behaviour](https://arxiv.org/abs/2507.13277)
*Emma M. A. Harrison*

Main category: cs.RO

TL;DR: 研究探讨了三种强化学习算法在训练四足机器人自主导航和避障中的效果，发现PPO算法表现最佳。
...
Method: 比较了三种强化学习算法（PPO、DQN、Q-learning），使用自定义环境和统一标准进行评估。

Result: PPO在各项指标中表现最优，尤其在平均和中间步骤数上。
...
### [119] [Latent Policy Steering with Embodiment-Agnostic Pretrained World Models](https://arxiv.org/abs/2507.13340)
*Yiqi Wang,Mrinal Verghese,Jeff Schneider*

Main category: cs.RO

TL;DR: 通过模仿学习视觉运动策略，但依赖大量训练数据。本文提出利用多体现数据（如公开机器人数据集和人类玩耍数据）减少数据收集，结合光流作为体现无关动作表示和潜在策略引导（LPS）提升策略性能。
...
Method: 使用光流作为体现无关动作表示训练世界模型（WM），并在少量目标体现数据上微调；开发潜在策略引导（LPS）优化行为克隆策略。

Result: 在少量数据下显著提升策略性能（30次演示相对提升50%，50次演示提升20%）。

Conclusion: 结合多体现数据预训练的WM和LPS，可高效提升小数据量下的策略性能。
...
### [120] [A Discrete Analog of Tutte's Barycentric Embeddings on Surfaces](https://arxiv.org/abs/2507.13096)
*Éric Colin de Verdière,Vincent Despré,Loïc Dubois*

Main category: cs.CG

TL;DR: 本文提出了一种离散化的Tutte定理，适用于非正曲率曲面，基于减少三角剖分的概念，证明了和谐顶点条件下的弱嵌入性质，并提供了多项式时间算法。


<details>
  <summary>Details</summary>
Motivation: Tutte的质心嵌入定理在平面图中有广泛应用，但其连续性质限制了在非正曲率曲面上的算法化应用。本文旨在填补这一空白。

Method: 基于减少三角剖分的离散化方法，定义和谐顶点作为离散凸位置的类比，并证明其弱嵌入性质。

Result: 证明了和谐顶点条件下的弱嵌入性质，并提供了多项式时间算法实现和谐化。

Conclusion: 本文为Tutte定理在非正曲率曲面上的离散化提供了理论支持，并展示了其算法可行性。
